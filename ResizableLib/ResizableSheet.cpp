...
BEGIN_MESSAGE_MAP(CResizableSheet, CPropertySheet)
	...
	ON_WM_ERASEBKGND()
	//}}AFX_MSG_MAP
	ON_NOTIFY_REFLECT_EX(PSN_SETACTIVE, &ThisClass::OnPageChanging)
END_MESSAGE_MAP()
...
static UINT propButtons_[] =
{
	IDOK, IDCANCEL, ID_APPLY_NOW, IDHELP,
	ID_WIZBACK, ID_WIZNEXT, ID_WIZFINISH
};
const int propButtons_Count = sizeof(propButtons_)/sizeof(UINT);
...
void CResizableSheet::PresetLayout()
{
	...
	for (int i = 0; i < propButtons_Count; i++)
	{
		if (NULL != GetDlgItem(static_cast<int>(propButtons_[i])))
			AddAnchor(propButtons_[i], BOTTOM_RIGHT);
	}
}
...
BOOL CResizableSheet::ArrangeLayoutCallback(LayoutInfo &layout)
{
	...
	layout.hWnd = (HWND)::SendMessage(m_hWnd, PSM_GETCURRENTPAGEHWND, 0, 0);		// NOLINT(performance-no-int-to-ptr) 
...
int CResizableSheet::GetMinWidth()
{
	...
	for (int i = 0; i < 7; i++)
	{
		pWnd = GetDlgItem( static_cast<int>(propButtons_[i]));
...
void CResizableSheet::LoadPage()
{
	...
	int page = static_cast<int>(AfxGetApp()->GetProfileInt(m_sSection, ACTIVEPAGE, 0));
