...
void CResizableSheetEx::PresetLayout()
{
	...
	for (int i = 0; i < 7; i++)
	{
		if (NULL != GetDlgItem((int)propButtons_[i]))
...
BOOL CResizableSheetEx::ArrangeLayoutCallback(LayoutInfo &layout)
{
	...
	layout.hWnd = (HWND)::SendMessage(GetSafeHwnd(), PSM_GETCURRENTPAGEHWND, 0, 0);		// NOLINT(performance-no-int-to-ptr) 
...
int CResizableSheetEx::GetMinWidth()
{
	...
	for (int i = 0; i < 7; i++)
	{
		pWnd = GetDlgItem((int)propButtons_[i]);
...
void CResizableSheetEx::LoadPage()
{
	...
	int page = (int)AfxGetApp()->GetProfileInt(m_sSection, ACTIVEPAGE, 0);
