...
#define BS_TYPEMASK_ 0x0000000FL
...
void CResizableLayout::AddAnchor(HWND hWnd, CSize sizeTypeTL, CSize sizeTypeBR)
{
	...
	ASSERT(sizeTypeTL.operator!=( NOANCHOR ));
	...
	if (sizeTypeBR.operator == ((const CSize)NOANCHOR) )
...
void CResizableLayout::ArrangeLayout()
{
	...
	while (pos != NULL)
	{
		...
		{
			hdwp = ::DeferWindowPos(hdwp, layout.hWnd, NULL, rectChild.left, rectChild.top, rectChild.Width(), rectChild.Height(), uFlags);
		}
	}
...
void CResizableLayout::EraseBackground(CDC* pDC)
{
	...
	if (atomWndClass == (ATOM)0x8002)
	{
		...
		hBrush = (HBRUSH)::SendMessage(hWnd, WM_CTLCOLORDLG, (WPARAM)pDC->GetSafeHdc(), (LPARAM)hWnd);	// NOLINT(performance-no-int-to-ptr)
	}
	else
	{
		// take the background brush from the window's class
		hBrush = (HBRUSH)::GetClassLongPtr(hWnd, GCLP_HBRBACKGROUND );	// NOLINT(performance-no-int-to-ptr)
	}
									...
									const CRect& rectOld, const CRect& rectNew)
{
	...
	if (bHScroll && (nDiffWidth > 0))
	{
		...
		if (::GetScrollInfo(layout.hWnd, SB_HORZ, &info))
		{
			...
			info.nMax -= (int)__max(info.nPage-1,0);
		}
								...
								const CRect &rectParent, CRect &rectChild, UINT& uFlags)
{
	...
	if (rectNew.TopLeft().operator == ((const CPoint)rectChild.TopLeft()) )
		uFlags |= SWP_NOMOVE;
	if (rectNew.Size().operator == ((const CSize)rectChild.Size()))
		uFlags |= SWP_NOSIZE;
