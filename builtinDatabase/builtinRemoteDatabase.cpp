...
static int remoteDbFieldInfoByName ( REMOTE_DB *db, DBS_MSG *msg )
{
	...
	for ( loop = 0; loop < db->numFields; loop++ )
	{
		if ( !fglstrccmp ( msg->dat.req.dbFieldInfoByName.fieldName, db->fields[loop].name ) )
		{
			...
			msg->dat.rsp.dbFieldInfo.fieldLen	= (int)db->fields[loop].leninfo.num.len;
			msg->dat.rsp.dbFieldInfo.fieldDec	= (char)db->fields[loop].leninfo.num.dec;
...
static int remoteDbFieldInfoByPos ( REMOTE_DB *db, DBS_MSG *msg  )
{
	...
	if ( (fNum >= 0) && (fNum < db->numFields) )
	{
		...
		msg->dat.rsp.dbFieldInfo.fieldLen	= db->fields[fNum].leninfo.num.len;
		msg->dat.rsp.dbFieldInfo.fieldDec	= (char)db->fields[fNum].leninfo.num.dec;
	...
	if ( (num > 0) && (num <= db->numFields) )
	{
		...
		switch(db->fields[num].type)
		{
			case 'B':
				...
				return 1 ;
			case 'N' :
				len = db->fields[num].leninfo.num.len;
				offset = db->fields[num].offset;
				break;
			case 'S':
...
static int _remoteDbMakeValid ( REMOTE_DB *db )
{
	DBS_MSG		 recGet{};
...
static int _remoteDbFieldPut ( REMOTE_DB *db, int num, char const *buff, int len )
...

{
	...
	if ( fInfo )
	{
		...
		if ( fType == 'N' )
		{
			fLen  = (char)fInfo->lenInfo.num.len;
			fDec  = (char)fInfo->lenInfo.num.dec;
		} else
		{
	...
	} else
	{
		...
		fType = (char)fieldInfo.dat.rsp.dbFieldInfo.fieldType;
	...
	if ( !_remoteDbFieldPut ( db, pos, bufferBuff ( &db->buffer ), (int)bufferSize ( &db->buffer ) ) )
	{
		throw 0;
	}
#if 0
...

{
	...
	switch ( fType )
	{
		...
		case 'N':					
			...
			if ( (resVal->dat.d < (double) 0x7FFFFFFF) && !modf ( resVal->dat.d, &dTmp ) )
			{
				resVal->type  = slangType::eLONG;
			...
			break;
		case 'S':
			res = (char *)instance->om->alloc ( sizeof ( char ) * (fLen + 1) );
...
//	EVP_Digest ( userId, strlen ( userId ), (unsigned char *)dbConnMsg.passHash, NULL, EVP_sha1(), NULL );
	...
	{
		case 'N':
			bufferMakeRoom ( &db->recordBuff, db->fields[db->numFields-1].offset + db->fields[db->numFields - 1].leninfo.num.len );
			bufferAssume ( &db->recordBuff, db->fields[db->numFields-1].offset + db->fields[db->numFields - 1].leninfo.num.len );
			break;
		default:
			bufferMakeRoom ( &db->recordBuff, db->fields[db->numFields-1].offset + db->fields[db->numFields - 1].leninfo.fieldlen );
			bufferAssume ( &db->recordBuff, db->fields[db->numFields-1].offset + db->fields[db->numFields - 1].leninfo.fieldlen );
			break;
	}
...
static int remoteDbFillFieldCache ( class vmInstance *instance, REMOTE_DB *db, char const *name, DBFIELDINFO *dbFieldInfo )
{
	...
	if ( (dbFieldInfo->fieldType = (char)fieldInfo.dat.rsp.dbFieldInfo.fieldType) == 'N' )
	{
...
{
	instance->debug->debugRegisterInspector ( "Database", remoteDbInspector );
