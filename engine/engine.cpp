...
UINT __cdecl testCompile ( LPVOID param )
{
	...
	BUFFER				 buff ( 1024 * 1024 * 4 );
	BUFFER				 objBuff ( 1024 * 1024 * 4 );
	bool				 isAP;
	...
	for ( ; !exiting && --ctr; )
	{
		for ( auto fIt = fList.begin (); fIt != fList.end () && !exiting; fIt++ )
		{
			...
			if ( !entry )
			{
				printf ( "unable to open file: %s", *fIt );
				continue;
			}
			...
			printf ( "front end time = %I64u ms\r\n", (end.QuadPart - start.QuadPart) / frequency.QuadPart );

			objBuff.reset();
			oFile.serialize ( &objBuff, false );

			printf ( "object file size = %zu\r\n", bufferSize ( &objBuff ) );
			...
			for ( uint32_t loop = 0; loop < 1 && !exiting; loop++ )
			{
				QueryPerformanceCounter ( &start );
				{
...

{
	...
	if ( !freopen ( "CONOUT$", "w", stdout ) )
	{
		...
		/* give it a title */
#if _DEBUG
		sprintf_s ( titleString, sizeof ( titleString ), "Slang MultiServer - v%u.%u.%u %s %s - DEBUG", HIWORD ( VerFixedFileInfo->dwFileVersionMS ), LOWORD ( VerFixedFileInfo->dwFileVersionLS ), SVN_REVISION, SVN_BUILD_TIME, SVN_LOCAL_MODIFICATIONS ? " - NON-COMMITTED" : "" );
#else
		sprintf_s ( titleString, sizeof ( titleString ), "Slang MultiServer - v%u.%u.%u %s %s", HIWORD ( VerFixedFileInfo->dwFileVersionMS ), LOWORD ( VerFixedFileInfo->dwFileVersionLS ), SVN_REVISION, SVN_BUILD_TIME, SVN_LOCAL_MODIFICATIONS ? " - NON-COMMITTED" : "" );
#endif
	...
	for ( auto &it : vmConf.servers )
	{
		switch ( it.type )
		{
			...
			case vmServers::debug:
				...
				break;
			default:
				printf ( "unsupported server type: %i\r\n", it.type );
				break;
		}
	}
	...
	for ( loop = 0; loop < params.size (); loop++ )
	{
		if ( params[loop][0] == '-' )
		{
			...
			} else if ( !strccmp ( params[loop], "--register" ) )
			{
				RegisterAsService ();
				doExec = false;
			} else if ( !strccmp ( params[loop], "--deregister" ) )
			{
				DeRegisterAsService ();
				loop++;
				doExec = false;
			} else if ( !strccmp ( params[loop], "--unregister" ) )
			{
				DeRegisterAsService ();
				loop++;
				doExec = false;
