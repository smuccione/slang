...
moduleLibraryEntry::moduleLibraryEntry ( class opFile *file, char const **expr, bool isLibrary )
{
	...
	if ( isLibrary )
	{
		...
		if ( !strccmp ( p.extension ().string ().c_str (), ".flb" ) || !strccmp ( p.extension ().string ().c_str (), ".slb" ) )
		{
			p.replace_extension ( ".sll" );
		}
		fName = file->sCache.get ( p.string().c_str() );
	}
...
void opFile::addStream( char const ** expr, bool isInterface, bool isLS )
{
	...
	for (; loop; loop-- )
	{
		oFunc = new opFunction ( this, &srcFiles, expr );
	...
	for (; loop; loop-- )
	{
		oClass = new opClass ( this, &srcFiles, expr );
...
void opFile::loadLibraries ( bool isInterface, bool isLS )
{
	for ( auto & it : libraries )
	{
		if ( !it.isLoaded )
		{
			auto entry = globalFileCache.read ( it.fName.operator const stringi &() );

			if ( !entry )
			{
				errorLocality e ( &errHandler, *it.locations.begin () );
				if ( !isLS ) throw (errorNum)GetLastError ();
				errors.push_back ( std::make_unique<astNode> ( errorNum(GetLastError ()), *it.locations.begin() ) );
				continue;
			}
...
errorNum opFile::addElem ( char const *lib, srcLocation &location, bool isModule, bool isLS )
{
	...
	if ( isModule )
	{
		if ( !strccmp ( p.extension ().string ().c_str (), ".flb" ) )
		{
			p.replace_extension ( ".slb" );
		}
		for ( auto it = modules.begin ( ); it != modules.end ( ); it++ )
		{
			if ( (*it).fName == p.string().c_str() )
			{
	...
	} else
	{
		...
		libraries.push_back ( moduleLibraryEntry ( sCache.get ( stringi ( p.string () ) ), location ) );
	}
			...
			try
			{
				switch ( it->second.type )
				{
					...
					case statementType::stObject:
						{
							...
							func = parseFunc ( src, name, doBraces, isLS );
					...
					case statementType::stClass:
						{
							...
							auto cls = parseClass ( src, doBraces, isLS );
					...
					case statementType::stNamespace:
						{
							if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeNamespace, statementLocation ) );
							if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::keywordOther, astLSInfo::semanticLineBreakType::beforeNamespace, statementLocation ) );
							...
							if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterNamespace, src ) );
							BS_ADVANCE_EOL_COMMENT ( this, isLS, src );
							...
							if ( doBraces )
							{
								...
								{
									if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeOpenBlock, statementLocation ) );
									if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::punctuation, src ) );
									src++;
									if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterOpenBlock, statementLocation ) );
								}
							} else
							{
								if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeImpliedBlock, statementLocation ) );
							}
							...
							{
								if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterImpliedBlock, statementLocation ) );
							}
							ns.popNamespace ();
						}
						break;
					case statementType::stUsing:
						{
							if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeStatement, statementLocation ) );
							if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::keywordOther, statementLocation ) );
							BS_ADVANCE_EOL_COMMENT ( this, isLS, src );
							srcLocation nameLocation = src;
							try
					...
					case statementType::stCloseBrace:
						...
						errors.push_back ( std::make_unique<astNode> ( errorNum::scSYNTAX_EXTRA_ON_LINE, src ) );
						if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterBlock, src ) );
						if ( !doBraces ) throw errorNum::scSYNTAX_EXTRA_ON_LINE;
						break;
					case statementType::stPragma:
						if ( isLS )
						{
							statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeStatement, statementLocation ) ); 
...
autoMain:
			...
			func = parseFuncAutoMain ( src, "main", doBraces, isLS );
