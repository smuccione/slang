...
struct linqStatement 
{
	...
	linqStatement ( linqStatementType type ) : type ( type )
	{
...
astNode *opFile::makeLambda ( source &src, opFunction *func, std::vector<linqProjectionVars> &projVars, astNode *expr, astNode *secondParam )
{
	char		 tmpName[512];	
	opFunction	*lambda = new opFunction ( this );
	astNode		*node;

	sprintf_s ( tmpName, sizeof ( tmpName ), LAMBDA_ID "%s@%i:%u:%I64u", srcFiles.getName ( expr->location.sourceIndex ).c_str ( ), expr->location.lineNumberStart, expr->location.columnNumber, GetUniqueCounter() );
	...
	if ( projVars.size() > 1 )
	{
		...
		{
			param.add ( it->name->symbolValue(), symVariantType, it->name->location, new astNode ( sCache, astOp::symbolValue, it->name->symbolValue () ), false );
			it->name->location = srcLocation ();
		}
		sym.push ( &param );

		lambda->params.add ( new astNode ( sCache, astOp::symbolValue, tidIdentifier ), symVariantType );

		expr = expr->symbolsToIVar ( sCache.get ( tidIdentifier ), sym, true );
	} else
	{
		lambda->params.add ( (new astNode ( sCache, astOp::symbolValue, projVars[0].name->symbolValue () ))->setLocation ( projVars[0].name->location ), symVariantType );
		projVars[0].name->location = srcLocation ();
	}

	if ( secondParam )
	{
		lambda->params.add ( secondParam->symbolValue(), symVariantType, secondParam->location, (new astNode ( sCache, astOp::symbolValue, secondParam->symbolValue () ))->setLocation ( secondParam->location ), false );
...
parseFrom:
							{
								linqStatement statement ( linqStatementType::stFrom );
								...
								if ( !cmpTerminated ( src, "in", 2 ) )
								{
									...
									{
									} else throw errorNum::scLINQ_MISSING_IN;
						...
						case linqStatementType::stJoin:
							{
								linqStatement statement ( linqStatementType::stJoin );
						...
						case linqStatementType::stGroup:
							{
								linqStatement statement ( linqStatementType::stGroup );
						...
						case linqStatementType::stWhere:
							{
								linqStatement statement ( linqStatementType::stWhere );
						...
						case linqStatementType::stLet:
							{
								linqStatement statement ( linqStatementType::stLet );
						...
						case linqStatementType::stOrder:
							{
								linqStatement statement ( linqStatementType::stOrder );
						...
						case linqStatementType::stSelect:
							{
								linqStatement statement ( linqStatementType::stSelect );
		...
		for ( it++; it != linqNodes.end (); it++ )
		{
			...
			switch ( (*it).type )
			{
				...
				case linqStatementType::stFrom:
					...
					pList = &result->pList ();
					pList->param.push_back ( makeLambda ( src, func, projVar, (*it).from.expr ) );
					...
					if ( (*itNext).type == linqStatementType::stSelect )
					{
						...
						pList->param.push_back ( makeLambda ( src, func, projVar, (*itNext).select.expr, (*it).from.var ) );
						pList->paramRegion.push_back ( {} );
					...
					} else
					{
						...
						pList->param.push_back ( makeLambda ( src, func, projVar, arr, (*it).from.var ) );
						pList->paramRegion.push_back ( {} );
				...
				case linqStatementType::stLet:
					{
						...
						pList->param.push_back ( makeLambda ( src, func, projVar, arr ) );
						pList->paramRegion.push_back ( {} );
				...
				case linqStatementType::stWhere:
					{
						...
						pList->param.push_back ( makeLambda ( src, func, projVar, expr ) );
						pList->paramRegion.push_back ( {} );
					}
					break;
				case linqStatementType::stOrder:
					{
						...
						pList->param.push_back ( makeLambda ( src, func, projVar, (*it2).node ) );
						pList->paramRegion.push_back ( {} );
				...
				case linqStatementType::stSelect:
					...
					pList->param.push_back ( makeLambda ( src, func, projVar, (*it).select.expr ) );
					pList->paramRegion.push_back ( {} );
				...
				case linqStatementType::stGroup:
					...
					pList->param.push_back ( makeLambda ( src, func, projVar, (*it).group.by ) );
					pList->paramRegion.push_back ( {} );
					pList->param.push_back ( makeLambda ( src, func, projVar, (*it).group.expr ) );
					pList->paramRegion.push_back ( {} );
				...
				case linqStatementType::stJoin:
					if ( (*it).join.into )
					{
						...
						if ( (*itNext).type == linqStatementType::stSelect )
						{
							...
							pList->paramRegion.push_back ( {} );
							pList->param.push_back ( makeLambda ( src, func, projVar, (*it).join.on ) );		// x1 => k1
							pList->paramRegion.push_back ( {} );

							std::vector<linqProjectionVars> p2;
							p2.push_back ( (*it).join.var );
							pList->param.push_back ( makeLambda ( src, func, p2, (*it).join.equals ) );		// x2 => k2
							pList->paramRegion.push_back ( {} );

							pList->param.push_back ( makeLambda ( src, func, projVar, (*itNext).select.expr, (*it).join.into ) );		// (x1, g ) => v
							pList->paramRegion.push_back ( {} );
						...
						} else
						{
							...
							pList->param.push_back ( makeLambda ( src, func, projVar, arr, (*it).join.into ) );		// ( x1, g ) => new { x1 , g }
							pList->paramRegion.push_back ( {} );
						}
					} else
					{
						...
						if ( (*itNext).type == linqStatementType::stSelect )
						{
							...
							pList->param.push_back ( makeLambda ( src, func, projVar, (*itNext).select.expr, (*it).join.var ) );		// (x1, x2) => v
							pList->paramRegion.push_back ( {} );
						...
						} else
						{
							...
							pList->param.push_back ( makeLambda ( src, func, projVar, arr, (*it).join.var ) );		// (x1, x2) => v
							pList->paramRegion.push_back ( {} );
						}
					}
					break;
