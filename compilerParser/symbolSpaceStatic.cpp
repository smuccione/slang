...
symbolStatic::symbolStatic ( opFile *file, cacheString const &qualPrefix, cacheString const &name, symbolTypeClass const &type, srcLocation const &location, stringi const &documentation, class astNode *initializer ) : symbol ( symbolSpaceType::symTypeStatic ), file ( file ), name ( name )
{
	if ( !initializer )
	{
		initializer = new astNode ( astOp::symbolValue, name );
	}
	qualName = file->sCache.get ( (stringi) qualPrefix + (stringi) name );
	file->symbols.insert( { qualName, opSymbol( opSymbol::symbolClass::symbolClass_functionStatic, name, location, type, initializer, false, false, false, documentation ) } );
}

symbolStatic::symbolStatic ( opFile *file, cacheString const &qualPrefix, class astNode *initializer, symbolTypeClass const &type, stringi const &documentation ) : symbol ( symbolSpaceType::symTypeStatic ), file ( file )
{
	...
	file->symbols.insert( { qualName, opSymbol( opSymbol::symbolClass::symbolClass_functionStatic, name, initializer->location, type, initializer, false, false, false, documentation ) } );
}
...
{
//	file->symbols[qualName].setAccessed ( qualName, true, acc, scanQueue );
}
...
{
	assert ( this->name == name );
//	file->symbols[qualName].setAccessed ( name, isAccess, acc, scanQueue );
}
...
bool symbolStatic::isAccessed ( cacheString const &name, bool isAccess ) const
{
	assert ( this->name == name );
	return true;
//	return !file->symbols[qualName].accessors.empty();
}
