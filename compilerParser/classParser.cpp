...
opClassElement::opClassElement ( class opFile *file, char const ** expr, opClass *classDef, class sourceFile *src )
{
	...
	new (&location) srcLocation ( src, expr );
...
void opFile::parseProperty (source &src, opClass *classDef, bool isStatic, bool isVirtual, fgxClassElementScope scope, bool isLS, stringi const &topDocumentation )
{
				...
				try
				{
					switch ( it->second.type )
					{
						...
						case statementType::stSet:
							...
							if ( isStatic )
							{
								func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), name.c_str (), "assign" ).c_str (), true, isLS );
								func->isStatic = true;
							} else
							{
								func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), name.c_str (), "assign" ).c_str (), true, isLS );
							}
						...
						case statementType::stGet:
							if ( isStatic )
							{
								func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), name.c_str (), "access" ).c_str (), true, isLS );
								func->isStatic = true;
							} else
							{
								func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), name.c_str (), "access" ).c_str (), true, isLS );
							}
							func->nameLocation = statementLocation;
							func->location = statementLocation;
							func->location.setEnd ( func->codeBlock->location );
...
bool opFile::parseInnerClass ( source &src, bool doBraces, opClass *classDef, bool isLS )
{
	cacheString				 name;
					...
					switch ( it->second.type )
					{
						...
						case statementType::stIterator:
							...
							if ( isStatic && isVirtual ) throw;
							{
								...
								{
									func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), tmpName, "iterator" ).c_str (), doBraces, isLS );
								} else
								{
									func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), tmpName, "iterator" ).c_str (), doBraces, isLS );
								}
						...
						case statementType::stMethod:
							...
							} else
							{
								...
								if ( isStatic || isExtension )
								{
									...
									func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), sCache.get ( tmpName ), "method" ).c_str (), doBraces, isLS );
								} else
								{
									func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), sCache.get ( tmpName ), "method" ).c_str (), doBraces, isLS );
								}
								func->isStatic = isStatic;
								func->nameLocation = nameLocation;
								func->location = statementLocation;
						...
						case statementType::stObject:
							...
							} else
							{
								...
								if ( isStatic )
								{
									func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), tmpName, "method" ).c_str (), doBraces, isLS );
								} else
								{
									func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), tmpName, "method" ).c_str (), doBraces, isLS );
								}
						...
						case statementType::stProperty:
							...
							parseProperty ( src, classDef, isStatic, isVirtual, scope, isLS, documentation );
							BS_ADVANCE_EOL ( this, isLS, src );
							break;
						case statementType::stAssign:
							...
							} else
							{
								...
								if ( isStatic )
								{
									func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), tmpName, "assign" ).c_str (), doBraces, isLS );
									func->isStatic = true;
								} else
								{
									func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), tmpName, "assign" ).c_str (), doBraces, isLS );
								}
						...
						case statementType::stAccess:
...

							{
								...
								if ( isStatic )
								{
									func = parseFunc ( src, buildString ( (char *)classDef->name.c_str (), tmpName, "access" ).c_str (), doBraces, isLS );
									func->isStatic = true;
								} else
								{
									func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), tmpName, "access" ).c_str (), doBraces, isLS );
								}
						...
						case statementType::stOperator:
							{
								...
								func = parseMethod ( src, classDef, buildString ( (char *)classDef->name.c_str (), tmpName, "operator" ).c_str (), doBraces, isLS );
...
opClass *opFile::parseClass ( source &src, bool doBraces, bool isLS )
{
	opClass		*classDef;
	...
	if ( !parseInnerClass ( src, doBraces, classDef, isLS ) )
	{
		if ( !isLS ) throw errorNum::scNOT_LEGAL_INSIDE_CLASS;
		errors.push_back ( std::make_unique<astNode> ( errorNum::scNOT_LEGAL_INSIDE_CLASS, src ) );
	} 
