...
private:
	...
	astNode			*makeLambda			( source &src, opFunction *func, std::vector<linqProjectionVars> &projVars, astNode *expr, astNode *secondParam = 0 );
	...
	opFunction		*_parseFunc			( source &src, char const *funcName, bool parseInitializers, bool autoMain, bool doBraces, bool cbParam, bool isLS, class opClass *classDef = 0 );
	opFunction		*parseFunc			( source &src, char const *funcName, bool doBraces, bool isLS );
	opFunction		*parseCBFunc		( source &src, char const *funcName, bool doBraces, bool isLS );
	opFunction		*parseAnonymousFunc	( source &src, char const *funcName, bool doBraces, bool isLS );
	opFunction		*parseFuncAutoMain	( source &src, char const *funcName, bool doBraces, bool isLS );

	opClass			*parseClass			( source &src, bool doBraces, bool isLS );
	bool			 parseInnerClass	( source &src, bool doBraces, opClass *classDef, bool isLS );
	void			 parseProperty		( source &src, opClass *oClass, bool isStatic, bool isVirtual, fgxClassElementScope scope, bool isLS, stringi const &documentation );
	class astNode	*parseBlockFGL		( source &src, opFunction *func, bool noImplied, bool isLS );
	class astNode	*parseBlockSlang	( source &src, opFunction *func, bool virtualBrace, bool isLS );
	class astNode	*parseBlock			( source &src, opFunction *func, bool doBrace, bool virtualBrace, bool isLS );
...
public:
	...
	opFunction		*parseMethod		( source &src, class opClass *classDef, char const *name, bool doBraces, bool isLS );
	...
	friend bool compExecutable::compMakeIterator ( opFile *file, opFunction *func, bool generic, accessorType const &acc, unique_queue<accessorType> *scanQueue, bool isLS );
	friend astNode *compileCodeBlockCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, class errorHandler *err );
	...
	std::tuple<stringi, int64_t, srcLocation> formateCode ( astLSInfo const &info, char const *src, srcLocation const &loc, int64_t defaultIndent, languageRegion::languageId const languageId, int64_t tabWidth, bool preferSpaces );
};
