...
opFunction::opFunction ( class opFile* file, sourceFile* srcFile, char const** expr )
{
	...
	new (&documentation) stringi ( expr );

	new (&location) srcLocation ( srcFile, expr );
	new (&nameLocation) srcLocation ( srcFile, expr );
...
std::vector<std::pair<stringi, stringi>> opFunction::getParameterHelp()
{
	std::vector<std::pair<stringi, stringi>>	sig;
	...
	for ( uint32_t loop = classDef ? 1 : 0; loop < params.size (); loop++ )
	{
		...
		p += " ";
		p += params[loop]->name;
		sig.push_back ( { p, params[loop]->getDocumentation () } );
	}
	return sig;
}
...
opFunction *opFile::_parseFunc( source &src, char const *name, bool parseInitializers, bool autoMain, bool doBraces, bool cbParams, bool isLS, opClass *classDef )
{
	opFunction *func;
	...
	try
	{
					...
					while ( 1 )
					{
						...
						try
						{
							if ( (*src == '.') && !memcmp( src + 1, "..", 2 ) )
							{
								...
								func->params.add( varArg, symArrayType );
							...
							} else if ( doBraces )
							{
								if ( cmpTerminated( src, "integer", 7 ) )
								{
									...
									(src) += 7;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symIntType );
								} else if ( cmpTerminated( src, "float", 5 ) )
								{
									...
									(src) += 5;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symDoubleType );
								...
								} else if ( cmpTerminated( src, "string", 6 ) )
								{
									...
									(src) += 6;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symStringType );
								} else if ( cmpTerminated( src, "array", 5 ) )
								{
									if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::type, src, 5 ) );
									(src) += 5;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symArrayType );
								} else if ( cmpTerminated( src, "codeblock", 9 ) )
								{
									...
									(src) += 9;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symCodeblockType );
								} else if ( cmpTerminated( src, "object", 6 ) )
								{
									if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::type, src, 6 ) );
									(src) += 6;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symObjectType );
								} else if ( cmpTerminated( src, "variant", 7 ) || (doBraces && cmpTerminated( src, "var", 3 )) )
								{
									if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::type, src, 7 ) );
									(src) += 7;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symVariantType );
								} else if ( cmpTerminated( src, "function", 8 ) )
								{
									...
									(src) += 8;
									BS_ADVANCE_EOL_COMMENT( this, isLS, src );
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symFuncType );
								} else
								{
									func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symUnknownType );
								}
							} else
							{
								func->params.add( parseExpr( src, false, false, func, doBraces, isLS ), symUnknownType );
							...
							if ( initializer )
							{
								switch ( initializer->getOp() )
								{
									case astOp::funcCall:
										...
										if ( initializer->pList().param.size() != 1 )
										{
											if ( !isLS ) throw errorNum::scILLEGAL_METHOD_INITIALIZER;
											errors.push_back( std::make_unique<astNode>( errorNum::scILLEGAL_METHOD_INITIALIZER, *initializer ) );
										}
										break;
		...
		func->codeBlock = parseBlock( src, func, doBraces, autoMain, isLS );
...
opFunction* opFile::parseFunc ( source &src, char const* name, bool doBraces, bool isLS )
{
	return (_parseFunc ( src, name, false, false, doBraces, false, isLS ));
}

opFunction* opFile::parseCBFunc ( source &src, char const* name, bool doBraces, bool isLS )
{
	return (_parseFunc ( src, name, false, false, doBraces, true, isLS ));
}

opFunction* opFile::parseAnonymousFunc ( source &src, char const* name, bool doBraces, bool isLS )
{
	opFunction* func;
	func = _parseFunc ( src, name, false, false, doBraces, false, isLS );
	return func;
}

opFunction* opFile::parseFuncAutoMain ( source &src, char const* name, bool doBraces, bool isLS )
{
	return (_parseFunc ( src, name, false, true, doBraces, false, isLS ));
}

opFunction* opFile::parseMethod ( source &src, opClass* classDef, char const* name, bool doBraces, bool isLS )
{
	opFunction* func;
	char		 methodName[64];
	source srcSave = src;
	...
	{
		func = _parseFunc ( src, name, true, false, doBraces, false, isLS, classDef );
	} else
	{
		func = _parseFunc ( src, name, false, false, doBraces, false, isLS, classDef );
	}
	func->params.add ( sCache.get ( "self" ), symbolTypeClass ( symbolType::symObject, classDef->name ), srcLocation ( srcSave ), (new astNode ( sCache, astOp::symbolValue, "self" ))->setLocation ( srcLocation ( srcSave ) ), true );
	return func;
}
