...
astNode *opFile::getNode ( source &src, lastOpType lastOp, opFunction *func, bool doSlang, bool isLS )
{
	...
	try
	{
		if ( (*src == '\"') || (src[0] == '$' && src[1] == '"') || (src[0] == 'R' && src[1] == '"') || (src[0] == 'F' && src[1] == '"') || (src[0] == '$' && src[1] == 'R' && src[2] == '"') )
		{
			...
			if ( multiLine )
			{
				multiLineTerminater.put ( ')' );	//we're building a terminator which ends with ")<text>"
				// build the <text> part
				while ( src[0] != '(' )
				{
					if ( !src[0] || src[0] == '(' )
					...
					multiLineTerminater.put ( *(src++) );
				}
				src++;	// consume the '('
				// add in the final terminating "
				multiLineTerminater.put ( '"' );
			}
			...
			while ( *src )
			{
				...
				{
					break;
				} else if ( multiLine && !memcmp ( src, multiLineTerminater.data<char *> (), multiLineTerminater.size () ))
				{
					// consume the terminator
					src += multiLineTerminater.size ();
					break;
				}
				if ( !isRaw && *src == '\\' )
				{
			...
			} else
			{
				...
				} else if ( !strccmp ( node->symbolValue (), "new" ) )
				{
					...
					if ( *src == '{' )
					{
						...
						auto newFunc = parseMethod ( fSrc, classDef, buildString ( classDef->name.c_str (), elem->name, "method" ).c_str (), true, isLS );
					...
					} else if ( *src != '(' )
					{
						...
						if ( *src == '(' )
						{
							...
							while ( *src != ')' )
							{
								...
								{
									node->pList ().paramRegion.pop_back ();
								}
					...
					return node;
				} else if ( !strccmp ( node->symbolValue (), "throw" ) )
				{
					node->setOp ( sCache, astOp::funcCall );
					node->pList ().paramRegion.push_back ( src );
					BS_ADVANCE_EOL_COMMENT ( this, isLS, src );
					node->pList ().param.push_back ( _parseExpr ( src, false, false, false, false, false, func, doSlang, isLS ) );
					BS_ADVANCE_EOL_COMMENT ( this, isLS, src );
					node->pList ().paramRegion.back ().setEnd ( src );
					node->left = (new astNode ( sCache, astOp::atomValue, "except" ))->setLocation ( node );
				} else if ( !strccmp ( node->symbolValue (), "function" ) )
				{
					...
					auto bound = parseFunc ( src, tmpName, doSlang, isLS );
...
//								node->pList ().param.push_back ( newNode );
		...
		} else
		{
			...
			{
				if ( (*opList[loop].c == *src) && !memcmp ( opList[loop].c + 1, src + 1, opList[loop].cLen - 1) )
				{
...
static errorNum seqToParam ( astNode *node, symbolSpaceParams &param )
{
	switch ( node->getOp ( ) )
	{
		...
		case astOp::symbolValue:
			try
			{
				param.add ( node->symbolValue (), symVariantType, node->location, new astNode ( *node ), false );
			} catch ( errorNum err )
			{
				return err;
			}
		...
		while ( *src )
		{
			...
			if ( !node )
			{
				...
				}
				{
					...
					ioStack.push ( new astNode ( errorNum::scUNBALANCED_PERENS, elem->location ) );
			...
			switch ( node->getOp ( ) )
			{
				...
				case astOp::closePeren:
					{
	#if 0
						// WHY OH WHY are we doing this??? this doesn't seem right... why didn't i put a comment in!!!!
			...
			while ( poStack.elem.size ( ) )
			{
				...
				switch ( node->getOp ( ) )
				{
					...
					case astOp::greaterEqual:
						...
						{
							case 0:
								if ( isLS ) operand2 = (new astNode ( errorNum::scMISSING_SYMBOL, node->location ));
								if ( isLS ) operand1 = (new astNode ( errorNum::scMISSING_SYMBOL, node->location ));
								if ( !isLS ) throw errorNum::scMISSING_SYMBOL;
								errHandler.setFatal ();
								break;
							case 1:
								operand1 = evalStack.popBack();
								if ( isLS ) operand2 = (new astNode( errorNum::scMISSING_SYMBOL, operand1->isValueType() ? node->location : operand1->location ));
								if ( !isLS ) throw errorNum::scMISSING_SYMBOL;
								errHandler.setFatal ();
								break;
							default:
