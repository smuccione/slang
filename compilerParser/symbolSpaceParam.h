...
class symbolParamDef {
	...
	public:
	symbolParamDef ( cacheString const &name, symbolTypeClass const &type, srcLocation const &location, class astNode *init ) : name ( name ), location ( location ), initializer ( init ), type ( type ), localType ( type )
	{
	}
...
public:
	...
	symbolSpaceParams ( const symbolSpaceParams &old ) : symbol ( symbolSpaceType::symTypeParam )
	{
		...
		{
			symbols.push_back ( new symbolParamDef ( *it ) );
		}
	}
	...
	void addStream ( opFile *file, class sourceFile *srcFile, char const ** expr )
	{
		...
		for ( ; cnt; cnt-- )
		{
			symbols.push_back ( new symbolParamDef ( file, srcFile, expr ) );
		}
	}
	...
	void add ( cacheString const &name, symbolTypeClass const &type, srcLocation const &location, astNode *init, bool front )
	{
		assert ( init );
		...
		if ( front )
		{
			symbols.insert ( symbols.begin ( ), new symbolParamDef ( name, type, location, init ) );
		} else
		{
			symbols.push_back ( new symbolParamDef ( name, type, location, init ) );
		}
	}
	void add ( astNode *node, symbolTypeClass const &type, bool front = false )
	{
		if ( !node ) throw errorNum::scBAD_VARIABLE_DEF;
		switch ( node->getOp() )
		{
			case astOp::symbolValue:
				add ( node->symbolValue(), type, node->location, node, front );
				break;
			case astOp::assign:
				...
				add ( node->left->symbolValue(), type, node->location, node, front );
				break;
			default:
				throw errorNum::scBAD_VARIABLE_DEF;
		}
	...
	symbolParamDef *operator [] ( cacheString const & name ) const
	{
		for ( auto &it : symbols )
		{
			if ( it->name == name )
	...
	symbolParamDef *operator [] ( uint32_t index )
	{
	...
	{
		(*this)[name]->setAccessed ( acc, scanQueue );
	}
	...
	{
		auto *sym = (*this)[oldName];
		sym->name = newName;

		sym->initializer->renameLocal ( oldName, newName );
	}
