...
static astNode *findExtentCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, srcLocation &location )
{
...
void errorHandler::throwFatalError ( bool isLS, errorNum errNum )
{
	if ( !isLS )
	{
		...
		printf ( ": %sError E%08x - %s\r\n", windowsToFgColorCode[int ( consoleColor::Red )], errNum, scCompErrorAsText ( size_t ( errNum ) ).c_str () );
		printf ( "%s", windowsToFgColorCode[int ( consoleColor::DarkGray )] );
