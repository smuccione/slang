...
astNode *opFile::parseBlockFGL ( source &src, opFunction *func, bool noImplied, bool isLS )
{
	astNode						*nodeChild;
	int							 defaultPresent;
				...
				try
				{
					switch ( it->second.type )
					{
							...
							while ( 1 )
							{
								...
								if ( init )
								{
									switch ( init->getOp () )
									{
										...
										case astOp::assign:
											basicNode->pushNode ( new astNode ( new symbolStatic ( this, sCache.get ( qualPrefix ), init, symUnknownType, documentation ), src ) );
											break;
										default:
						...
						case statementType::stPragma:
							if ( isLS )
							{
								statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeStatement, src ) );
...
startIf:
													...
													condBlock->block = parseBlockFGL ( src, func, false, isLS );
												...
												} else if ( cmpTerminated ( src, "ELSE", 4 ) )
												{
													...
													src += 4;
													if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterCondition, src ) );
													nodeChild->ifData ().elseBlock = parseBlockFGL ( src, func, false, isLS );
													break;
												} else
												{
													break;
										...
										case statementType::stForEach:
											...
											nodeChild->forEachData ().statement = parseBlockFGL ( src, func, false, isLS );
										...
										case statementType::stFor:
											{
												...
												f->loopData ().block = parseBlockFGL ( src, func, false, isLS );
										...
										case statementType::stWhile:
											...
											if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterCondition, src ) );
											nodeChild->loopData ().block = parseBlockFGL ( src, func, false, isLS );
										...
										case statementType::stSwitch:
											...
											for ( ;; )
											{
												...
												if ( cmpTerminated ( src, "CASE", 4 ) )
												{
													...
													caseOp->block = parseBlockFGL ( src, func, true, isLS );
										...
										case statementType::stTry:
											...
											nodeChild->tryCatchData ().tryBlock = parseBlockFGL ( src, func, false, isLS );
											if ( cmpTerminated ( src, "CATCH", 5 ) )
											{
												...
												nodeChild->tryCatchData ().catchBlock = parseBlockFGL ( src, func, false, isLS );
												...
												if ( cmpTerminated ( src, "FINALLY", 7 ) )
												{
													...
													nodeChild->tryCatchData ().finallyBlock = parseBlockFGL ( src, func, false, isLS );
												}
											...
											} else if ( cmpTerminated ( src, "FINALLY", 7 ) )
											{
												...
												nodeChild->tryCatchData ().finallyBlock->pushNode ( parseBlockFGL ( src, func, false, isLS ) );
...
astNode *opFile::parseBlockSlang ( source &src, opFunction *func, bool virtualBrace, bool isLS )
{
	astNode						*nodeChild;
	int							 defaultPresent;
					...
					switch ( it->second.type )
					{
						...
						case statementType::stStatic:
							...
							while ( 1 )
							{
								...
								sprintf_s ( qualPrefix, sizeof ( qualPrefix ), "%s@%s@%u", srcFiles.getName ( src.getSourceIndex () ).c_str (), func->name.c_str (), src.getLine () );
								basicNode->pushNode ( new astNode ( new symbolStatic ( this, sCache.get ( qualPrefix ), parseExpr ( src, false, false, func, true, isLS ), symUnknownType, documentation ), src ) );
						...
						case statementType::stOpenBrace:
							...
							} else
							{
								...
								nodeChild->basicData ().blocks.push_back ( parseBlockSlang ( src, func, false, isLS ) );
								basicNode->pushNode ( nodeChild );
							}
							break;
						case statementType::stIf:
										...
										case statementType::stIf:
											...
											while ( *src )
											{
												...
												if ( cmpTerminated ( src, "else", 4 ) )
												{
													...
													{
														nodeChild->ifData ().elseBlock = parseBlockSlang ( src, func, false, isLS );
														break;
													}
...
startIf:
													...
													condBlock->block = parseBlockSlang ( src, func, false, isLS );
										...
										case statementType::stForEach:
											...
											if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterCondition, src ) );
											nodeChild->forEachData ().statement = parseBlockSlang ( src, func, false, isLS );
										...
										case statementType::stFor:
											{
												...
												f->loopData ().block = parseBlockSlang ( src, func, false, isLS );
												f->setEnd ( src );
												nodeChild->setEnd ( src );
												nodeChild->pushNode ( f.release () );
												basicNode->pushNode ( nodeChild );
										...
										case statementType::stWhile:
											...
											if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterCondition, src ) );
											nodeChild->loopData ().block = parseBlockSlang ( src, func, false, isLS );
											nodeChild->setEnd ( src );
											basicNode->pushNode ( nodeChild );
											break;
										case statementType::stRepeat:
										...
										case statementType::stSwitch:
											{
												...
												while ( *src )
												{
													...
													if ( cmpTerminated ( src, "CASE", 4 ) )
													{
														...
														if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::beforeCaseContents, src ) );
														caseOp->block = parseBlockSlang ( src, func, true, isLS );
														if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterCaseContents, src ) );
														nodeChild->switchData ().caseList.push_back ( caseOp.release () );
														if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::info, astLSInfo::semanticLineBreakType::afterCase, src ) );
													} else if ( cmpTerminated ( src, "DEFAULT", 7 ) )
										...
										case statementType::stTry:
											nodeChild = new astNode ( sCache, astOp::btTry, srcSave );
											nodeChild->tryCatchData ().tryBlock = parseBlockSlang ( src, func, false, isLS );
											if ( cmpTerminated ( src, "CATCH", 5 ) )
											{
												if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::keywordFlow, src, 5 ) );
												src += 5;
												...
												nodeChild->tryCatchData ().catchBlock = parseBlockSlang ( src, func, false, isLS );

												if ( cmpTerminated ( src, "FINALLY", 7 ) )
												{
													if ( isLS ) statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::keywordFlow, src, 7 ) );
													src += 7;
													nodeChild->tryCatchData ().finallyBlock = parseBlockSlang ( src, func, false, isLS );
												}
												basicNode->pushNode ( nodeChild );
											} else if ( cmpTerminated ( src, "FINALLY", 7 ) )
											{
												...
												nodeChild->tryCatchData ().errSymbol = (new astNode ( sCache, astOp::symbolValue, "$tmpCatchVariable" ))->setLocation ( nodeChild );
												nodeChild->tryCatchData ().finallyBlock = (new astNode ( sCache, astOp::btBasic, srcLocation ( src ) ))->setLocation ( nodeChild );
												nodeChild->tryCatchData ().finallyBlock->pushNode ( parseBlockSlang ( src, func, false, isLS ) );
												basicNode->pushNode ( nodeChild );
											} else
											{
												if ( !isLS ) throw errorNum::scMISSING_CONTROL_STRUCTURE;
...
//					errHandler.throwFatalError ( isLS, err, )
		...
		if ( !virtualBrace && multiStatement )
		{
			if ( !isLS ) throw errorNum::scMISSING_CLOSE_BRACE;
			basicNode->pushNode ( new astNode ( errorNum::scMISSING_CLOSE_BRACE, src ) );
			if ( !virtualBrace ) basicNode->pushNode ( new astNode ( errorNum::scMISSING_CLOSE_BRACE, srcSave ) );
		}
		if ( !virtualBrace )
		{
			if ( multiStatement )
...
astNode *opFile::parseBlock ( source &src, opFunction *func, bool doBrace, bool virtualBrace, bool isLS )
{
	if ( doBrace )
	{
		return parseBlockSlang ( src, func, virtualBrace, isLS);
	} else
	{
		return parseBlockFGL ( src, func, false, isLS );
	}
}
