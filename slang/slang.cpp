...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	auto handleArray = (HANDLE *)malloc ( sizeof ( HANDLE ) * argc );
	...
	for ( int loop = 1; loop < argc; loop++ )
	{
		if ( argv[loop][0] != '-' )
		{
			...
			} else 	if ( !strccmp ( p.extension ().string ().c_str (), ".ap" ) )
			{
				nFiles++;
			...
			} else 	if ( !strccmp ( p.extension ().string ().c_str (), ".aps" ) )
			{
				nFiles++;
				PostQueuedCompletionStatus ( port, 0, 0, (LPOVERLAPPED) (new compParams ( argv[loop], true, true )) );
			} else 	if ( !strccmp ( p.extension ().string ().c_str (), ".apf" ) )
			{
				nFiles++;
			...
			} else if ( !*p.extension ().string ().c_str () )
			{
				...
				if ( _isfile ( p.string().c_str() ) )
				{
					nFiles++;
					PostQueuedCompletionStatus ( port, 0, 0, (LPOVERLAPPED) (new compParams ( p.string ().c_str (), false, false )) );
					continue;
				}
				...
				{
					nFiles++;
					PostQueuedCompletionStatus ( port, 0, 0, (LPOVERLAPPED) (new compParams ( p.string ().c_str (), true, true )) );
					continue;
				}
				...
				{
					nFiles++;
					PostQueuedCompletionStatus ( port, 0, 0, (LPOVERLAPPED) (new compParams ( p.string ().c_str (), true, false )) );
					continue;
				}
				...
				{
					nFiles++;
					PostQueuedCompletionStatus ( port, 0, 0, (LPOVERLAPPED) (new compParams ( p.string ().c_str (), false, true )) );
					continue;
				}
			}
		}
	...
	{
		handleArray[nHandles++] = (HANDLE)_beginthread ( compFile, 0, (void *)port );
	}

	WaitForMultipleObjects ( (DWORD)nHandles, handleArray, true, INFINITE );
