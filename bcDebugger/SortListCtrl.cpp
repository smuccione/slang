...
BEGIN_MESSAGE_MAP(CSortListCtrl, CListCtrl)
	...
	ON_NOTIFY_REFLECT(LVN_COLUMNCLICK, &ThisClass::OnColumnClick)
	ON_WM_DESTROY()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
	ItemData* pid = reinterpret_cast<ItemData*>( CListCtrl::GetItemData( iItem ) );		// NOLINT(performance-no-int-to-ptr)
...
int NumberCompare( LPCTSTR pszNumber1, LPCTSTR pszNumber2 )
{
	...
	auto iNumber1 = _atoi64( pszNumber1 );
	auto iNumber2 = _atoi64( pszNumber2 );
...
{
	CSortListCtrl* pListCtrl = reinterpret_cast<CSortListCtrl*>( lParamData );		// NOLINT(performance-no-int-to-ptr)
	ASSERT( pListCtrl->IsKindOf( RUNTIME_CLASS( CListCtrl ) ) );

	ItemData* pid1 = reinterpret_cast<ItemData*>( lParam1 );		// NOLINT(performance-no-int-to-ptr)
	ItemData* pid2 = reinterpret_cast<ItemData*>( lParam2 );		// NOLINT(performance-no-int-to-ptr)
...
BOOL CSortListCtrl::SetItemData( int nItem, DWORD dwData )
{
	...
	ItemData* pid = reinterpret_cast<ItemData*>( CListCtrl::GetItemData( nItem ) );		// NOLINT(performance-no-int-to-ptr)
	ASSERT( pid );
	pid->dwData = dwData;
