	...
	for (int i = 0; i < nLength; i++)
	{
		if (bCmd)
		{
			...
			case (TCHAR)'R':
				...
				break;
			case (TCHAR)'H':
				str += FormatingString(PPDUMP_BAR_AREA_HEX, GetDataFromCurrentAddress(nAddress, nDir, nNum), nNum);
				bCmd = FALSE;
				break;
			case (TCHAR)'D':
				str += FormatingString(PPDUMP_BAR_AREA_DEC, GetDataFromCurrentAddress(nAddress, nDir, nNum), nNum);
				bCmd = FALSE;
				break;
			case (TCHAR)'B':
				str += FormatingString(PPDUMP_BAR_AREA_BIN, GetDataFromCurrentAddress(nAddress, nDir, nNum), nNum);
				bCmd = FALSE;
				break;
			case (TCHAR)'O':
				str += FormatingString(PPDUMP_BAR_AREA_OCT, GetDataFromCurrentAddress(nAddress, nDir, nNum), nNum);
				bCmd = FALSE;
				break;
			case (TCHAR)'A':
				str += FormatingString(PPDUMP_BAR_AREA_ASCII, GetDataFromCurrentAddress(nAddress, nDir, nNum), nNum);
				bCmd = FALSE;
				break;
			case (TCHAR)'n':
				str += _T("\r\n");
...
void CPPDumpCtrl::SetPointerData(DWORD nLength, LPBYTE pNewData, BOOL bRedraw /* = TRUE */)
{
	...
	m_pNewData = (unsigned char *)malloc ( sizeof ( char ) * nLength );
