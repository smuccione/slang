...
BEGIN_MESSAGE_MAP ( CPPDumpCtrl, CWnd )
	...
	ON_NOTIFY ( UDM_PPNUMEDIT_ENTER, nIdEdit, &ThisClass::NotifyEditEnter )
	ON_NOTIFY ( UDM_PPNUMEDIT_CANCEL, nIdEdit, &ThisClass::NotifyEditCancel )
	ON_NOTIFY ( UDM_PPNUMEDIT_MOVE, nIdEdit, &ThisClass::NotifyEditMove )
	ON_NOTIFY ( UDM_PPNUMEDIT_HOTKEY, nIdEdit, &ThisClass::NotifyEditHotKeys )
END_MESSAGE_MAP ()
...
BOOL CPPDumpCtrl::RegisterWindowClass ()
{
	...
	if ( !(::GetClassInfo ( hInst, PPDUMPCTRL_CLASSNAME, &wndcls )) )
	{
		...
		wndcls.hIcon = NULL;
		wndcls.hCursor = AfxGetApp ()->LoadStandardCursor ( IDC_ARROW );		// NOLINT(performance-no-int-to-ptr)
		wndcls.hbrBackground = (HBRUSH)(COLOR_3DFACE + 1);							// NOLINT(performance-no-int-to-ptr)
...
BOOL CPPDumpCtrl::myCreate ( const RECT &rect, CWnd *pParentWnd, UINT nID, DWORD dwStyle /* = WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE */ )
{
...
void CPPDumpCtrl::OnMouseMove ( UINT nFlags, CPoint point )
{
	...
	auto wndActive = GetActiveWindow ();
...
void CPPDumpCtrl::OnPaint ()
{
	...
	BOOL bDisable = (BOOL)(GetStyle () & WS_DISABLED);
	if ( bDisable )
	{
	...
	m_pVScroll.EnableWindow ( (BOOL)!(bool)bDisable );
		...
		if ( bCmd )
		{
			switch ( m_sFormatToolTip.GetAt ( i ) )
			{
				...
				case (TCHAR)'R':
					...
					break;
				case (TCHAR)'H':
					str += FormatingString ( PPDUMP_BAR_AREA_HEX, static_cast<int>(GetDataFromCurrentAddress ( nAddress, nDir, nNum)), nNum );
					bCmd = FALSE;
					break;
				case (TCHAR)'D':
					str += FormatingString ( PPDUMP_BAR_AREA_DEC, static_cast<int>(GetDataFromCurrentAddress ( nAddress, nDir, nNum)), nNum );
					bCmd = FALSE;
					break;
				case (TCHAR)'B':
					str += FormatingString ( PPDUMP_BAR_AREA_BIN, static_cast<int>(GetDataFromCurrentAddress ( nAddress, nDir, nNum)), nNum );
					bCmd = FALSE;
					break;
				case (TCHAR)'O':
					str += FormatingString ( PPDUMP_BAR_AREA_OCT, static_cast<int>(GetDataFromCurrentAddress ( nAddress, nDir, nNum)), nNum );
					bCmd = FALSE;
					break;
				case (TCHAR)'A':
					str += FormatingString ( PPDUMP_BAR_AREA_ASCII, static_cast<int>(GetDataFromCurrentAddress ( nAddress, nDir, nNum)), nNum );
					bCmd = FALSE;
					break;
				case (TCHAR)'n':
					str += _T ( "\r\n" );
...
LRESULT CPPDumpCtrl::SendNotify ( UINT uNotifyCode, UINT nAddress, UINT nValue )
{
	...
	lpnm.iAddress = static_cast<int>(nAddress);
	lpnm.iValue = nValue;
	lpnm.hdr.hwndFrom = m_hWnd;
	lpnm.hdr.idFrom = GetDlgCtrlID ();
...
void CPPDumpCtrl::NotifyEditMove ( NMHDR *pNMHDR, LRESULT *result )
{
	...
	*result = 0;
	...
	NM_PPNUM_EDIT *pNotify = (NM_PPNUM_EDIT *)pNMHDR;
	MoveCaretAddress ( static_cast<int>(pNotify->iEvent), TRUE, pNotify->iValue );
}
...
BOOL CPPDumpCtrl::IsExistField ( int nArea )
{
	...
	{
		case 0x01:
			return static_cast<BOOL>(m_nStyle & PPDUMP_BAR_ADDRESS);
		case 0x02:
			return static_cast<BOOL>(m_nStyle & PPDUMP_BAR_HEX);
		case 0x03:
			return static_cast<BOOL>(m_nStyle & PPDUMP_BAR_DEC);
		case 0x04:
			return static_cast<BOOL>(m_nStyle & PPDUMP_BAR_BIN);
		case 0x05:
			return static_cast<BOOL>(m_nStyle & PPDUMP_BAR_OCT);
		case 0x06:
			return static_cast<BOOL>(m_nStyle & PPDUMP_BAR_ASCII);
		case 0x11:
			return static_cast<BOOL>(m_nStyle & PPDUMP_FIELD_ADDRESS);
		case 0x12:
			return static_cast<BOOL>(m_nStyle & PPDUMP_FIELD_HEX);
		case 0x13:
			return static_cast<BOOL>(m_nStyle & PPDUMP_FIELD_DEC);
		case 0x14:
			return static_cast<BOOL>(m_nStyle & PPDUMP_FIELD_BIN);
		case 0x15:
			return static_cast<BOOL>(m_nStyle & PPDUMP_FIELD_OCT);
		case 0x16:
			return static_cast<BOOL>(m_nStyle & PPDUMP_FIELD_ASCII);
	}
	return FALSE;
}
...
int CPPDumpCtrl::GetAddressForControlBar ()
{
	...
	if ( m_pEdit == NULL )
	{
		...
		if ( m_rLastTrackRect.IsRectEmpty () || (m_nCurArea < 0x10) )
			nAddr = m_nCaretAddrBegin;
		else
			nAddr = m_nCurrentAddr;
	} else
	{
		//The edit is exist
		if ( m_nEditedArea < 0x11 || m_rLastTrackRect.IsRectEmpty () || m_nCurArea < 0x10 )
...
BOOL CPPDumpCtrl::CompleteEditValue ( UINT nValue, BOOL bDelete /* = TRUE */ )
{
	...
	if ( m_pEdit->IsChanged () )
	{
		if ( (m_nEditedArea & 0xF) == 1 )
		{
			...
			SetSelectRange ( static_cast<int>(nValue) - m_nOffsetAddress, -1 );
		} else
...
int CPPDumpCtrl::DrawAddressValue ( CDC *pDC, CRect rect, UINT nAddress, BOOL bBkgnd /* = FALSE */, BOOL bDisable /* = FALSE */ )
{
	...
	CString str = FormatingString ( PPDUMP_BAR_AREA_ADDRESS, static_cast<int>(nAddress), nLength );
...
{
	CString str = FormatingString ( PPDUMP_BAR_AREA_HEX, static_cast<int>(nValue) );
	str = (TCHAR)' ' + str + (TCHAR)' ';
...
{
	CString str = FormatingString ( PPDUMP_BAR_AREA_DEC, static_cast<int>(nValue) );
...
{
	CString str = FormatingString ( PPDUMP_BAR_AREA_OCT, static_cast<int>(nValue) );
...
{
	CString str = FormatingString ( PPDUMP_BAR_AREA_BIN, static_cast<int>(nValue) );
	str = (TCHAR)' ' + str + (TCHAR)' ';
...
{
	CString str = FormatingString ( PPDUMP_BAR_AREA_ASCII, static_cast<int>(nValue) );
	str = (TCHAR)' ' + str + (TCHAR)' ';
...
//
/////////////////////////////////////////////////////////////////////////////
DWORD CPPDumpCtrl::GetDataFromCurrentAddress ( DWORD nAddress, int nDir /* = 0 */, int nByte /* = 0 */ )
{
	BYTE *pArray = m_pNewData;
							...
							BOOL bRedraw /* = TRUE */ )
{
	...
	strcpy_s ( lf.lfFaceName, sizeof ( lf.lfFaceName ), lpszFaceName );
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPDumpCtrl::SetPointerData ( DWORD nLength, LPBYTE pNewData, BOOL bRedraw /* = TRUE */ )
{
	KillEdit ();
	...
	m_pNewData = (LPBYTE)malloc ( sizeof ( BYTE ) * nLength );
	memcpy ( m_pNewData, pNewData, nLength );

	m_nRealLengthData = static_cast<int>(nLength); //remember the real length of array
	m_nBeginAddress = 0;
	m_nLengthData = (m_nStyle & PPDUMP_WORD_DATA) ? static_cast<int>(nLength) / 2 : static_cast<int>(nLength);
...
//
///////////////////////////////////////////////////////////////////////
BOOL CPPDumpCtrl::mySetMenu ( UINT nMenu, BOOL bRedraw /* = TRUE */ )
{
	...
	if ( nMenu )
	{
		...
		// Load menu resource
		m_hMenu = ::LoadMenu ( hInstResource, MAKEINTRESOURCE ( nMenu ) );			// NOLINT(performance-no-int-to-ptr)
		// If something wrong
		if ( m_hMenu == NULL )
			return FALSE;
	} // if
