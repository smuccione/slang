...
BOOL CColumnTreeCtrl::myCreate(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CColumnTreeCtrl::OnTreeCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case CDDS_ITEMPOSTPAINT:
		{
			HTREEITEM hItem = (HTREEITEM)pNMCustomDraw->dwItemSpec;		// NOLINT (performance-no-int-to-ptr)
			...
			BOOL bFullRowSelect = (BOOL)(m_Tree.GetStyle () & TVS_FULLROWSELECT);
...
int CColumnTreeCtrl::InsertColumn(int nCol, LPCTSTR lpszColumnHeading, int nFormat, int nWidth, int nSubItem)
{
	...
	if(m_Header.GetItemCount()>0) 
	{
		...
		hditem.pszText = const_cast<char *>(_T ( "" ));
...
void CColumnTreeCtrl::SetFirstColumnMinWidth(UINT uMinWidth)
{
	...
	m_uMinFirstColWidth = (int)uMinWidth;
}
...
BOOL CColumnTreeCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult)
{
	...
	LPNMHDR pHdr = (LPNMHDR)lParam;			// NOLINT(performance-no-int-to-ptr)
