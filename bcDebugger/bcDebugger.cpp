...
static int fastStat ( char const *fName, struct _stat *buf )
{
	...
	buf->st_size	= attrData.nFileSizeLow;
	buf->st_mode	= 0;
...

{
	...
	coord.X = 0;
	coord.Y = (SHORT)(*currentLine - topLine) - 1;
...
//									doWatchWindow();
								...
								case VK_DOWN:
									...
									break;
								case VK_NEXT:
									currentLine += (consoleInfo.srWindow.Bottom - consoleInfo.srWindow.Top) - 1;
									break;
								case VK_PRIOR:
									if ( (int)currentLine > (consoleInfo.srWindow.Bottom - consoleInfo.srWindow.Top) - 1 )
									{
										currentLine -= (consoleInfo.srWindow.Bottom - consoleInfo.srWindow.Top) - 1;
									} else
									{
				...
				case MOUSE_EVENT:
					...
					{
						currentLine = inputBuffer[loop].Event.MouseEvent.dwMousePosition.Y + 1;
						debuggerShowFile ( screenBuffer, instance, *currentFile, debugErrorLine, (uint32_t)dbgMsg->location.line, &currentLine, msg );
					}
					if ( inputBuffer[loop].Event.MouseEvent.dwEventFlags & MOUSE_WHEELED )
					{
						...
						if ( scroll < 0 )
						{
							currentLine -= scroll - ((scroll + 1) * (scroll + 1));
						} else
						{
							currentLine -= scroll + ((scroll - 1) * (scroll - 1));
						}
			...
			if ( !lastInstance || (msg->instance == lastInstance) )
			{
				...
				switch ( msg->state )
				{
					case vmStateType::vmTrap:
						_snprintf_s ( msgBuff, sizeof ( msgBuff ), _TRUNCATE, "%s    %s:%s(%I64u): error E%x: %s", msg->instance->getName ( ), msg->location.fName, msg->location.functionName, msg->location.line, msg->location.err, scCompErrorAsText ( size_t(msg->location.err) ).c_str ( ) );
