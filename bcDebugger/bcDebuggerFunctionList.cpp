	    ...
	    case WM_INITDIALOG:
			...
			for ( loop = 0; loop < funcList->entry.size(); loop++ )
			{
				if ( funcList->entry[loop].isUsercode )
				{
					...
					{
						/* new class */
						htreeClass = AddOneTreeItem ( (HTREEITEM)NULL, funcList->entry[loop].className, (HTREEITEM)TVI_ROOT, hwndTree, 0, 1 );							// NOLINT (performance-no-int-to-ptr)
						AddOneTreeItem ( (HTREEITEM)htreeClass, funcList->entry[loop].funcName, (HTREEITEM)TVI_ROOT, hwndTree, (void *)(ptrdiff_t)(loop+1), 0 );		// NOLINT (performance-no-int-to-ptr)
						strcpy_s ( lopClass, sizeof ( lopClass ), funcList->entry[loop].className );
					} else
					{
						...
						{
							/* not a class method */
							AddOneTreeItem ( (HTREEITEM)NULL, funcList->entry[loop].funcName, (HTREEITEM)TVI_ROOT, hwndTree, (void *)(ptrdiff_t)(loop+1), 0 );			// NOLINT (performance-no-int-to-ptr)
						}
					}
				}
			}
