...
INT_PTR CALLBACK debuggerCallstackDlgProc ( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	switch ( uMsg )
	{
	    case WM_INITDIALOG:
			...
			lineBuff	= (char *)malloc ( sizeof ( char ) * (width + 1) );
			...
			{
				len = _snprintf_s ( lineBuff, width - 1, _TRUNCATE, "%s ( ", callStack->entry[loop].funcName );
				
				for ( loop2 = 0; loop2 < (int)callStack->entry[loop].params.size(); loop2++ )
				{
					len += _snprintf_s ( lineBuff + len, width - len, _TRUNCATE, "%.32s", callStack->entry[loop].params[loop2]->getValue() );
					if ( loop2 != callStack->entry[loop].params.size() - 1 )
					{
						if ( len < width )
						{
							len += _snprintf_s ( lineBuff + len, width - len, _TRUNCATE, ", " );
						}
					}
				}
				...
				if ( len < width )
				{
					strncat_s ( lineBuff, width - len, " )", width - len);
				}

				AddOneListItem ( hwndList, lineBuff, (void *)(ptrdiff_t)loop );
			}
		...
		case WM_NOTIFY:
			...
			switch (((LPNMHDR) lParam)->code) 
			{
				case LVN_ITEMACTIVATE:
					EndDialog ( hDlg, ((LPNMITEMACTIVATE) lParam)->iItem );
					break;
				case NM_RETURN:
...
void debuggerCallstackDlg (	class vmInstance *instance, char const ** fileName, size_t *currentLine, size_t *stackFrame, HWND parentWindow )
{
    ...
    if ( (entry = DialogBox ( (HINSTANCE) GetWindowLongPtr ( GetConsoleWindow(), GWLP_HINSTANCE ), MAKEINTRESOURCE ( IDD_CALLSTACK ), parentWindow, debuggerCallstackDlgProc )) >= 0 )
	{
		callStack = debugGetCallstack ( instance );
