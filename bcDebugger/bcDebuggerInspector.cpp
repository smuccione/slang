...
HTREEITEM AddOneTreeItem ( HTREEITEM hParent, LPSTR szText, HTREEITEM hInsAfter, HWND hWnd, void *param, int expandable )
{
	...
	hItem = (HTREEITEM) ::SendMessage ( hWnd, TVM_INSERTITEM, 0, (LPARAM)(LPTV_INSERTSTRUCT)&tvIns);
		...
		switch ( code )
		{
			case TVN_SELCHANGED:
				...
				for ( loop = 0; loop < m_inspectorTrees.size(); loop++ )
				{
					...
					{
						SetData ( (vmDebugVar *)tv->itemNew.lParam );
						break;
					}
				}
				break;
			case TVN_ITEMEXPANDINGA:
				if ( tv->itemNew.lParam )
				{
					if ( !(tv->itemNew.state & TVIS_EXPANDEDONCE) )
					{
						...
						var = (vmDebugVar *)tv->itemNew.lParam;
...
BOOL CInspector::OnInitDialog()
{
	...
	for ( loop = 0; loop < m_inspectors->entry.size (); loop++ )
	{
		...
		m_topTab.InsertItem ( loop, m_inspectors->entry[loop] );
		...
		for ( loop2 = 0; loop2 < (iList->entry.size () > 100 ? 100 : iList->entry.size ()); loop2++ )
		{
			...
			AddOneTreeItem ( (HTREEITEM)TVI_ROOT, tmpBuff, (HTREEITEM)TVI_SORT, m_inspectorTrees[loop]->GetSafeHwnd (), (void *)iList->entry[loop2], iList->entry[loop2]->hasChildren () );
		}
	}
