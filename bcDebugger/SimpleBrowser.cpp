...
BOOL SimpleBrowser::myCreate(DWORD dwStyle, 
                           const RECT& rect, 
                           CWnd* pParentWnd, 
						   UINT nID)
{
...
BOOL SimpleBrowser::CreateFromControl(CWnd *pParentWnd,UINT nID)
{
	...
	CWnd *control = pParentWnd->GetDlgItem( static_cast<int>(nID));
	...
	if (control != NULL) {
		...
		result = myCreate((WS_CHILD | WS_VISIBLE),
		                rect,
						pParentWnd,
						nID);
...
void SimpleBrowser::NavigateResource(int resource_ID)
{
	if (_Browser != NULL) {
		...
		HRSRC resource_handle = FindResource(AfxGetResourceHandle(),
											 MAKEINTRESOURCE(resource_ID),				// NOLINT(performance-no-int-to-ptr)
											 RT_HTML);									// NOLINT(performance-no-int-to-ptr)
		...
		if (resource_handle != NULL) {
			...
			if (resource != NULL) {
				...
				if (resource_memory != NULL) {
					...
					int      UNICODE_size     = static_cast<int>(resource_size / sizeof(wchar_t));
					...
					if (UNICODE_document) {
						...
						char *MBCS_buffer = resource_string.GetBufferSetLength( static_cast<int>(resource_size) + 1);
						...
						int MBCS_length = ::WideCharToMultiByte(CP_ACP,
											                    ...
											                    UNICODE_size,
											                    MBCS_buffer,
																static_cast<int>(resource_size) + 1,
											                    NULL,
											                    NULL);
						...
						resource_string = CString((char *)resource_memory, static_cast<int>(resource_size));
		...
		Clear();
		Write(resource_string, strlen ( resource_string ));
...
void SimpleBrowser::Write(LPCTSTR string, size_t len )
{
	if (_Browser != NULL) {
		...
		if (document != NULL) {
			...
			variant->vt      = VT_BSTR;
			variant->bstrVal = CString(string, (int)len).AllocSysString();
...
void SimpleBrowser::Print(LPCTSTR header,LPCTSTR footer)
{
	if (_Browser != NULL) {
		...
		index = 0;
		SafeArrayPutElement(parameter_array,&index,&header_variant);

		index = 1;
		SafeArrayPutElement(parameter_array,&index,&footer_variant);
...
BEGIN_EVENTSINK_MAP(SimpleBrowser,CWnd)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
			 ...
			 SimpleBrowser::_OnBeforeNavigate2,
			 VTS_DISPATCH VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT VTS_PBOOL)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_DOCUMENTCOMPLETE,
			  SimpleBrowser::_OnDocumentComplete,
			 VTS_DISPATCH VTS_PVARIANT)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_DOWNLOADBEGIN,
			  SimpleBrowser::_OnDownloadBegin,
			 VTS_NONE)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_PROGRESSCHANGE,
			 SimpleBrowser::_OnProgressChange,
			 VTS_I4 VTS_I4)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_DOWNLOADCOMPLETE,
			 SimpleBrowser::_OnDownloadComplete,
			 VTS_NONE)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_NAVIGATECOMPLETE2,
			 SimpleBrowser::_OnNavigateComplete2,
			 VTS_DISPATCH VTS_PVARIANT)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_STATUSTEXTCHANGE,
			 SimpleBrowser::_OnStatusTextChange,
			 VTS_BSTR)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
	         DISPID_TITLECHANGE,
			 SimpleBrowser::_OnTitleChange,
			 VTS_BSTR)
END_EVENTSINK_MAP()
