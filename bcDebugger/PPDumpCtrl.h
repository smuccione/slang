...
protected:
	...
	DWORD GetDataFromCurrentAddress(DWORD nAddress, int nDir = 0, int nByte = 0); //Gets the data from the current address
...
public:
	//Create
	BOOL myCreate(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle = WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE);
	...
	void  SetPointerData(DWORD nLength, LPBYTE pNewData, BOOL bRedraw = TRUE); 
	...
	BOOL  mySetMenu(UINT nMenu, BOOL bRedraw = TRUE);
