...
#define MAKEuint64_t64(a, b) ((ZPOS64_T)(((uint64_t)(a)) | ((ZPOS64_T)((uint64_t)(b))) << 32))
...
#ifndef NOCRYPT
    uint64_t keys[3];     /* keys defining the pseudo-random sequence */
    const uint64_t* pcrc_32_tab;
...
local int add_data_in_datablock(linkedlist_data* ll, const void* buf, uint64_t len)
{
    linkedlist_datablock_internal* ldi;
...
local int zip64local_putValue (const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, ZPOS64_T x, int nbByte)
{
    ...
    if (ZWRITE64(*pzlib_filefunc_def,filestream,buf,nbByte)!=(uint64_t)nbByte)
        return ZIP_ERRNO;
    else
        return ZIP_OK;
}
...
local uint64_t zip64local_TmzDateToDosDate(const tm_zip* ptm)
{
    ...
    if (year>=1980)
        ...
        year-=80;
    return
      (uint64_t) (((ptm->tm_mday) + (32 * (ptm->tm_mon+1)) + (512 * year)) << 16) |
        ((ptm->tm_sec/2) + (32* ptm->tm_min) + (2048 * (uint64_t)ptm->tm_hour));
}
...
*/
local int zip64local_getShort OF((const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, uint64_t *pX));

local int zip64local_getShort (const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, uint64_t* pX)
{
    uint64_t x;
    int i = 0;
    int err;
...
local int zip64local_getLong OF((const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, uint64_t *pX));

local int zip64local_getLong (const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, uint64_t* pX)
{
    uint64_t x;
    int i = 0;
    int err;
...
int LoadCentralDirectoryRecord(zip64_internal* pziinit)
{
  ...
  uint64_t number_disk;          /* number of the current dist, used for
                              spaning ZIP, unsupported, always 0*/
  uint64_t number_disk_with_CD;  /* number the the disk with central dir, used
                              for spaning ZIP, unsupported, always 0*/
  ZPOS64_T number_entry;
  ZPOS64_T number_entry_CD;      /* total number of entries in
                                the central dir
                                (same than number_entry on nospan) */
  uint64_t VersionMadeBy;
  uint64_t VersionNeeded;
...
extern int ZEXPORT zipWriteInFileInZip (zipFile file,const void* buf,unsigned int len)
{
    ...
    if(zi->ci.method == Z_BZIP2ED && (!zi->ci.raw))
    {
      ...
      while ((err==BZ_RUN_OK) && (zi->ci.bstream.avail_in>0))
      {
        ...
        if ((zi->ci.method == Z_BZIP2ED) && (!zi->ci.raw))
        {
          uint64_t uTotalOutBefore_lo = zi->ci.bstream.total_out_lo32;
//          uint64_t uTotalOutBefore_hi = zi->ci.bstream.total_out_hi32;
...
extern int ZEXPORT zipCloseFileInZipRaw (zipFile file, uint64_t uncompressed_size, uint64_t crc32)
{
    return zipCloseFileInZipRaw64 (file, uncompressed_size, crc32);
}

extern int ZEXPORT zipCloseFileInZipRaw64 (zipFile file, ZPOS64_T uncompressed_size, uint64_t crc32)
{
    zip64_internal* zi;
    ZPOS64_T compressed_size;
    uint64_t invalidValue = 0xffffffff;
    short datasize = 0;
    int err=ZIP_OK;
...
int Write_Zip64EndOfCentralDirectoryRecord(zip64_internal* zi, uint64_t size_centraldir, ZPOS64_T centraldir_pos_inzip)
{
  int err = ZIP_OK;
...
int Write_EndOfCentralDirectoryRecord(zip64_internal* zi, uint64_t size_centraldir, ZPOS64_T centraldir_pos_inzip)
{
  int err = ZIP_OK;
