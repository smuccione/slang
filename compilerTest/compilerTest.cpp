...
UINT __cdecl testCompile( std::vector<char const *> const &fList )
{
	...
	if ( !doDebug )
	{
		debugRegister ( &instance, static_cast<vmDebug *>(&nDbg) );
	}
...
//	SetThreadPriority ( GetCurrentThread(), THREAD_PRIORITY_HIGHEST );
	...
	for ( loop = 0; loop < fList.size(); loop++ )
	{
		if ( fList[loop][0] != '-' )
		{
			...
			if ( !doTest ) printf ( "front end time = %I64u ms\r\n", (end.QuadPart - start.QuadPart) / frequency.QuadPart );

			objBuff.reset();
			oFile.serialize ( &objBuff, false );
			oFile.addBuildSource ( file );

			if ( !doTest ) printf ( "object file size = %zu\r\n", bufferSize ( &objBuff ) );
	...
	if ( !doTest ) printf ( "compiled file size = %zu\r\n", bufferSize ( &buff ) );
	if ( !doTest ) printf ( "optimization time = %I64u ms\r\n", comp.tOptimizer);
	if ( !doTest ) printf ( "back end time     = %I64u ms\r\n", comp.tBackEnd );
	...
	if ( !doTest ) printf ( "load time = %I64u ms\r\n\r\n", (end.QuadPart - start.QuadPart) / frequency.QuadPart );
	...
	if ( !doTest ) printf ( "run time = %I64u ms\r\n", (end.QuadPart - start.QuadPart) / frequency.QuadPart );
