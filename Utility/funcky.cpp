...
char *_fmerge ( char const *temp, char const *inName, char *dest, size_t destSize )
{
	...
	if ( !pat.root_path().string ().c_str ()[0] || pat.root_path().string ().c_str ()[0] == '*' )
	{
		d = in.root_path ();
	} else
	{
	...
	if ( !std::filesystem::path( pat ).remove_filename().string().c_str()[0] || std::filesystem::path( pat ).remove_filename().string().c_str()[0] == '*' )
	{
	...
	if ( !pat.stem().string ().c_str ()[0] || pat.stem().string ().c_str ()[0] == '*' )
	{
		d /= in.stem ();
	} else
	{
	...
	if ( !pat.extension ().string ().c_str ()[0] || pat.extension ().string ().c_str ()[0] == '*' )
	{
		d += in.extension();
	} else
	{
	...
	strcpy_s ( dest, destSize, d.string().c_str () );
...
char *_fmake ( char const *drive, char const *path, char const *name, char const *ext, char *dest, size_t destSize )
{
	...
	auto ret = std::accumulate ( std::next ( components.begin () ), components.end (), std::filesystem::path {}, std::divides {} ).string ();
...
unsigned int _crc32 ( unsigned char *data, unsigned int result, int len )
{
	...
	unsigned int	*e = (unsigned int *) ((ptrdiff_t) (data + len) & ~0x03); //  0xFFFFFFFC);
	unsigned int	 tmp = 0;
