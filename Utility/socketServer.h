...
template <typename ...T >
class socketServer : public jsonRpcServer<socketServer, T...>
{
...
public:
	socketServer ( uint16_t port, T &&...values ) : jsonRpcServer<socketServer<T...>, T...>  ( std::forward<T...> ( values... ) )
	{
	...
	void mainLoop ( SOCKET listenSocket )
	{
		...
		while ( listenSocket )
		{
			...
			jsonRpcServer<socketServer<T...>, T...>::template startWorker<decltype (termLambda), T...> ( termLambda );
						...
						while ( state == serverState::wait )
						{
							...
							if ( select ( 0, &rxSet, nullptr, &errSet, &timeout ) )
							{
								if ( FD_ISSET ( listenSocket, &errSet ) )
								{
									...
									jsonRpcServer<socketServer<T...>, T...>::template terminate ();
									connSocket = 0;
									listenSocket = 0;
								}
					...
					case serverState::processBody:
						{
							...
							jsonRpcServer<socketServer, T...>::call ( req );
...
template <typename ...T >
void socketServer< T... >::startServer ( socketServerTaskControl<T...> *tc )
{
