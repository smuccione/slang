...
jsonElement::operator stringi () const
{
	if ( std::holds_alternative<stringi> ( value ) )
	{
		return stringi(std::get<stringi> ( value ));
...
// ------------------------------- serialization
void jsonElement::serialize ( BUFFER &buff, bool quoteNames )
{
	if ( std::holds_alternative<objectType> ( value ) )
	{
		auto &obj = std::get<objectType> ( value );
