...
public:
	...
	auto begin ( ) noexcept ->decltype(str.begin ( ))
	{
		return str.begin ( );
	}
	auto end ( ) noexcept ->decltype(str.end ( ))
	{
		return str.end ( );
	}
	auto rbegin ( ) noexcept ->decltype(str.rbegin ( ))
	{
		return str.rbegin ( );
	}
	auto rend ( ) noexcept ->decltype(str.rend ( ))
	{
		return str.rend ( );
	}
	...
	stringi findAndReplace ( stringi const &find, stringi const &replace ) const
	{
		...
		assert ( find.size () );
		while ( (pos = result.str.find ( (std::string)find, pos )) != std::string::npos )
		{
...
//		return !strccmp ( str.c_str (), obj );
	...
	{
		return ((str.c_str ( )[0] | 32) == (obj.c_str ( )[0] | 32)) && !strccmp ( str.c_str ( ), str.c_str ( ) );
	}
