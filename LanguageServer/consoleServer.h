...
template <typename ...T >
class lspConsoleServer : public jsonRpcServer<lspConsoleServer, T...>
{
	std::mutex					sendMutex;
	...
	public:
	lspConsoleServer ( T &&...values ) : jsonRpcServer<lspConsoleServer, T...> ( std::forward<T...> ( values... ) )
	{
	}
	...
	void mainLoop ()
	{
		...
		jsonRpcServer<lspConsoleServer<T...>, T...>::template startWorker<decltype (termLambda), T...> ( termLambda );
				...
				case serverState::processBody:
					{
						...
						jsonRpcServer<lspConsoleServer, T...>::call ( req );
...
template <typename ...T >
void lspConsoleServer <T...>::startServer ( lspConsoleServerTaskControl<T...> *tc )
{
