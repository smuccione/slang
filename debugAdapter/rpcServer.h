...
class jsonRpcServer : public jsonRPCServerBase
{
	// public types
public:
	using funcType = jsonElement ( * )(jsonElement const &req, jsonRPCServerBase &server, T...);
...
public:
	...
	void workerThread ( TERMINATE terminate )
	{
		while ( 1 )
		{
			...
			{
				auto const req = std::move ( requests.front () );
				requests.pop ();
				...
				if ( type == "request" )
				{
...

					{
						...
						try
						{
							rsp["body"] = dispatch ( command, *req["arguments"] );
							rsp["success"] = true;
	...
	void call ( jsonElement const &req )
	{
		...
		if ( method == "cancel" && req["arguments"] && (*req["arguments"])["requestId"] )
		{
