...
class debugSocketServerWriterData
{
	public:
	SOCKET		 socket;
	std::mutex &sendMutex;

	debugSocketServerWriterData ( SOCKET socket, std::mutex &sendMutex ) : socket ( socket ), sendMutex ( sendMutex )
	{}


	void send ( BUFFER &buff, debugSocketServerWriterData const &data )
	{
		BUFFER header;
...
class debugSocketServerTaskControl;

template <typename ...T >
class debugSocketServer : public jsonRpcServer<T...>
{
	...
	public:
	debugSocketServer ( uint16_t port, T &&...values ) : jsonRpcServer<T...> ( std::forward<T...> ( values... ) )
	{
	...
	void startServer ( debugSocketServerTaskControl *tc );
	...
	void mainLoop ( SOCKET connSocket )
	{
		...
		debugSocketServerWriterData w ( connSocket, sendMutex );
		auto termLambda = [=]() {closesocket ( connSocket ); };

		jsonRpcServer<T...>::template startWorker<debugSocketServerWriterData, decltype (termLambda), T...> ( w, termLambda );
		...
		exit:
		jsonRpcServer<T...>::doTerminate = false;
	}
};

class debugSocketServerTaskControl : public taskControl
{
	public:
...
template <typename ...T >
void debugSocketServer<T...>::startServer ( debugSocketServerTaskControl *tc )
{
