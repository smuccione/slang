...
void dbServerInit ( taskControl *ctrl, char const *configFile, vmInstance *instance )
{
	...
	/* bind it to our port */
	if ( bind ( listenSocket, (SOCKADDR *) &addr, sizeof ( addr ) ) == INVALID_SOCKET )
	{
	...
	CreateIoCompletionPort ( (HANDLE) listenSocket, dbIocpCompletionPort, 0, 0 );

	for ( size_t loop = 0; loop < vmConf.dbServerThreads * 4; loop++ )
	{
...
taskControl	*dbServerStart ( size_t numWorkerThreads, vmInstance *instance )
{
	...
	{
		*ctrl += (HANDLE) _beginthread ( dbServerWorkerThread, CPU_STACK_SIZE, dynamic_cast<vmTaskInstance *>(instance) );
	}
