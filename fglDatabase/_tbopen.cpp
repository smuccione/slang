...
TABLE *_tbOpen ( char const *filename, int openmode )
{
	...
	*/
	...
	if ( (dbf->fileHandle = CreateFile ( p.string().c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL )) == INVALID_HANDLE_VALUE )	// NOLINT (performance-no-int-to-ptr)
	{	
		delete dbf;
		return nullptr;
	}
	...
	*/
	...
	if ( dbf->getHeader ( )->capabilities & DBF_CAPABILITY_BLOB )
	{
		...
		if ( !(dbf->blob = blobInit ( p2.string().c_str(), dbf->updateCount )) )
		{
			delete dbf;
			return 0;
		}
...
TABLE *_tbOpenRepair ( char const *filename, int openmode )
{
	...
	*/
	...
	if ( dbf->getHeader ( )->capabilities & DBF_CAPABILITY_BLOB )
	{
		...
		if ( !(dbf->blob = blobInitRead ( p2.string().c_str(), dbf->updateCount )) )
		{
			delete dbf;
