...
CProgressWnd::CProgressWnd(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth /* = FALSE */)
{
    ...
    myCreate(pParent, pszTitle, bSmooth);
}

void CProgressWnd::SetTitle ( LPCTSTR pszTitle )
...
CProgressWnd::~CProgressWnd()
{
    CProgressWnd::DestroyWindow();
}

BOOL CProgressWnd::myCreate(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth /* = FALSE */)
{
    BOOL bSuccess;

    // Register window class
    CString csClassName = AfxRegisterWndClass(CS_OWNDC|CS_HREDRAW|CS_VREDRAW,
                                              ::LoadCursor(NULL, IDC_APPSTARTING),      // NOLINT(performance-no-int-to-ptr)
...
BOOL CProgressWnd::GoModal(LPCTSTR pszTitle /*=_T("Progress")"*/, BOOL bSmooth /*=FALSE*/)
{
    ...
    if (!::IsWindow(m_hWnd) && !myCreate(pMainWnd, pszTitle, bSmooth))
        return FALSE;
...
int CProgressWnd::SetPos(int nPos)                    
{
    ...
    if (m_nMaxValue > m_nMinValue)
        nPercentage = lround (((nPos - m_nMinValue)*100.0)/(m_nMaxValue - m_nMinValue));
    else
        nPercentage = 0;
    ...
    if (nPercentage != m_nPrevPercent) 
    {
        ...
        strTitle.Format(_T("%s [%d%%]"),(LPCSTR)m_strTitle,nPercentage);
...
BEGIN_MESSAGE_MAP(CProgressWnd, CWnd)
    ...
    ON_WM_ERASEBKGND()
	//}}AFX_MSG_MAP
    ON_BN_CLICKED(IDC_CANCEL, &ThisClass::OnCancel)
END_MESSAGE_MAP()
...
{
    int x = static_cast<int>(AfxGetApp()->GetProfileInt(szSection, szEntryX, -1));
    int y = static_cast<int>(AfxGetApp()->GetProfileInt(szSection, szEntryY, -1));
