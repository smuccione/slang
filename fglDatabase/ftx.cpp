	...
	switch ( ftx->getBlockType ( ftxConn->stack.back().nPage ) )
	{
		case FTX_ROOT_TYPE | FTX_LEAF_TYPE:
			...
			} else
			{
				...
				memcpy ( leftEntry->getData(), keyStart,  lHalf * keyLen );
				memcpy ( rightEntry->getData(), keyStart + lHalf * keyLen, rHalf * keyLen );
				...
				*((unsigned long *)(keyStart + ftx->getHeader()->keySize + sizeof ( unsigned long ))) = rightPage;
				memcpy   (keyStart + sizeof ( unsigned long ), leftEntry->getData() + (lHalf - 1) * (ftx->getHeader()->keySize + ftx->getHeader()->depSize) + ftx->getHeader()->depSize, ftx->getHeader()->keySize );
			}
			break;
		case FTX_ROOT_TYPE | FTX_BRANCH_TYPE:
			...
			} else
			{
				...
				memcpy ( leftEntry->getData(),  keyStart,  lHalf * keyLen + sizeof ( unsigned long ) );
				memcpy ( rightEntry->getData(), keyStart + lHalf * keyLen, (rHalf * keyLen) + sizeof ( unsigned long ) );
				...
				*((unsigned long *)(keyStart + sizeof ( unsigned long ) + ftx->getHeader()->keySize)) = rightPage;
				memcpy    (keyStart + sizeof ( unsigned long ), leftEntry->getData() + (lHalf - 1) * keyLen + sizeof ( unsigned long ), ftx->getHeader()->keySize );
			}
			break;
		case FTX_LEAF_TYPE:
			...
			} else
			{
				...
				memcpy  ( leftEntry->getData(), keyStart,  lHalf * keyLen );
				memmove ( rightEntry->getData(), keyStart + lHalf * keyLen, rHalf * keyLen );
				...
				memcpy ( tmpKey, leftEntry->getData() + (lHalf - 1) * keyLen + ftx->getHeader()->depSize, keyLen );
		...
		case FTX_BRANCH_TYPE:
			...
			} else
			{
				...
				memcpy  ( leftEntry->getData(), keyStart,  lHalf * keyLen + sizeof ( unsigned long ) );
				memmove ( rightEntry->getData(), keyStart + lHalf * keyLen, rHalf * keyLen + sizeof ( unsigned long ));
				...
				memcpy ( tmpKey, leftEntry->getData() + (lHalf - 1) * keyLen + sizeof ( unsigned long ), keyLen );
				ftxKeyAdd ( ftx, ftxConn, tmpKey, (char *)(&leftPage) );
			}
			break;
	}
...

{
	...
	if ( ftxConn->stack.back().nKeys == 1 )
	{
		...
		{
			// get new maximum key
			ptr1 = keyStart + nKeys * keyLen + sizeof ( unsigned long );
	...
	} else
	{
		...
		if ( ftxConn->stack.back().pos == (unsigned char)(ftxConn->stack.back().nKeys - 1) && (ftxConn->stack.size() > 1) )	// pos is 0 based, nKeys is not
		{
			...
			ptr1 = keyStart + (nKeys - 1) * keyLen + ftx->getHeader()->depSize;
			memcpy ( tmpKey, ptr1, ftx->getHeader()->keySize );
...
FTX _ftxCreate ( char const *name, char const *keyExpr, size_t mode, size_t keySize, size_t depSize, bool isDescending, char exprType, char const *filterExpr, uint32_t updateCount )
{
	...
	ftx.fileSize = FTX_PAGE_SIZE * 2;	 /* one for root */
...
void _ftxAdjustFileSize ( FTX *ftx, FTX_CONNECTION *ftxConn )
{
	...
	if ( ftx->getHeader()->nLeafKeys < 4 )
	{
		...
		nBlocksRequired = (ftx->nKeys + (ftx->getHeader()->nLeafKeys - 1)) / ftx->getHeader()->nLeafKeys;
	} else
	{
		// make sure to round up
		nBlocksRequired = (ftx->nKeys  + (ftx->getHeader()->nLeafKeys - 2)) / (ftx->getHeader()->nLeafKeys - 1);
	}
...
void _ftxAddKeys ( FTX *ftx, FTX_CONNECTION *ftxConn, int keyType, size_t nKeys )
{
	...
	switch ( keyType )
	{
		...
		case FTX_BRANCH_TYPE:
			...
			{
				// need to be dense
				nBlocksRequired = (nKeys  + (ftx->getHeader()->nBranchKeys - 1)) / (ftx->getHeader()->nBranchKeys);
			} else
			{
				nBlocksRequired = (nKeys  + (ftx->getHeader()->nBranchKeys - 2)) / (ftx->getHeader()->nBranchKeys - 1);
			}
			break;
		case FTX_LEAF_TYPE:
			if ( ftx->getHeader()->nLeafKeys < 4 )
			{
				nBlocksRequired = (nKeys + (ftx->getHeader()->nLeafKeys - 1)) / ftx->getHeader()->nLeafKeys;
			} else
			{
				nBlocksRequired = (nKeys + (ftx->getHeader()->nLeafKeys - 2)) / (ftx->getHeader()->nLeafKeys - 1);
			}
			break;
	}
	...
	memset ( ftx->data + (startBlock * FTX_PAGE_SIZE), 0, (nBlocksRequired - startBlock) * FTX_PAGE_SIZE );
...
void _ftxKeyAddToIndex ( FTX *ftx, FTX_CONNECTION *ftxConn, char const *key, char const *dep )
{
	...
	if ( ftx->getHeader()->nLeafKeys < 4 )
	{
		page	= ftx->nKeys  / (ftx->getHeader()->nLeafKeys);	
	} else
	{
		page	= ftx->nKeys  / (ftx->getHeader()->nLeafKeys - 1);	// extra space for addition at end
	}
...
static void ftxKeyAddToBranchIndex ( FTX *ftx, FTX_CONNECTION *ftxConn, char const *key, char const *dep, int offset )
{
	...
	if ( ftx->getHeader()->nBranchKeys < 4 )
	{
		page	= ftx->nKeys / (ftx->getHeader()->nBranchKeys);	
	} else
	{
		page	= ftx->nKeys / (ftx->getHeader()->nBranchKeys - 1);	// extra space for addition at end
	}
...
static _inline void *ftxGetLeafKeyPtr ( FTX *ftx, FTX_LEAF *leaf, size_t nKey )
{
	...
	{
		keyPos  = nKey % (ftx->getHeader()->nLeafKeys - 1);
	}

	keyStart = leaf->getData ( );

	keyPtr = keyStart + keyPos * (ftx->getHeader()->keySize + ftx->getHeader()->depSize);
...
static _inline void *ftxGetLargestKeyPtr ( FTX *ftx, uint64_t page )
{
	...
	keyPtr = keyStart + keyPos * (keySize + depSize);
...
static _inline int ftxSortCompare ( FTX *ftx, FTX_CONNECTION *ftxConn, size_t a, size_t b )
{
	...
	{
		aPage	 = a / (header->nLeafKeys);			// extra space for addition at end
	} else
	{
		aPage	 = a / (header->nLeafKeys - 1);		// extra space for addition at end
	}
	aPage   += 2;									// add 2 to get past header and root block

	if ( header->nLeafKeys < 4 )
	{
		bPage	 = b / (header->nLeafKeys);			// extra space for addition at end
	} else
	{
		bPage	 = b / (header->nLeafKeys - 1);		// extra space for addition at end
	}
...
static _inline void ftxSortSwap ( FTX *ftx, FTX_CONNECTION *ftxConn, int a, int b )
{
	...
	if ( ftx->getHeader()->nLeafKeys < 4 )
	{
		aPage	 = a / (ftx->getHeader()->nLeafKeys);		// extra space for addition at end
	} else
	{
		aPage	 = a / (ftx->getHeader()->nLeafKeys - 1);	// extra space for addition at end
	}
	aPage   += 2;									// add 2 to get past header and root block

	if ( ftx->getHeader()->nLeafKeys < 4 )
	{
		bPage	 = b / (ftx->getHeader()->nLeafKeys);		// extra space for addition at end
	} else
	{
		bPage	 = b / (ftx->getHeader()->nLeafKeys - 1);	// extra space for addition at end
	}
	bPage   += 2;									// add 2 to get past header and root block
	...
	memcpy ( tmpBuff,	ptr1,	 ftx->getHeader()->depSize + ftx->getHeader()->keySize );
	memcpy ( ptr1,		ptr2,	 ftx->getHeader()->depSize + ftx->getHeader()->keySize );
	memcpy ( ptr2,		tmpBuff, ftx->getHeader()->depSize + ftx->getHeader()->keySize );
}
