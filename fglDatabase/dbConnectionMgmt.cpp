...
//			setsockopt ( dbFd->socket, SOL_SOCKET, SO_SNDBUF, (char *)&value, sizeof ( value ) );
			...
			CreateIoCompletionPort ( (HANDLE) dbFd->socket, dbIocpCompletionPort, (ULONG_PTR)dbFd, 0);
		} else
		{
...
DB_SERVER_DB *DB_SERVER::dbOpenTable ( vmInstance *instance, char const *tableName )
{
...

	{
		...
		auto it = dbs.find ( stringi ( ap.string() ) );
		if ( it != dbs.end ( ) )
		{
	...
	newDbs->name = ap.string().c_str();
	dbs[stringi ( ap.string() )] = newDbs;
