...
//	header.updateCount	= updateCount;
	...
	SetFilePointer ( fHandle, fileSize.LowPart, &fileSize.HighPart, FILE_BEGIN );
...
static void blobAllocateFreeBlocks ( BLOB_FILE *blob )
{
	...
	// get our file size
	blob->fileSize += blob->getHeader ( )->blockSize * BLOB_ALLOCATION_QUANTA;
	blob->mapFile ( );
	...
	do	{
		...
		if ( compressed )
		{
			...
			for ( ;; )
			{
				...
				switch ( (ret = inflate ( &stream, Z_NO_FLUSH )) )
				{
					case Z_OK:
		...
		if ( addr )
		{
			...
			if ( compressed != blobField->dat.used.compressed )
			{
				throw errorNum::scDATABASE_INTEGRITY;
			}
		}
...
void blobCopy ( BLOB_FILE *blob, char const *destDir )
{
	...
	CopyFile ( blob->name, dest.string().c_str(), 0 );
}
