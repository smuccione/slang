...
static int _tbFieldSet ( TABLE *dbf, size_t position, char const *fieldname, unsigned char type, size_t length, size_t decimals )
{
...
TABLE * _tbBuild( char const *filename, TBFIELD * field, int count)
{
	...
	for( x = 0; x < count; x++)
	{
		_tbFieldSet ( dbf, x + 1, field[x].name, field[x].type, field[x].length, field[x].decimals );
	}
...
void _tbBuildFromTable ( char const *newFilename, TABLE *tblOld )
{
	...
	if ( header.capabilities & DBF_CAPABILITY_BLOB )
	{
		...
		if ( !(blob = blobCreate ( p.string().c_str(), header.updateCount )) )
		{
			CloseHandle ( fHandle );
			throw (errorNum) GetLastError();
		}
