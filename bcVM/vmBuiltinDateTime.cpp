...
static void datef ( struct tm *utcTime, char const *fmt, BUFFER &buff )
{
	...
	while ( *fmt )
	{
		...
		} else if ( !memcmp ( fmt, "MN", 2 ) )
		{
			fmt += 2;
			buff.printf ( "%02i", utcTime->tm_min );
		} else if ( !memcmp ( fmt, "SS", 2 ) )
		{
			fmt += 2;
			buff.printf ( "%02i", utcTime->tm_sec );
		} else if ( !memcmp ( fmt, "AP", 2 ) )
		{
			fmt += 2;
		...
		} else if ( !memcmp ( fmt, "ap", 2 ) )
		{
			fmt += 2;
			buff.printf ( "%2s", utcTime->tm_hour >= 12 ? "pm" : "am");
		} else if ( !memcmp ( fmt, "DDDD", 4 ) )
		{
			fmt += 4;
			buff.write ( aDays[utcTime->tm_wday] );
		} else if ( !memcmp ( fmt, "DDD", 3 ) )
		{
			fmt += 3;
...
VAR_STR cUTCFormat ( vmInstance *instance, VAR *var, VAR *fmt )
{
	...
	{
		if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "MMMM", 4 ) )
		{
			fmtOffset += 4;
			val = classIVarAccess ( var, "MONTH" );
			if ( val->dat.l < 1 || val->dat.l > 12 ) throw errorNum::scINVALID_PARAMETER;
			buff.write ( aMonths[val->dat.l - 1] );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "MMM", 3 ) )
		{
			fmtOffset += 3;
			val = classIVarAccess ( var, "MONTH" );
			if ( val->dat.l < 1 || val->dat.l > 12 ) throw errorNum::scINVALID_PARAMETER;
			buff.write ( aShortMonths[val->dat.l - 1] );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "MM", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "MONTH" );
			if ( val->dat.l < 1 || val->dat.l > 12 ) throw errorNum::scINVALID_PARAMETER;
		...
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "YYYY", 4 ) )
		{
			fmtOffset += 4;
			val = classIVarAccess ( var, "YEAR" );
			buff.printf ( "%04zu", val->dat.l );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "YY", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "YEAR" );
			buff.printf ( "%02zu", val->dat.l % 100 );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "HH", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "HOUR" );
			buff.printf ( "%02zu", val->dat.l );
		...
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "MN", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "MINUTE" );
			buff.printf ( "%02zu", val->dat.l );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "SS", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "SECOND" );
			buff.printf ( "%02zu", val->dat.l );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "MS", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "MILLISECONDS" );
			buff.printf ( "%03zu", val->dat.l );
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "AP", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "HOUR" );
		...
		} else if ( !memcmp ( (fmt->dat.str.c + fmtOffset), "DD", 2 ) )
		{
			fmtOffset += 2;
			val = classIVarAccess ( var, "DAY" );
			buff.printf ( "%02zu", val->dat.l );
