...
static int vmClose ( class vmInstance *instance, int handle )
{
	...
	instance->resourceFree ( (void *) (ptrdiff_t) handle, [] ( void *resource ){_close ( (int) (ptrdiff_t) resource );} );
...
static int vmFcreate( class vmInstance *instance, char const *name )
{
	...
	{
		instance->resourceRegister( (void *)(ptrdiff_t)handle,  [] ( void *resource ){_close ( (int) (ptrdiff_t) resource );} , RES_MODE::RES_REINIT );
	}
	return (handle);
}
...
static VAR_STR vmFmerge( class vmInstance *instance, char const *temp, char const *inName )
{
	...
	if ( pat.extension ().string ().c_str ()[0] == '*' )
	{
		d += in.extension ();
	} else
	{
...
{
	std::filesystem::path in ( file );
	return VAR_STR ( instance, in.filename().string() );
}

static VAR_STR vmFFullPath ( class vmInstance *instance, char const *file )
{
	std::filesystem::path in ( file );
	return VAR_STR ( instance, (in.remove_filename()).string () );
}
...
static VAR_STR vmFmake( class vmInstance *instance, char const *drive, char const *path, char const *name, char const *ext )
{
	...
	return VAR_STR ( instance, std::accumulate ( std::next ( components.begin () ), components.end (), std::filesystem::path {}, std::divides {} ).string() );
}
...
static uint64_t vmFLen ( size_t handle )
{
	...
	if ( !GetFileInformationByHandle ( (HANDLE) handle, &fi ) )
	{
		throw GetLastError ( );
	}
...
static VAR_STR vmFileUniqueName ( vmInstance *instance, char const *fName )
{
	GUID guid;
	CoCreateGuid ( &guid );
...
static VAR_STR  vmFileDate ( vmInstance *instance, char const *fName )
{
	...
	{
		sTime = *localtime ( &fileStat.st_ctime );
