...
class PackParam
{
	...
	PackParam() : buff ( 1024 * 1024 ), xRefBuff ( 1024 * 1024 )
	{
	}
	virtual ~PackParam()
	{
...
static void packData ( PackParam &packParam, VAR *var )
{
	...
	switch ( TYPE ( var ) )
	{
		...
		case slangType::eCODEBLOCK:
			...
			for ( loop = 0; loop < (int)var->dat.cb.cb->nSymbols - (int)var->dat.cb.cb->nParams; loop++ )
			{
				packData ( packParam, &var[loop + 1] );
			}
			break;
		...
		case slangType::eOBJECT:
			...
			if ( classDef->cPackCB )
			{
				...
				tmp.type	= slangType::eOBJECT_ROOT;
				tmp.dat.ref.v	= var;
				tmp.dat.ref.obj	= var;
	...
	switch ( type )
	{
		...
		case packElementType::packType_codeBlock:
			...
			}
			{
				...
				var->dat.ref.v = instance->om->allocVar ( var->dat.cb.cb->nSymbols - var->dat.cb.cb->nParams + 1 );
				var->dat.ref.obj = var->dat.ref.v;
				memset ( &var->dat.ref.v[1], 0, sizeof ( VAR ) * (var->dat.cb.cb->nSymbols - var->dat.cb.cb->nParams) );
		...
		case slangType::eOBJECT:
			{
				...
				if ( classDef->cPackCB )
				{
					...
					// patch in the amount written by the C callback
					*((int32_t *) (bufferBuff ( &packParam.buff ) + sizeOffset)) = (uint32_t) (bufferSize ( &packParam.buff ) - sizeOffset);
				} else
				{
					// no data written out
		...
		case slangType::eOBJECT_ROOT:
			{
				...
				if ( data.showObjName )
				{
					...
					name.write ( classDef->name, classDef->nameLen - 1);
					nameLen += classDef->nameLen - 1;
				}
				for ( size_t loop = 0; loop < classDef->nElements; loop++ )
				{
					...
					switch ( classDef->elements[loop].type )
					{
						...
						case fgxClassElementType::fgxClassType_static:
							...
							name.write ( elem->name, elem->nameLen - 1);
							len += elem->nameLen - 1;
							if ( elem->isStatic )
							{
