...
void vmBuildSystemError ( vmInstance *instance, errorNum err )
{
	...
	*classIVarAccess ( &instance->result, "line" ) = VAR ( instance->debug->debugGetCurrentLine ( instance->error.funcDef, (fglOp *) instance->error.opCode ) );

	*classIVarAccess ( &instance->result, "functionName" ) = VAR_STR ( instance->error.funcDef->name );

	*classIVarAccess ( &instance->result, "fileName" ) = VAR_STR ( instance->error.funcDef->loadImage->srcFiles.getName ( instance->debug->debugGetCurrentSourceIndex ( instance->error.funcDef, (fglOp *) instance->error.opCode ) ) );
	...
	*arr = VAR_ARRAY ( instance );
	...
	fglOp		*op = (fglOp *) instance->error.opCode;

	while ( stack > instance->eval )
	{
		if ( stack->type == slangType::ePCALL )
		{
			callStack.entry.push_back ( vmCallstackEntry ( instance, stack->dat.pCall.func, instance->debug->debugGetCurrentLine ( stack->dat.pCall.func, op ), stack->dat.pCall.nParams, stack ) );
	...
	{
		size_t len;
		len = _snprintf_s ( entry, sizeof ( entry ) - 6, _TRUNCATE, "%s ( ", callStack.entry[loop].funcName );

		for ( size_t loop2 = 0; loop2 < (int) callStack.entry[loop].params.size ( ); loop2++ )
		{
			len += _snprintf_s ( entry + len, sizeof ( entry ) - 6 - len, _TRUNCATE, "%.32s", callStack.entry[loop].params[loop2]->getValue ( ) );
			if ( loop2 != callStack.entry[loop].params.size ( ) - 1 )
			{
				if ( len < sizeof ( entry ) - 6 )
				{
					len += _snprintf_s ( entry + len, sizeof ( entry ) - 6 - len, _TRUNCATE, ", " );
				}
			}
		}
...
static VAR_OBJ_TYPE<_P("fixedArrayEnumerator")> fixedArrayEnumeratorGetEnumerator ( vmInstance *instance, VAR_OBJ *object )
{
	return *object;
}
...
static VAR_OBJ_TYPE<_P("sparseArrayEnumerator")> sparseArrayEnumeratorGetEnumerator ( vmInstance *instance, VAR_OBJ *object )
{
	return *object;
}
...
static VAR_OBJ_TYPE<_P("stringEnumerator")>  stringEnumeratorGetEnumerator ( vmInstance *instance, VAR_OBJ *object )
{
	return *object;
}
...
{
	auto type = instance->atomTable->atomGetType ( fName );
...
static bool vmLoadLibrary ( vmInstance *instance, char const *fName )
{
	...
	auto type = instance->atomTable->atomGetType ( p.string().c_str() );
	if ( type == atom::atomType::aLOADIMAGE )
	{
		// already loaded
		return true;
	...
	type = instance->atomTable->atomGetType ( p.string().c_str() );
	if ( type == atom::atomType::aLOADIMAGE )
	{
		// already loaded
		return true;
	...
	auto entry = globalFileCache.read ( p.string ().c_str () );
	if ( !entry )
	{
		return false;
	}
	...
	try
	{
		...
		instance->load ( dup, p.string ().c_str (), true, true );
	} catch ( ... )
	{
		free ( data );
	}
	...
	auto loadImage = instance->atomTable->atomGetLoadImage ( p.string ().c_str () ); // NOLINT
...
static VAR *varCopy ( vmInstance *instance, VAR *dst, VAR *src, std::vector<std::pair<VAR *, VAR *>> *xRef )
{
	...
	if ( !dst )
	{
		switch ( src->type )
		{
			...
			case slangType::eCODEBLOCK_ROOT:
				{
					dst = instance->om->allocVar ( src->dat.cb.cb->nSymbols - src->dat.cb.cb->nParams + 1 );	// allocate array_fixed + elements
					*dst = *src;		// copy over array information... but leave elements null, we'll copy them individually
				}
				break;
			default:
	...
	switch ( src->type )
	{
		...
		case slangType::eARRAY_SPARSE:
			...
			break;
		case slangType::eARRAY_FIXED:
			for ( size_t loop = 0; loop < (unsigned)(src->dat.arrayFixed.endIndex - src->dat.arrayFixed.startIndex + 1); loop++ )
			{
