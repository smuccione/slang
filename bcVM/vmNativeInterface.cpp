...
static void makeTypes( opFile *file, opFunction *funcDef, vmDispatcher *func )
{
	...
	for ( uint32_t loop = 1; loop <= nCParams; loop++ )
	{
		...
		switch ( pTypes[loop] )
		{
			case bcFuncPType::bcFuncPType_Short_Int:
			case bcFuncPType::bcFuncPType_Long_Int:
				funcDef->params.add( file->sCache.get ( paramName ), symIntType, funcDef->location, (new astNode ( file->sCache, astOp::symbolValue, paramName ))->setLocation ( funcDef->location ), false );
				break;
			case bcFuncPType::bcFuncPType_Double:
				funcDef->params.add( file->sCache.get ( paramName ), symDoubleType, funcDef->location, (new astNode ( file->sCache, astOp::symbolValue, paramName ))->setLocation ( funcDef->location ), false );
				break;
			case bcFuncPType::bcFuncPType_Variant:
				funcDef->params.add( file->sCache.get ( paramName ), symVariantType, funcDef->location, (new astNode ( file->sCache, astOp::symbolValue, paramName ))->setLocation ( funcDef->location ), false );
				break;
			case bcFuncPType::bcFuncPType_String:
			case bcFuncPType::bcFuncPType_Var_String:
				funcDef->params.add( file->sCache.get ( paramName ), symStringType, funcDef->location, (new astNode ( file->sCache, astOp::symbolValue, paramName ))->setLocation ( funcDef->location ), false );
				break;
...
void vmNativeInterface::nativeClass::property( char const *propName, fgxClassElementType type, char const *initializer )
{
	...
	if ( (ptrdiff_t) this )
	{
		...
		elem->name = native->file->sCache.get ( propName );
		elem->isVirtual = nativeVirtual;
		elem->isStatic = nativeStatic;
		elem->symType = symVariantType;
		...
		if ( initializer )
		{
			...
			elem->data.iVar.initializer = native->file->parseExpr( src, false, true, 0, true, false );
		} else
		{
			elem->data.iVar.initializer = (new astNode ( native->file->sCache, astOp::symbolValue, elem->name ));
		}

		assert ( elem->data.iVar.initializer );
...
void vmNativeInterface::nativeClass::require( char const* name, ... )
{
	...
	do {
		name = va_arg( nameList, char const *); // NOLINT (clang-analyzer-valist.Uninitialized)
		if ( name )
		{
			cls->requiredClasses.push_back( native->file->sCache.get ( name ) );
		}
...
opFunction *vmNativeInterface::nativeClass::method( char const *methodName, vmDispatcher *disp, ... )
{
	...
	if ( native->file )
	{
		...
		if ( !strccmp( methodName, "new" ) )
		{
			...
			newFunc = native->file->parseMethod( src, cls, buildString( nativeClassName, "new", "method" ).c_str(), true, false );
		...
		if ( !strccmp( methodName, "release" ) )
		{
			...
			newMeth = "(){}";
			source src ( &native->file->srcFiles, native->file->sCache, "(INTERNAL)", newMeth );
			releaseFunc = native->file->parseMethod( src, cls, buildString( nativeClassName, "release", "method" ).c_str(), true, false );
		...
		for ( uint32_t loop = 1; loop <= bcFunc->nCParams; loop++ )
		{
			switch ( bcFunc->pTypes[loop] )
			{
				...
				case bcFuncPType::bcFuncPType_Long_Int:
					...
					if ( newFunc ) newFunc->params.add( funcDef->params[sParam]->getName(), symIntType, funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
					sParam++;
					break;
				case bcFuncPType::bcFuncPType_Double:
					if ( !selfProcessed ) throw errorNum::scTYPE_CONVERSION;
					if ( newFunc ) newFunc->params.add( funcDef->params[sParam]->getName(), symDoubleType, funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
					sParam++;
					break;
				case bcFuncPType::bcFuncPType_Variant:
					if ( selfProcessed )
					{
						if ( newFunc ) newFunc->params.add( funcDef->params[sParam]->getName(), symVariantType, funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
					} else
					{
						selfProcessed = true;
						...
						if ( newFunc && !newFunc->params.size() ) newFunc->params.add( native->file->selfValue, symbolTypeClass ( symObjectType, native->file->sCache.get ( nativeClassName ) ), funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
						if ( releaseFunc && !releaseFunc->params.size() ) releaseFunc->params.add( native->file->selfValue, symObjectType, funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
					}
					sParam++;
					break;
				case bcFuncPType::bcFuncPType_String:
				case bcFuncPType::bcFuncPType_Var_String:
					if ( !selfProcessed ) throw errorNum::scTYPE_CONVERSION;
					if ( newFunc ) newFunc->params.add( funcDef->params[sParam]->getName(), symStringType, funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
					sParam++;
					break;
				default:
					break;
		...
		for ( ;; )
		{
			...
			if ( nParam <= (int)funcDef->params.size() )
			{
				...
				if ( newFunc )
				{
					...
					newFunc->params[nParam - 1]->initializer = ini;
				} else
				{
					funcDef->params[nParam - 1]->initializer = ini;
				}
			}
		}
	...
	if ( native->file )
	{
			...
			switch ( bcFunc->pTypes[loop] )
			{
				...
				case bcFuncPType::bcFuncPType_Variant:
					...
					} else
					{
						...
						funcDef->params[sParam]->name = native->file->selfValue;
						funcDef->params[sParam]->type = symbolTypeClass ( symObjectType, native->file->sCache.get ( nativeClassName ) );
						if ( newFunc && !newFunc->params.size ( ) ) newFunc->params.add ( native->file->selfValue, symObjectType, funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
						if ( releaseFunc && !releaseFunc->params.size ( ) ) releaseFunc->params.add ( native->file->selfValue, symbolTypeClass ( symObjectType, native->file->sCache.get ( nativeClassName ) ), funcDef->params[sParam]->location, (new astNode ( native->file->sCache, astOp::symbolValue, funcDef->params[sParam]->getName () ))->setLocation ( funcDef->location ), false );
					}
					sParam++;
					break;
				case bcFuncPType::bcFuncPType_String:
		...
		for ( ;; )
		{
			...
			if ( nParam <= ( int) funcDef->params.size ( ) )
			{
				...
				if ( !ini->isValue ( ) ) throw errorNum::scINTERNAL;
		...
		try
		{
			...
			for ( ;; )
			{
				...
				if ( nParam <= (int32_t)funcDef->params.size() )
				{
					...
					funcDef->params[nParam - 1]->initializer = file->parseExpr ( src, false, true, 0, true, false );
					if ( !funcDef->params[nParam - 1]->initializer->isValue() )
					{
						throw errorNum::scINTERNAL;
					}
				}
...
static size_t numLines ( char const *str )
{
	size_t nLines = 0;
	while ( *str )
	{
...
void vmNativeInterface::compile( int cls, size_t lineNumber, char const *fileName, char const *code )
{
	if ( file )
	{
		source src ( &file->srcFiles, file->sCache, fileName, code, (uint32_t) (lineNumber - numLines ( code )) );
		file->parseFile( src, true, false );
	}
}

void vmNativeInterface::compile ( vmNativeInterface::nativeClass &cls, size_t lineNumber, char const *fileName, char const *code )
{
	if ( file )
	{
		auto c = file->findClass ( cls.nativeClassName );
		source src ( &file->srcFiles, file->sCache, fileName, code, (uint32_t) (lineNumber - numLines ( code )) );
		if ( file->parseInnerClass ( src, true, c, false ) )
		{
			throw errorNum::scINTERNAL;
		}
	}
