...
{
	CFgl_image	*cImage = (CFgl_image *) classGetCargo ( obj );
	return ( cImage->m_bmp->GetHeight() );
}

int cImageGetWidth ( VAR *obj )
{
	CFgl_image	*cImage = (CFgl_image *) classGetCargo ( obj );
	return ( cImage->m_bmp->GetWidth() );
}
...
static void cGraphicsDrawString ( VAR *graphics, VAR_STR *string, VAR_OBJ *font, VAR_OBJ *pointF, VAR_OBJ *brush )
{
	...
	cGraphics->DrawString ( _bstr_t ( string->dat.str.c ), _bstr_t ( string->dat.str.c ).length ( ), cFont, *cPointF, cSolidBrush );
}
