...
static size_t at ( VAR_STR substr, VAR_STR str, size_t offset )
{
	...
	for ( matchPos = offset; matchPos < searchLen; matchPos++ )
	{
		if ( !memcmp ( str + matchPos, substr.dat.str.c, substr.dat.str.len ) )
		{
			return matchPos + 1;
		}
	}
...
static size_t stratprev ( char const *subStr, char const *str, int occur, int startpos )
{
	...
	while ( --startpos > 0 )
	{
		if ( !memcmp ( str + startpos, subStr, len) )
		{
			if ( !--occur )
			{
				return ( startpos + 1 );
...
static int atlast ( char const *substr, char const *str )
{
	...
	{
		if ( !memcmp ( str + matchPos, substr, subStrLen ) )
		{
			foundPos = matchPos;
		}
	}
	...
	void init ( const std::vector<std::string> &words )
	{
		...
		for ( size_t i = 0; i < words.size ( ); ++i )
		{
			...
			for ( size_t j = 0; j < keyword.size ( ); ++j )
			{
				...
				if ( g[c * MAXS + currentState] == -1 )
				{ // Allocate a new node
					g[c * MAXS + currentState] = (uint32_t)states++;
				}
				currentState = g[c * MAXS + currentState];
			}
...
static int multiPatternFind ( VAR_STR *strP, VAR_STR *pattern, uint32_t maxMatch )
{
	...
	{
		currentState = matcher.findNextState ( currentState, *str );
...
VAR_STR strTemplate ( vmInstance *instance, VAR_STR *str, VAR *arr )
{
	...
	for ( int i = 0; s[0]; s++, i++ )
	{
		currentState = matcher.findNextState ( currentState, *s );
		if ( matcher.out[currentState].isEmpty ( ) )
		{
...
void builtinStringInit ( class vmInstance *instance, opFile *file )
{
	REGISTER( instance, file );
		...
		DOC ( R"(
					@func at returns the offset of the first occurance of substring in string beginning at [offset=1]
					...
					@func atI returns the case insensitive offset of the first occurance of substring in string beginning at [offset=1]
						@param param1(substr) string to search for
						@param param2(str) specifies the string to search
						@param param3(offset) starting offset to search
					...
					@func atDiffI returns the case insensitive offset of the first difference between string1 and string1
						@param param1(str1)
						@param param2(str2)
					@func atLast returns the offset of the last occurance of substr in string
						@param param1(str)
						@param param2(substr)
					@func atLastI returns the case insensitive offset of the last occurance of substr in string
						@param param1(str)
						@param param2(substr)
					@func atNext returns the offset of the index'th occurance of substr in str
						@param param1(str)
						@param param2(substr)
						@param param3(index)
					@func atNextI returns the case insensitive offset of the index'th occurance of substr in str
						@param param1(str)
						@param param2(substr)
						@param param3(index)
