...
void vmDebug::debugRemoveTracePoint ( void )
{
	dbTmpBP *next;
	dbTmpBP *bp;
...
void vmDebug::debugRemoveTracePoint ( fglOp *op )
{
	dbTmpBP *prev;
	dbTmpBP *bp;
...
void vmDebug::debugSetTracePoint ( vmBPType type, fglOp *op )
{
	dbTmpBP *bp;
...
uint32_t vmDebug::debugGetCurrentLine ( bcFuncDef *funcDef, fglOp *op )
{
	bcLoadImage *image;
...
uint32_t vmDebug::debugGetCurrentSourceIndex ( bcFuncDef *funcDef, fglOp *op )
{
	bcLoadImage *image;
...
void vmDebug::debugStepLine ( vmInstance *instance, bcFuncDef *funcDef, fglOp *op, VAR *stack )
{
		...
		switch ( op->op )
		{
			case fglOpcodes::ret:
				...
				for ( ; tmpStack > instance->eval; )
				{
					...
					if ( tmpStack->type == slangType::ePCALL && (tmpStack->dat.pCall.func->conv == fgxFuncCallConvention::opFuncType_Bytecode) )
					{
						...
						{
							debugSetTracePoint ( vmBPType::vmBPType_Trace, tmpStack->dat.pCall.op );
						}
						break;
					}
				}
			...
			case fglOpcodes::jmpnnull:
				if ( (&image->csBase[op->imm.index] < opStart) || (&image->csBase[op->imm.index] > opEnd) )
				{
					...
					debugSetTracePoint ( vmBPType::vmBPType_Trace, &image->csBase[op->imm.index] );
				}
				break;
			default:
				break;
	...
	if ( opEnd )
	{
		...
		debugSetTracePoint ( vmBPType::vmBPType_Trace, opEnd );
	}
}
...
void vmDebug::debugStepOp ( vmInstance *instance, bcFuncDef *funcDef, fglOp *op, VAR *stack, bool stepInto, vmBPType type )
{
	VAR *dest;
	VAR *basePtr;
	VAR *caller;
	...
	switch ( op->op )
	{
		case fglOpcodes::ret:
			...
			for ( ;; )
			{
				...
				if ( stack->type == slangType::ePCALL && (stack->dat.pCall.func->conv == fgxFuncCallConvention::opFuncType_Bytecode) )
				{
					...
					{
						debugSetTracePoint ( type, stack->dat.pCall.op );
					}
					break;
				}
			}
		...
		case fglOpcodes::jmpnnull:
			// destination of the jump is outside of current code emitted for this line... set a temporary breakpoint on the destination
			debugSetTracePoint ( type, &funcDef->loadImage->csBase[op->imm.index] );
			break;
		...
		case fglOpcodes::callBCTail:
			if ( stepInto )
			{
				...
				debugSetTracePoint ( type, &callerFDef->cs[0] );
			} else
			{
				debugSetTracePoint ( type, op );
			}
			break;
...
void vmDebug::debugSetProfiling ( vmInstance *instance )
{
...
// step by line... step into steps through calls
void vmDebug::debugStepOut ( vmInstance *instance, bcFuncDef *funcDef, fglOp *op, VAR *stack )
{
	bcLoadImage *image;
			...
			case fglOpcodes::ret:
				...
				for ( ;; )
				{
					...
					if ( stack->type == slangType::ePCALL && (stack->dat.pCall.func->conv == fgxFuncCallConvention::opFuncType_Bytecode) )
					{
						...
						if ( stack->dat.pCall.op )
						{
							debugSetTracePoint ( vmBPType::vmBPType_Trace, stack->dat.pCall.op );
						}
						break;
					}
				}
...
std::vector<size_t> vmDebug::debugGetGotoTargets ( vmInstance *instance, bcFuncDef *funcDef )
{
	auto image = funcDef->loadImage;
	image->populateDebug ();
...
// step by line... step into steps through calls
bool vmDebug::debugSetIP ( vmInstance *instance, bcFuncDef *funcDef, char const *fileName, size_t line )
{
	fglOp *targetOp = 0;
	fglOp *op;
	bcLoadImage *image;
...
void vmDebug::debugRunToLine ( vmInstance *instance, char const *fileName, size_t line )
{
	size_t	srcIndex = 0;
	...
	instance->atomTable->typeMap ( atom::atomType::aLOADIMAGE, [&]( uint32_t index )
								   {
									   ...
									   if ( (srcIndex = image->srcFiles.getStaticIndex ( fileName )) != -1 )
									   {
										   ...
										   {
											   debugSetTracePoint ( vmBPType::vmBPType_Trace, dIt->op );
											   return true;
										   }
									   }
									   return false;
	...
	for ( ;; )
	{
		...
		switch ( TYPE ( var ) )
		{
			...
			case slangType::eCODEBLOCK:
				{
					...
					for ( loop = 0; loop < var->dat.cb.cb->nSymbols - var->dat.cb.cb->nParams; loop++ )
					{
			...
			case slangType::eOBJECT:
				...
				for ( loop = 0; loop < var->dat.obj.classDef->nElements; loop++ )
				{
...
vmCallstackEntry::vmCallstackEntry ( vmInstance *instance, bcFuncDef *func, size_t lineNum, size_t nParams, VAR *param )
{
	...
	fileName = _strdup ( func->loadImage->srcFiles.getName ( instance->debug->debugGetCurrentSourceIndex ( func, func->cs ) ) );
...
vmCallStack *vmDebug::debugGetCallstack ( vmInstance *instance )
{
	...
	while ( stack >= instance->eval )
	{
		...
		{
			callStack->entry.push_back ( vmCallstackEntry ( instance, stack->dat.pCall.func, debugGetCurrentLine ( stack->dat.pCall.func, op ), stack->dat.pCall.nParams, stack ) );
...
bcFuncDef *vmDebug::debugFunc ( vmInstance *instance, size_t stackFrame )
{
	VAR *stack;
...
VAR *vmDebug::debugBasePtr ( vmInstance *instance, size_t stackFrame )
{
	VAR *stack;
...
vmFunctionListEntry::vmFunctionListEntry ( vmInstance *instance, uint32_t atomNum )
{
	...
	if ( !funcDef || !funcDef->loadImage )
	{
		...
		this->funcName = _strdup ( funcName );
		this->fileName = _strdup ( "unknown" );
		this->atomNum = 0;
		this->lineNum = 0;
		this->className = 0;
	...
	if ( (memcmp ( LAMBDA_ID, funcDef->name, sizeof ( LAMBDA_ID ) - 1 ) != 0) && (_at ( ".", (char *)funcDef->name ) != -1) )
	{
	...
	this->funcName = _strdup ( funcName );
	this->fileName = _strdup ( funcDef->loadImage->srcFiles.getName ( instance->debug->debugGetCurrentSourceIndex ( funcDef, funcDef->cs ) ) );
	this->atomNum = funcDef->atom;
	this->lineNum = instance->debug->debugGetCurrentLine ( funcDef, funcDef->cs );
...
vmFunctionList *vmDebug::debugGetFunctionList ( vmInstance *instance )
{
	return new vmFunctionList ( instance );
}

void vmDebug::debugRegisterInspector ( char const *name, vmInspector::vmInspectorCb cb )
{
	...
	inspectors.push_back ( vmInspector ( name, cb ) );
}
...
vmInspectorList *vmDebug::debugGetInspectors ( vmInstance *instance )
{
...
// expansion callback
vmInspectList *vmDebug::debugInspect ( vmInstance *instance, size_t stackFrame, vmDebugVar *var, uint32_t start, uint32_t end )
{
	VAR *stack;
...
// top level
vmInspectList *vmDebug::debugInspect ( vmInstance *instance, size_t num, size_t stackFrame )
{
	VAR *stack;
...
vmFunctionProfile::vmFunctionProfile ( vmInstance *instance, uint32_t atomNum )
{
	...
	fileName = funcDef->loadImage->srcFiles.getName ( instance->debug->debugGetCurrentSourceIndex ( funcDef, funcDef->cs ) );
...
vmProfileData *vmDebug::debugGetExecutionProfile ( vmInstance *instance )
{
...
size_t vmDebug::debugSetBreakpoint ( vmInstance *instance, char const *fileName, size_t lineNum )
{
	lineNum = debugAdjustBreakpoint ( instance, fileName, lineNum );

	if ( lineNum )
	{
		if ( breakPoints.find ( vmBreakpoint ( fileName, lineNum, 0 ) ) == breakPoints.end () )
		{
			instance->atomTable->typeMap ( atom::atomType::aLOADIMAGE, [&]( uint32_t atm )
										   {
											   ...
											   if ( srcIndex != -1 )
											   {
												   ...
												   {
													   breakPoints.insert ( vmBreakpoint ( fileName, lineNum, (*dIt).op ) );
													   return true;
												   }
											   }
											   return false;
...
void vmDebug::debugClearBreakpoint ( vmInstance *instance, char const *fileName, size_t lineNum )
{
	lineNum = debugAdjustBreakpoint ( instance, fileName, lineNum );

	if ( lineNum )
	{
		auto it = breakPoints.find ( vmBreakpoint ( fileName, lineNum, 0 ) );
...
void vmDebug::debugClearAllBreakpoints ( vmInstance *instance, char const *fileName, size_t lineNum )
{
	breakPoints.clear ();
	// not found... just ignore
}

void vmDebug::debugMakeBreakpoints ( vmInstance *instance, fglOp *op )
{
	if ( breakPoints.size () )
	{
		instance->atomTable->typeMap ( atom::atomType::aLOADIMAGE, [&]( uint32_t index )
											   ...
											   if ( it->op && it->op != op )
											   {
												   switch ( it->op->op )
												   {
													   ...
													   case fglOpcodes::debugTrace:
														   ...
														   break;
													   default:
														   debugSetTracePoint ( vmBPType::vmBPType_Breakpoint, it->op );
														   break;
												   }
											   }
										   }
...
size_t vmDebug::debugAdjustBreakpoint ( vmInstance *instance, stringi const &fileName, size_t lineNum )
{
...
vmDebug::vmDebug ()
{
	...
	debugRegisterInspector ( "Locals", debugLocalInspector );
	debugRegisterInspector ( "Globals", debugGlobalInspector );
}
...
fglOp *debugSignal ( vmInstance *instance, vmStateType state, fglOp *op, bcFuncDef *funcDef, VAR *stackPtr )
{
	...
	if ( !instance->debug || !instance->debug->enabled )
	{
		...
		//instance->debug->debugRemoveTracePoint();
	...
	switch ( state )
	{
		...
		case vmStateType::vmHalt:
			if ( instance->debug->isResuming )
			{
				...
				instance->debug->halt = false;
				instance->debug->debugRemoveTracePoint ();
				instance->debug->debugMakeBreakpoints ( instance, 0 );
				return op;
			}
			instance->debug->debugRemoveTracePoint ();
		...
		case vmStateType::vmTrap:
			if ( !instance->debug->isStepping || !instance->debug->stepInto )
			{
				return op;
			}
			instance->debug->debugRemoveTracePoint ();
	...
	if ( op )
	{
		strcpy_s ( msg.location.fName, sizeof ( msg.location.fName ), funcDef->loadImage->srcFiles.getName ( instance->debug->debugGetCurrentSourceIndex ( funcDef, op ) ) );
		...
		msg.location.line = instance->debug->debugGetCurrentLine ( funcDef, op );
		...
		if ( !msg.location.line )
		{
			...
			instance->debug->debugStepLine ( instance, funcDef, op, stackPtr );
			return (op);
		}
	} else
	{
		...
		strcpy_s ( msg.location.fName, sizeof ( msg.location.fName ), funcDef->loadImage->srcFiles.getName ( instance->debug->debugGetCurrentSourceIndex ( funcDef, funcDef->cs ) ) );
	...
	instance->debug->debugInterrupt ( &msg );		// send message to client
	instance->debug->debugMakeBreakpoints ( instance, op );
...
void debugRegister ( vmInstance *instance, vmDebug *debug )
{
...
void debugContinue ( vmDebugMessage *msg )
{
	...
	msg->instance->debug->debugContinue ( msg );
}
...
void debugStepInto ( vmInstance *instance )
{
	...
	instance->debug->debugStepLine ( instance, instance->debug->funcDef, instance->debug->IP, instance->debug->stack );
}
...
void debugRunToLine ( vmInstance *instance, char const *fileName, size_t line )
{
	instance->debug->isStepping = false;
	instance->debug->stepInto = false;
	instance->debug->debugRunToLine ( instance, fileName, line );
}
...
void debugStepOut ( vmInstance *instance )
{
	instance->debug->isStepping = false;
	instance->debug->stepInto = false;
	instance->debug->debugStepOut ( instance, instance->debug->funcDef, instance->debug->IP, instance->debug->stack );
}
...
bool debugSetIP ( vmInstance *instance, char const *fileName, size_t line )
{
	...
	return instance->debug->debugSetIP ( instance, instance->debug->funcDef, fileName, line );
}

vmCallStack *debugGetCallstack ( vmInstance *instance )
{
	return instance->debug->debugGetCallstack ( instance );
}

vmFunctionList *debugGetFunctionList ( vmInstance *instance )
{
	return instance->debug->debugGetFunctionList ( instance );
}

vmInspectorList *debugGetInspectors ( vmInstance *instance )
{
	return instance->debug->debugGetInspectors ( instance );
}

vmInspectList *debugInspect ( vmInstance *instance, size_t num, size_t stackFrame )
{
	return instance->debug->debugInspect ( instance, num, stackFrame );
}

vmInspectList *debugInspect ( vmInstance *instance, size_t stackFrame, vmDebugVar *var, uint32_t start, uint32_t end )
{
	return instance->debug->debugInspect ( instance, stackFrame, var, start, end );
}

size_t debugSetBreakpoint ( vmInstance *instance, char const *fileName, size_t addr )
{
	return instance->debug->debugSetBreakpoint ( instance, fileName, addr );
}

void debugClearBreakpoint ( vmInstance *instance, char const *fileName, size_t addr )
{
	instance->debug->debugClearBreakpoint ( instance, fileName, addr );
}

void debugClearAllBreakpoints ( vmInstance *instance, char const *fileName, size_t addr )
{
	instance->debug->debugClearAllBreakpoints ( instance, fileName, addr );
}
...
{
	return instance->debug->debugGetExecutionProfile ( instance );
}

size_t debugAdjustBreakpoint ( vmInstance *instance, char const *fName, size_t lineNum )
{
	return instance->debug->debugAdjustBreakpoint ( instance, fName, lineNum );
}
