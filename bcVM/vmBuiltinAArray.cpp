...
static VAR_OBJ_TYPE<_P("aArrayEnumerator")> cBTreeGetEnumerator ( vmInstance *instance, VAR *object )
{
	PUSH ( *object );
...
static VAR arrayAdd( vmInstance *instance, VAR *arr, nParamType numParam )
{
#if 0
	VAR			**var;
	VAR			 *var2;
	...
	{
		case slangType::eARRAY_FIXED:
			var = (VAR **)instance->om->allocVar ( (unsigned)(arr->dat.arrayFixed.endIndex - arr->dat.arrayFixed.startIndex + 2) );

			memcpy( var, arr->dat.arrayFixed.elem, sizeof( VAR * ) * (unsigned)(arr->dat.arrayFixed.endIndex - arr->dat.arrayFixed.startIndex + 1) );

			arr->dat.arrayFixed.elem = *var;
			arr->dat.arrayFixed.endIndex += 1;

			arr->dat.arrayFixed.elem[arr->dat.arrayFixed.endIndex - arr->dat.arrayFixed.startIndex] = 0;

			if ( (uint64_t )numParam )
			{
				arr->dat.arrayFixed.elem[arr->dat.arrayFixed.endIndex - arr->dat.arrayFixed.startIndex] = numParam[0];
			} else
			{
				var2 = instance->om->allocVar();
				var2->type = slangType::eNULL;

				// not sure about code generation... this ensures that any movement by cause by .elem being volatile during a collection is handled
				arr->dat.arrayFixed.elem[arr->dat.arrayFixed.endIndex - arr->dat.arrayFixed.startIndex] = var2;
			}
			instance->result = std::move ( VAR_REF ( arr, arr ) );
			break;
...
static int64_t arraySize( vmInstance *instance, VAR *param, int64_t size )
{
	int64_t 	  oldSize;
	int64_t 	  nCopy;
	int64_t 	  loop;
	...
	if ( TYPE ( param ) != slangType::eARRAY_ROOT )
	{
		throw errorNum::scINVALID_PARAMETER;
	}

	switch ( TYPE( param->dat.ref.v ) )
	{
		case slangType::eARRAY_FIXED:
			oldSize = param->dat.ref.v->dat.arrayFixed.endIndex - param->dat.ref.v->dat.arrayFixed.startIndex + 1;
			...
			param->dat.arrayFixed.endIndex = param->dat.arrayFixed.startIndex + size - 1;

			newVar = instance->om->allocVar( param->dat.ref.v->dat.arrayFixed.endIndex - param->dat.ref.v->dat.arrayFixed.startIndex + 1 + 1 );
			*newVar = *param;

			memcpy( &newVar[1], &param[1], (size - nCopy ) * sizeof( VAR ) );

			for ( loop = nCopy; loop < size; loop++ )
			{
				param[loop + 1].type = slangType::eNULL;
			}
		...
		} else
		{
			...
			switch ( TYPE ( var2 ) )
			{
				...
				case slangType::eSTRING:
					std::stable_sort ( sArr.begin ( ), sArr.end ( ), []( auto l, auto r )
										{
											return memcmp ( l->dat.str.c, r->dat.str.c, l->dat.str.len + 1 );
										}
					);
					break;
				default:
		...
		} else
		{
			switch ( TYPE ( var2 ) )
			{
				...
				case slangType::eSTRING:
					std::sort ( sArr.begin ( ), sArr.end ( ), []( auto l, auto r )
									   {
										   return memcmp ( l->dat.str.c, r->dat.str.c, l->dat.str.len + 1 ) < 0;
									   }
					);
					break;
				default:
		...
		switch ( TYPE ( arr[0] ) )
		{
			...
			case slangType::eSTRING:
				if ( isCaseSensitive )
				{
					std::stable_sort ( sArr.begin ( ), sArr.end ( ), [dir]( auto l, auto r )
										{
										   return memcmp ( l->dat.str.c, r->dat.str.c, l->dat.str.len + 1 ) * dir < 0; // NOLINT ( bugprone-suspicious-string-compare )
										}
					);
				} else
				{
					std::stable_sort ( sArr.begin ( ), sArr.end ( ), [dir]( auto l, auto r )
										{
										   return memcmpi ( l->dat.str.c, r->dat.str.c, l->dat.str.len + 1 ) * dir < 0; // NOLINT ( bugprone-suspicious-string-compare )
										}
					);
				}
				break;
					...
					std::stable_sort ( sArr.begin ( ), sArr.end ( ), [dir, column]( auto l, auto r )
									   {
										   ...
										   switch ( TYPE ( aVal ) )
										   {
											   ...
											   case slangType::eLONG:
												   ...
												   break;
											   case slangType::eSTRING:
												   ret = (int64_t) memcmp ( aVal->dat.str.c, bVal->dat.str.c, aVal->dat.str.len + 1 ) * dir;
												   break;
											   default:
												   throw errorNum::scINVALID_PARAMETER;
										   }
...
static VAR_OBJ_TYPE<_P("aArrayEnumerator")>  aArrayEnumeratorGetEnumerator ( vmInstance *instance, VAR_OBJ *object )
{
	return *object;
}
