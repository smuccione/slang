...
void omCopyGcMemoryPage::validateGuard ( void )
{
	...
	while ( og )
	{
		...
		if ( memcmp ( og->beginPad, PAD_BYTES, NUM_PAD_BYTES ) != 0 )
		{
			throw errorNum::scINTERNAL;
		}
		if ( memcmp ( og->endPad, PAD_BYTES, NUM_PAD_BYTES ) != 0 )
		{
			throw errorNum::scINTERNAL;
		}
...
//			instance->atoms->markInUse (  newVal->dat.atom );
		...
		case slangType::eARRAY_FIXED:
			for ( loop = 0; loop < (unsigned)(newVal->dat.arrayFixed.endIndex - newVal->dat.arrayFixed.startIndex + 1); loop++ )
			{
				_move ( &newVal[loop + 1], false, depth );
			}
			break;
...

{
	...
	{
		ptr = (unsigned char *)((ptrdiff_t)(lastPage->data + lastPage->used + OM_BOUNDARY) & ~OM_BOUNDARY);
