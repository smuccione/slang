...
public:
	...
	libStore ( stringi fName, bool del = false )
	{
		FILE			*file;
		libEntry		*entry;
		...
		if ( !del )
		{
			if ( !fopen_s ( &file, fName.c_str ( ), "rb" ) )
			{
				while ( !feof ( file ) )
				{
					...
					fread ( entry->data, sizeof ( char ), entry->dataLen, file );
	...
	void add ( stringi fName )
	{
		...
		} else
		{
			...
			entry->data = (uint8_t *)malloc ( sizeof ( char ) * entry->dataLen );
