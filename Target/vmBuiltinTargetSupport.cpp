...
#if _DEBUG
	sprintf_s ( version, sizeof ( version ), "v%u.%u.%u %s %s - DEBUG", HIWORD ( VerFixedFileInfo->dwFileVersionMS ), LOWORD ( VerFixedFileInfo->dwFileVersionLS ), SVN_REVISION, SVN_BUILD_TIME, SVN_LOCAL_MODIFICATIONS ? " - NON-COMMITTED" : "" );
#else
	sprintf_s ( version, sizeof ( version ), "v%u.%u.%u %s %s", HIWORD ( VerFixedFileInfo->dwFileVersionMS ), LOWORD ( VerFixedFileInfo->dwFileVersionLS ), SVN_REVISION, SVN_BUILD_TIME, SVN_LOCAL_MODIFICATIONS ? " - NON-COMMITTED" : "" );
#endif
...
auto vmSmartRun ( vmInstance *instance, char const *fName, int flags, nParamType params )
{
	...
	auto dir = std::filesystem::path ( p ).remove_filename ().string ();
