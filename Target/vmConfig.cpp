...
static flagId areas[]	=	{	
								{ "CONFIG",		6, CONFIG_CONFIG },
								{ "VM",			6, CONFIG_VM },
								{ "SERVER",		6, CONFIG_SERVER },
								{ "DATABASE",	8, CONFIG_DATABASE },
								{ "MODULE",		6, CONFIG_MODULE },
								{ "RUN",		6, CONFIG_RUN },
								{ "SCHEDULED",	9, CONFIG_SCHEDULED },
								{ 0,			0, 0 }
							};
...
void vmConfig ( std::string const &fName )
{
	...
	vmConf.vmCacheEntry				= 32*1024;		// defaults to 32K cache entries
	...
	vmConf.vmObjectDefaultPageSize	= 32*1024;
	vmConf.vmObjectMemoryMax		= 512 * 1024 * 1024;
			...
			for ( loop = 0; areas[loop].id; loop++ )
			{
				if ( !strccmp ( areas[loop].id, inLine ) )
				{
					switch ( areas[loop].bitPos )
					{
						...
						case CONFIG_RUN:
						case CONFIG_VM:
						case CONFIG_SCHEDULED:
							area = (CONFIG_SECTIONS)areas[loop].bitPos;
							break;
					}
					break;
				}
				...
				case CONFIG_CONFIG:
					...
					if ( !strccmp ( name, "debugger" )  )
					{
						...
						} else if ( !strccmp ( subValue1, "adapter" ) )
						{
							...
							if ( *subValue2 )
							{
								vmConf.servers.push_back ( vmServers ( vmServers::debug, subValue2 ) );
							} else
							{
								vmConf.servers.push_back ( vmServers ( vmServers::debug, "9772" ) );
							}
						} else
						{
					...
					} else if ( !strccmp ( name, "language_server" ) )
					{
						...
						} else if ( !strccmp ( subValue1, "on" ) )
						{
							...
							if ( *subValue2 )
							{
								vmConf.servers.push_back ( vmServers ( vmServers::language, subValue2 ) );
							} else
							{
								vmConf.servers.push_back ( vmServers ( vmServers::language, "6996" ) );
							}
						} else
						{
							printf ( "error: debugger must be either ON, OFF or ADAPTER\r\n" );
				...
				case CONFIG_SERVER:
					...
					} else if ( !strccmp ( name, "SERVER" ) )
					{
						...
						} else
						{
							...
							{
								vmConf.servers.push_back ( vmServers ( vmServers::http, subValue1 ) );
							} else if ( !strccmp ( value, "https" ) )
							{
								vmConf.servers.push_back ( vmServers ( vmServers::https, subValue1 ) );
							} else if ( !strccmp ( value, "db" ) )
							{
								vmConf.servers.push_back ( vmServers ( vmServers::db, subValue1 ) );
							}
						}
					}
					break;
