...
struct fileCacheDir {
	...
	fileCacheDir ( fileCacheDir&& old )
	{
		*this = std::move ( old );
	}
	fileCacheDir& operator = ( fileCacheDir&& old )
	{
		std::swap ( ov, old.ov );
...
class fileCache {
	...
	std::list<fileCacheEntry *>				 entries;			// file cache handles
	fileCacheEntry							*freeEntries = 0;
	fileCacheEntry							*pendingFree = 0;

	std::list<fileCacheEntry *>				 handles;			// file cache handles
	fileCacheEntry							*freeHandles = 0;
	...
	{
		if ( !heapSize ) heapSize = 1024 * 1024 * 1024; // 1G of cache size by default
