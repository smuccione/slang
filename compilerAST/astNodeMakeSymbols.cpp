...
void astNode::makeSymbols ( errorHandler *err, symbolStack *sym, astNode *addPt, symbolStack *parentSym, opFunction *func, compExecutable *comp, symbolSpaceLocalAdd *localAdd, symbolSpaceClosures *closureAdd, bool isLS, bool isAccess )
{
	...
	switch ( op )
	{
		case astOp::funcValue:
			{
				...
				*this = *astNode( sym->file->sCache, astOp::funcCall ).setLocation( this );
				...
				{
					pList().param.push_back ( new astNode( sym->file->sCache, astOp::nameValue, sym->file->selfValue ) );
					pList ().paramRegion.push_back ( *this );
				}
		...
		case astOp::btInlineRet:
			if ( node() ) node()->makeSymbols( err, sym, addPt, parentSym, func, comp, localAdd, closureAdd, isLS, true );
			break;
		case astOp::btInline:
		...
		case astOp::logicalAnd:
			if ( left->getOp () == astOp::logicalOr || right->getOp () == astOp::logicalOr )
			{
				errorLocality e ( &sym->file->errHandler, this );
				if ( isLS )
				{
					sym->file->warnings.push_back ( std::make_unique<astNode> ( sym->file->sCache, warnNum::scwINCORRECT_LOGIC, sym->file->errHandler.throwWarning ( isLS, warnNum::scwINCORRECT_LOGIC, symbolValue ().c_str () ) ) );
					sym->file->warnings.back ().get ()->setLocation ( this );
				} else
				{
...
void astNode::makeSymbolsCodeBlock( errorHandler *err, symbolStack *sym, astNode *addPt, symbolStack *parentSym, opFunction *func, compExecutable *comp, symbolSpaceLocalAdd *localAdd, symbolSpaceClosures *closureAdd, bool isAccess )
{
	...
	switch ( op )
	{
		...
		case astOp::btIf:
			...
			break;
		case astOp::btInlineRet:
			if ( node() ) node()->makeSymbolsCodeBlock ( err, sym, addPt, parentSym, func, comp, localAdd, closureAdd, true );
			break;
		case astOp::btInline:
