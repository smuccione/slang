...
bool scheduleMakeIterator( class opFile* file, opFunction * func, bool generic, accessorType const& acc, unique_queue<accessorType> *scanQueue, bool isLS )
{
	...
	scanQueue->push( new makeIteratorType { func, acc, generic } );
	return true;
}
...
void markClassInuse ( class compExecutable *comp, accessorType const &acc, class opClass *cls, symbolStack const *sym, unique_queue<accessorType> *scanQueue, bool isLS )
{
	if ( !cls->inUse )
	{
		...
		{
			auto elem = &cls->cClass->elements[cls->cClass->overload[int(fgxOvOp::ovFuncCall)] - 1];
			auto callee = elem->methodAccess.func;
			callee->getMarkReturnType ( comp, sym, acc, scanQueue, isLS );
			callee->setAccessed ( acc, scanQueue );
		}
	...
	switch ( op )
	{
		...
		case astOp::btInlineRet:
			if ( node () )
			{
				...
				symbolTypeClass newType = node ()->typeInfer ( comp, acc, err, sym, inlineRetType, interProcedural, true, markUsed, scanQueue, isLS );
				...
				if ( newType != symbolType::symUnknown )
				{
					switch ( *inlineRetType )
					{
						...
						default:
							...
							{
								*inlineRetType = symWeakVariantType;
							}
							break;
					}
				}
		...
		case astOp::btReturn:
			...
			return symWeakVariantType;
		case astOp::assign:
//			isInFunc ( "test1.descShort.method" )
//				printf ( "" );
				...
				switch ( sym->getScope ( left->symbolValue (), false ) )
				{
					...
					case symbolSpaceScope::symLocalParam:
						...
						if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
						{
							...
							{
								auto elem = &classDef->elements[classDef->overload[int(fgxOvOp::ovAssign)] - 1];
								if ( elem )
								{
									if ( interProcedural )
									{
				...
				if ( leftType.isAnObject () )
				{
					if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
					{
						if ( classDef && left->right->getOp () == astOp::nameValue )
						{
							...
							{
								auto elem = &classDef->elements[classDef->defaultAssignIndex - 1];
								if ( elem->assign.func )
								{
									if ( interProcedural )
									{
...
//												it.elem->setTypeNoThrow ( false, rightType, acc, scanQueue );
			...
			} else if ( left->getOp () == astOp::arrayDeref || left->getOp () == astOp::cArrayDeref )
			{
				...
				if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
				{
					...
					{
						auto elem = &classDef->elements[classDef->overload[int(fgxOvOp::ovArrayDerefLHS)] - 1];
						if ( elem )
						{
							if ( interProcedural )
							{
		...
		case astOp::funcCall:
			if ( left->getOp () == astOp::atomValue )
			{
						...
						if ( (pList ().param[0]->getOp () == astOp::nameValue) )
						{
							...
							if ( cl )
							{
								...
								if ( cl->newIndex )
								{
									...
									auto destFunc = cl->elements[cl->newIndex - 1].methodAccess.func;
				...
				} else if ( left->symbolValue () == sym->file->exceptValue )
				{
					...
					return symWeakVariantType;
				} else if ( left->symbolValue () == sym->file->typeValue )
				{
					// this effects global state since we may catch many functions up (if at all)
					if ( pList ().param.empty () )
					{
				...
				{
					if ( sym->isDefined ( left->symbolValue(), true ) )
					{
					...
					} else
					{
						switch ( sym->getScope ( left->symbolValue (), true ) )
						{
							...
							default:
								...
								{
									errorLocality e ( err, this );
									err->throwFatalError ( isLS, errorNum::scUNKNOWN_IVAR );
									return symWeakVariantType;
								}
								break;
						}
				...
				if ( type.isAnObject () )
				{
					if ( type.hasClass () && (classDef = sym->findClass ( type.getClass () )) )
					{
						if ( classDef && left->right->getOp () == astOp::nameValue )
						{
							...
							{
								elem = &classDef->elements[classDef->defaultAccessIndex - 1];
...
func_call_by_type:
				...
				if ( leftType.hasClass() && (classDef = sym->findClass( leftType.getClass() )) )
				{
					...
					{
						auto elem = &classDef->elements[classDef->overload[int(fgxOvOp::ovFuncCall)] - 1];
						if ( elem )
						{
				...
				if ( leftType.isAnObject () )
				{
					...
					for ( auto &cls : sym->file->classList )
					{
						if ( cls.second->inUse )
						{
							...
							{
								auto elem = &cls.second->cClass->elements[cls.second->cClass->overload[int(fgxOvOp::ovFuncCall)] - 1];
								if ( std::holds_alternative<opFunction *> ( acc ) && !elem->methodAccess.func->isConst ) std::get<opFunction *> ( acc )->isConst = false;
								if ( std::holds_alternative<opFunction *> ( acc ) && !elem->methodAccess.func->isPure ) std::get<opFunction *> ( acc )->isPure = false;
								if ( elem )
								{
									if ( !elem->methodAccess.func->isForcedVariant )
									{
		...
		case astOp::sendMsg:
			...
			} else if ( !isLS && leftType.hasClass() && right->getOp() == astOp::errorValue && sym->findClass( leftType.getClass() ) )
			{
				return symWeakVariantType;
			}
			if ( std::holds_alternative<opFunction *> ( acc ) ) std::get<opFunction *> ( acc )->isConst = false;
			...
			if ( right->getOp () == astOp::nameValue )
			{
				...
				for ( auto &cls : sym->file->classList )
				{
					...
					if ( cls.second->inUse )
					{
						...
						{
							elem = &cls.second->cClass->elements[cls.second->cClass->defaultAccessIndex - 1];
		...
		case astOp::powAssign:
			...
			if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
			{
				...
				{
					auto elem = &classDef->elements[classDef->overload[int(opToOvXref[static_cast<size_t>(this->getOp ())])] - 1];
					if ( elem )
					{
						if ( interProcedural )
						{
		...
		case astOp::preDec:
			if ( left->getOp () == astOp::sendMsg )
			{
					...
					case astOp::preInc:
						{
							...
							astNode assignNode ( sym->file->sCache, astOp::assign, new astNode ( *left ), new astNode ( addNode ) );
					...
					case astOp::preDec:
						{
							...
							astNode assignNode ( sym->file->sCache, astOp::assign, new astNode ( *left ), new astNode ( subNode ) );
							assignNode.right->forceLocation ( srcLocation () );
							if ( isLS ) sym->file->statements.push_back ( std::make_unique<astNode> ( astLSInfo::semanticSymbolType::keywordOperator, *this ) );
							release ();
							*this = assignNode;
				...
				this->typeInfer ( comp, acc, err, sym, inlineRetType, interProcedural, needValue, markUsed, scanQueue, isLS );
			} else
			{
				leftType = left->typeInfer ( comp, acc, err, sym, inlineRetType, interProcedural, needValue, markUsed, scanQueue, isLS );
				if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
...
void astNode::markInUse ( class compExecutable *comp, accessorType const &acc, errorHandler *err, symbolStack *sym, bool interProcedural, unique_queue<accessorType> *scanQueue, bool isLS )
{
	...
	switch ( op )
	{
		...
		case astOp::btInlineRet:
			if ( node () )
			{
				node ()->markInUse ( comp, acc, err, sym, interProcedural, scanQueue, isLS );
			}
			break;
		case astOp::btInline:
			sym->file->functionList.find( inlineFunc ().funcName )->second->setInlineUsed ( acc, scanQueue );
