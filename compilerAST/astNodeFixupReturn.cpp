...
static astNode *fixupReturnCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, uint64_t value, opFunction *func )
{
	astNode	*newGoto;
	astNode	*newValue;
		...
		case astOp::btReturn:
			// before we do anything else, we need to modify the label
			{
				if ( func->isProcedure )
				{
					...
					newGoto->gotoData().label = node->returnData().label;
					newValue = nullptr;
				} else
				{
					...
					newGoto = (new astNode ( sym->file->sCache, astOp::btSlide ))->setLocation ( node );
					newGoto->gotoData().nPop = node->returnData().nPop;
					newGoto->gotoData().finallyBlock = 0;
					newGoto->gotoData().label = node->returnData().label;
					...
					{
						newValue = (new astNode ( sym->file->sCache, astOp::btInlineRet ))->setLocation ( node );
						newValue->node() = node->returnData().node;
					} else
					{
						newValue = (new astNode ( sym->file->sCache, astOp::btInlineRet ))->setLocation ( node );
						newValue->node() = nullptr;
					}
				}
				//	emit Finally
				//	emit value
