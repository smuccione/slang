...
static astNode *compileCodeBlockCB ( astNode *node, astNode *parent, symbolStack *parentSym, bool isAccess, class errorHandler *err )
{
	switch ( node->getOp() )
	{
		case astOp::codeBlockExpression:
			{
				...
				while ( *src && (*src != '|') )
				{
					cbNode = parentSym->file->getNode ( src, opFile::lastOpType::binary, &oFunc, true, false );
					if ( cbNode->getOp() != astOp::symbolValue )
					{
						delete cbNode;
						throw errorNum::scILLEGAL_IDENTIFIER;
					}

					symbols->add ( cbNode->symbolValue(), symVariantType, srcLocation (), nullptr, false );
				...
				nParams = (uint32_t) symbols->size ( );

				cbNode = parentSym->file->_parseExpr ( src, true, false, false, false, false, 0, false, false );
