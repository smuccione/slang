...
struct opStack
{
	using BoostNodeAllocator = boost::fast_pool_allocator<astNode *>;
	using BoostNodeAllocatorPool = boost::singleton_pool<astNode *, sizeof( astNode * )>;

	std::deque<astNode *, BoostNodeAllocator >	elem;
...
class astNode final {
	...
	using semanticData = std::pair<astLSInfo::semanticSymbolType, astLSInfo::semanticLineBreakType>;
...
public:
	...
	{		
		return std::get<astLSInfo::semanticSymbolType> ( std::get<semanticData> ( nodeData ) );
	}
	...
	{
		return std::get<astLSInfo::semanticLineBreakType> ( std::get<semanticData> ( nodeData ) );
	}
	...
	astNode ( stringCache &sCache, astOp oper, char const *format, ... )
	{
		va_list		marker;
	...
	astNode ( stringCache &sCache, warnNum num, char const *format, ... )
	{
		va_list		marker;
	...
	void					makeAtoms				( symbolStack *sym, class errorHandler *err, bool isLS );
