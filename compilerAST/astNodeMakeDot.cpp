...
astNode *makeDotNodesCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, BUFFER &buff )
{
	uint32_t		arrIndex;
	...
	switch ( node->getOp() )
	{
		...
		case astOp::btInlineRet:
			emitNode ( node, buff );
			bufferPrintf ( &buff, "|<f%x> %x", node->node(), node->node() );
			break;
		case astOp::btInline:
			emitNode ( node, buff );
...
astNode *makeDotGraphCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, BUFFER &buff )
{
	switch ( node->getOp() )
	{
		case astOp::symbolDef:
