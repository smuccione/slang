...
void astNode::freeType () noexcept
{
	switch ( op )
	{
		...
		case astOp::btInlineRet:
			{
				auto tmp = std::get_if<astNode *> ( &nodeData );
				if ( tmp )
				{
					delete *tmp;
					*tmp = nullptr;
...
astNode::astNode ( class opFile *file, class sourceFile *srcFile, char const **expr )
{
	...
	switch ( op )
	{
		...
		case astOp::btLabel:
			...
			break;
		case astOp::btInlineRet:
			nodeData = OP_NODE_DESERIALIZE ( file, srcFile, expr );
			break;
...
astNode::astNode ( const astNode &old )
{
	...
	switch ( old.op )
	{
		...
		case astOp::btInlineRet:
			if ( old.node() )
			{
				node() = new (std::nothrow) astNode ( *old.node() );
			} else
			{
				node() = nullptr;
			}
...
static astNode *forceLocationCB ( astNode *node, astNode *parent, class symbolStack *sym, bool isAccess, accessorType const &acc, srcLocation const &loc )
{
	node->location = loc;
	return node;
}
