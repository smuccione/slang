...
static void cRequestWritePostData ( vmInstance *instance, VAR *obj, char const *fName, size_t part )
{
	...
	if ( part < conn->req.multiParts.size ( ) )
	{
		...
		while ( len )
		{
			DWORD writeSize = len < 1024 * 1024 ? (DWORD) len : 1024 * 1024;
			DWORD nWritten;
...

{
	...
	return (uint32_t) conn->req.multiParts.size ( );
}
...
void cResponseSetCookie ( class vmInstance *instance, VAR *obj, VAR_STR *name, VAR_STR *data, VAR_STR *path, VAR_STR *expires, uint32_t secure, uint32_t httpOnly )
{
	...
	{
		outPtr[0] = (uint8_t)(int)webBase64EncodingTable[(in[0] >> 2)];
		outPtr[1] = (uint8_t)(int)webBase64EncodingTable[((in[0] & 0x03) << 4) | (in[1] >> 4)];
		outPtr[2] = (uint8_t)(int)webBase64EncodingTable[((in[1] & 0x0F) << 2) | (in[2] >> 6)];
		outPtr[3] = (uint8_t)(int)webBase64EncodingTable[((in[2] & 0x3F))];
...
{
	instance->debug->debugRegisterInspector ( "Web Server", sessionTab );
