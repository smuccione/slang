...
int webServerSSLPasswordCB (char *buf, int size, int rwflag, void *userdata)
{
	...
	return  (uint32_t)server->sslPassword.length();
}
...
webProtocolSSL::protoType http2_select_next_protocol( unsigned char **out, unsigned char *outlen, const unsigned char *in, unsigned int inlen )
{
#if 0
	if ( select_next_protocol( out, outlen, in, inlen, HTTP2_PROTO_ALPN, HTTP2_PROTO_ALPN_LEN ) == 0 )
	{
		return webProtocolSSL::protoHttp2;
	}
#endif
	if ( select_next_protocol( out, outlen, in, inlen, HTTP2_HTTP_1_1_ALPN, HTTP2_HTTP_1_1_ALPN_LEN ) == 0 )
	{
		return webProtocolSSL::protoHttp1;
	}
	return  webProtocolSSL::protoError;
}
...
static int alpn_select_proto_cb( SSL *ssl, const unsigned char **out, unsigned char *outlen, const unsigned char *in, unsigned int inlen, void *arg )
{
	...
	if ( sock->protocol != webProtocolSSL::protoNone ) return SSL_TLSEXT_ERR_OK;

	sock->protocol = http2_select_next_protocol ( (unsigned char **) out, outlen, in, inlen );

	switch ( sock->protocol )
	{
		case webProtocolSSL::protoHttp1:
		case webProtocolSSL::protoHttp2:
			return SSL_TLSEXT_ERR_OK;
		default:
			return SSL_TLSEXT_ERR_NOACK;
	}
	...
	for ( ;; )
	{
		switch ( protoChild->getIoOp() )
		{
			...
			case needRecv:
				...
				len = SSL_read ( ssl, sslBuff.getFreeBuff(), (uint32_t)inProcessBuffs->totalIo );
				...
				} else 
				{
					...
					if ( protoChild ) protoChild->processBuffer ( instance, (size_t) len );
...
void webProtocolSSL::Recv	( class vmInstance *instance, int32_t len )
{
	...
	BIO_write ( pBioRead, sslBuff.getBuffer(), (uint32_t)len );
}
...
bool webServerSSLInit ( webServerTaskControl *ctrl, char const *configFile, vmInstance *instance )
{
	...
	// this is a "pascal" string ( lenght, data ) with a 1 char length byte followed by data
	next_proto_list[0] = HTTP2_PROTO_VERSION_ID_LEN;
	memcpy( &next_proto_list[1], HTTP2_PROTO_VERSION_ID, HTTP2_PROTO_VERSION_ID_LEN );  // NOLINT (bugprone-not-null-terminated-result)
	next_proto_list_len = 1 + HTTP2_PROTO_VERSION_ID_LEN;
