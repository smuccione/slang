...
struct webServerRequestCookie {
	...
	int operator == ( char const *s )
	{
		if ( !s[nameLen] )
		{
			return !memcmpi ( (*basePtr + nameOffset), s, nameLen );
		}
		return 0;
	}
	...
	void getValueRaw ( uint8_t *out )
	{
		memcpy ( out, *basePtr + valueOffset, valueLen );
	}

	size_t getValueLenRaw ( )
	{
		return valueLen;
	}

	void getValue ( uint8_t *out )
	{
		size_t		 loop;
		size_t		 decodedLen;
		uint8_t		*in;
...
public:
	...
	struct apProjectDirEntry
	{
		...
		apProjectDirEntry &operator = ( apProjectDirEntry &&old )
		{
			std::swap ( ov, old.ov );
	...
	webVirtualServer ( webVirtualServer const &old )
	{
		InitializeSRWLock ( &lock );
		InitializeSRWLock ( &apProjectDirLock );
	...
	void monitor ( char const *file )
	{
		...
		auto dir = std::filesystem::path ( f ).remove_filename ().string ();
		{
