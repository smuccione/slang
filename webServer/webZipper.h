...
//		printf ( "%s\r\n", stateNames[state] );
		switch ( state )
		{
			...
			case SendingBodyEnd:
				...
				new (&dataDescriptor) zipDataDescriptor ( header.crc32, header.cFileSize, header.fileSize );
			...
			case SendingCentralDirectory:
				...
				{
					new (&zip64EOCDRecord) zip64EndOfCentralDirectoryRecord ( fHeaders.size ( ), streamOffset - streamCDOffset, streamCDOffset );
				...
				new (&centralDirectory) zipCentralDirectory ( fHeaders[index] );
				txBytes = (uint32_t) centralDirectory;
				txOffset = 0;
				index++;
				break;
			case SendingZip64EndCentralDirectoryRecord:
				new (&zip64EOCDLocator) zip64EndOfCentralDirectoryLocator ( streamZip64CDOffset );
				txBytes = (uint32_t) zip64EOCDLocator;
				txOffset = 0;
				state = SendingZip64EndCentralDirrectoryLocator;
				break;
			case SendingZip64EndCentralDirrectoryLocator:
				new (&EOCD) EndOfCentralDirectory ();
