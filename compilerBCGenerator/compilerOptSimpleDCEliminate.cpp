...
void optimizer::makeLabelList ( astNode *block, std::unordered_map<cacheString, std::pair<astNode *, bool> > &labels )
{
	...
	switch ( block->getOp () )
	{
		...
		case astOp::btInlineRet:
			makeLabelList ( block->node(), labels );
			break;
		case astOp::btInline:
	...
	switch ( block->getOp () )
	{
		...
		case astOp::btInline:
			...
			if ( block->inlineFunc().node->getOp () == astOp::btBasic &&
					...
					block->inlineFunc().node->basicData().blocks[1]->basicData().blocks[2]->getOp () == astOp::btSlide && !block->inlineFunc().node->basicData().blocks[1]->basicData().blocks[2]->gotoData().nPop
				)
			{
				auto ret = block->inlineFunc().node->basicData().blocks[1]->basicData().blocks[1]->node();
		...
		case astOp::btInlineRet:
			assert ( !block->left );
			assert ( !block->right );
			block->node() = simpleDCEliminateBlock ( block->node(), labels, sym );
			if ( !block->node() )
			{
				delete block;
				return nullptr;
