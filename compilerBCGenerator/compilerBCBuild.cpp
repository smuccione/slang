...
compExecutable::compExecutable ( opFile *file ) : codeSegment ( *this ), listing ( &codeSegment, &dataSegment, &atom, this )
{
	genDebug = false;
	genProfiling = false;
...
compExecutable::compExecutable () : codeSegment ( *this ), listing ( &codeSegment, &dataSegment, &atom, this )
{
	genDebug = false;
	genProfiling = false;
...
void compExecutable::compileGlobalInitializers ( bool isSLL )
{
	...
	for ( auto &it : file->symbols )
	{
		...
		if ( !it.second.accessors.empty() || (!it.second.loadTimeInitializable && !it.second.isInterface) )
		{
	...
	if ( globals.size () )
	{
		...
		symbolStack	 sym ( file, &paramFunc );
...
void compExecutable::genCode( char const* entry )
{
	...
	optimizer ( file, this ).go ( file->sCache.get ( entry ? entry : "<NO MAIN>" ) );
...
#if 1
	auto funcIt = file->functionList.find ( file->sCache.get ( "cSkillAssignment.dataTemplate.method" ) );
	...
	for ( auto it = file->functionList.begin(); it != file->functionList.end(); it++ )
	{
		if ( it->second->inUse && !it->second->isInterface )
		{
			try	{
...
void compExecutable::serialize ( BUFFER *buff, bool isLibrary )
{
	...
	for ( auto &it : libraries )
	{
		...
		strcpy_s ( lib.name, sizeof ( lib.name ), p.string ().c_str () );
