...
static void getElemInfo ( opFile *file, compClassElementSearch *elem, astNode *node, accessorType const &acc, bool isAccess, astLSInfo &info, bool refOnly )
{
	switch ( elem->type )
	{
		...
		case fgxClassElementType::fgxClassType_const:
			if ( !refOnly && file->isInRegion ( *node ) ) info.semanticTokens.insert ( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::ivar, node->right->location, elem->elem, isAccess } );
			info.symbolReferences.insert ( astLSInfo::symbolReference { node->right->location, elem->elem->name, node, elem->elem } );
			break;
...
static astNode *getInfoCB( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, accessorType const &acc, astLSInfo &info, bool refOnly )
{
	try
	{
		switch ( node->getOp() )
		{
							...
							if ( !node->pList().param.empty() )
							{
								if ( node->pList().param[0]->getOp() == astOp::nameValue )
								{
									...
									if ( cls )
									{
										...
										info.symbolReferences.insert( astLSInfo::symbolReference{ node->location, node->pList().param[0]->nameValue(), node->pList().param[0], cls->oClass } );
										if ( elem && isInRegion( sym->file, node->pList().param[0] ) )
										{
											info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::type, node->pList().param[0]->location, cls->oClass, isAccess } );
										}
									}
								}
							}
			...
			case astOp::atomValue:
				{
					...
					if ( !refOnly && isInRegion( sym->file, node ) )
					{
						if ( !func->classDef )
						{
							if ( func->isIterator )
							{
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::iterator, node->location, func, isAccess } );
							} else
							{
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::function, node->location, func, isAccess } );
							}
						} else
						{
							if ( func->isIterator )
							{
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::methodIterator, node->location, func, isAccess } );
							} else
							{
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::method, node->location, func, isAccess } );
							}
						}
					}
					...
					if ( type.isAnObject() )
					{
						...
						if ( type.hasClass() && (cls = sym->findClass( type.getClass() )) )
						{
							...
							if ( !elem )
							{
								...
								{
									elem = &cls->elements[cls->defaultMethodIndex - 1];
								}
							}
							if ( elem )
							{
						...
						{
							if ( !refOnly && isInRegion( sym->file, node->right ) ) info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::method, node->right->location, node->right, isAccess } );
						}
					} else if ( !refOnly && isInRegion( sym->file, node->right ) )
					{
						info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::ivar, node->right->location, node->right, isAccess } );
					}
					...
					if ( type.isAnObject() )
					{
						...
						if ( type.hasClass() && (cls = sym->findClass( type.getClass() )) )
						{
							...
							if ( !elem )
							{
								...
								{
									elem = &cls->elements[cls->defaultAssignIndex - 1];
								}
							}
							if ( elem )
							{
						...
						{
							if ( !refOnly && isInRegion( sym->file, node->right ) ) info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::ivar, node->right->location, node->right, isAccess } );
						}
					} else if ( !refOnly && isInRegion( sym->file, node->right ) )
					{
						info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::ivar, node->right->location, node->right, isAccess } );
					...
					if ( type.isAnObject() )
					{
						...
						if ( type.hasClass() && (cls = sym->findClass( type.getClass() )) )
						{
							...
							if ( !elem )
							{
								...
								{
									elem = &cls->elements[cls->defaultAccessIndex - 1];
								}
							}
							if ( elem )
							{
				...
				switch ( node->symbolDef()->symType )
				{
					...
					case symbolSpaceType::symTypeLocal:
						...
						if ( !refOnly && isInRegion( sym->file, node ) )
						{
							...
							{
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::variable, init->left->location, node->symbolDef()->getType( node->symbolDef()->getSymbolName(), isAccess ), isAccess } );
							...
							{
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::variable, init->location, node->symbolDef()->getType( node->symbolDef()->getSymbolName(), isAccess ), isAccess } );
					...
					if ( !refOnly && isInRegion( sym->file, node ) )
					{
						switch ( sym->getScope( node->symbolValue(), true ) )
						{
							...
							case symbolSpaceScope::symClassStatic:
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::ivar, node->location, sym->getSymbolSource( node->symbolValue() ), isAccess } );
								break;
							...
							case symbolSpaceScope::symClassAccess:
								{
									...
									info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::property, node->location, func, isAccess } );
								}
								break;
							...
							case symbolSpaceScope::symLocalField:
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::field, node->location, node, isAccess } );
								break;
							case symbolSpaceScope::symLocalParam:
								if ( node->symbolValue() != sym->file->outputStringValue )
								{
									info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::parameter, node->location, sym->getSymbolSource( node->symbolValue() ), isAccess } );
								}
								break;
							...
							case symbolSpaceScope::symLocalStatic:
								assert( node->location.sourceIndex );
								info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::variable, node->location, sym->getSymbolSource( node->symbolValue() ), isAccess } );
								break;
						}
					}
				}
			...
			case astOp::stringValue:
				...
				{
					if ( !refOnly && isInRegion( sym->file, node ) ) info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::name, node->location, node, isAccess } );
				} else
				{
					if ( !refOnly && isInRegion( sym->file, node ) ) info.semanticTokens.insert( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::stringLiteral, node->location, node, isAccess } );
				}
				break;
			...
			if ( it.second.symClass == opSymbol::symbolClass::symbolClass_global || it.second.symClass == opSymbol::symbolClass::symbolClass_globalConst )
			{
				if ( it.second.location.sourceIndex == sourceIndex && file->isInRegion ( it.second.location ) )
				{
					...
					if ( it.second.initializer->getOp () == astOp::assign )
					{
						...
						info.semanticTokens.insert ( astLSInfo::symbolRange{ accessorType (), astLSInfo::semanticSymbolType::variable, it.second.initializer->left->location, it.second.getType (), true } );
					...
					{
						info.semanticTokens.insert ( astLSInfo::symbolRange{ accessorType (), astLSInfo::semanticSymbolType::variable, it.second.location,  it.second.getType (), true } );
					}
				}
				...
				switch ( elem->type )
				{
					case fgxClassElementType::fgxClassType_method:
						{
							...
							if ( func->nameLocation.sourceIndex == sourceIndex )
							{
								...
								info.semanticTokens.insert ( astLSInfo::symbolRange{ elem, astLSInfo::semanticSymbolType::method, func->nameLocation, func, true } );
					...
					case fgxClassElementType::fgxClassType_prop:
						if ( elem->data.prop.access )
						{
							...
							info.semanticTokens.insert ( astLSInfo::symbolRange{ elem, astLSInfo::semanticSymbolType::property, func->nameLocation, func, true } );
							info.symbolReferences.insert ( astLSInfo::symbolReference { func->nameLocation, file->sCache.get ( func->getSimpleName () ), nullptr, elem } );
						}
					...
					case fgxClassElementType::fgxClassType_const:
						if ( file->isInRegion ( elem->location ) )
						{
							...
							if ( init->getOp () == astOp::assign )
							{
								...
								info.semanticTokens.insert ( astLSInfo::symbolRange{ elem, astLSInfo::semanticSymbolType::ivar, init->left->location, init->left, true } );
							...
							{
								info.semanticTokens.insert ( astLSInfo::symbolRange{ elem, astLSInfo::semanticSymbolType::ivar, init->location, elem, true } );
							}
						}
					...
					case fgxClassElementType::fgxClassType_inherit:
						info.semanticTokens.insert ( astLSInfo::symbolRange{ elem, astLSInfo::semanticSymbolType::customLiteral, elem->location, elem->symType } );
						break;
					case fgxClassElementType::fgxClassType_message:
						info.semanticTokens.insert ( astLSInfo::symbolRange{ elem, astLSInfo::semanticSymbolType::customLiteral, elem->location, elem->symType, true } );
						info.symbolReferences.insert ( astLSInfo::symbolReference { elem->location, elem->name, nullptr, elem } );
						break;
				}
			}
		...
		auto *func = it.second;
		{
			...
			if ( func->location.sourceIndex == sourceIndex )
			{
				if ( func->params.size () )
				{
					...
					for ( size_t it = func->params.size (); it; it-- )
					{
						auto init = func->params[(uint32_t)it - 1]->initializer;
						if ( init )
						{
							if ( init->getOp () == astOp::assign )
							{
								...
								info.semanticTokens.insert ( astLSInfo::symbolRange{ func, astLSInfo::semanticSymbolType::parameter, *init->left, func->params[(uint32_t)it - 1]->type, true } );
							} else
							{
				...
				if ( func->nameLocation.sourceIndex == sourceIndex && !func->classDef )
				{
					...
					info.semanticTokens.insert ( astLSInfo::symbolRange{ func, astLSInfo::semanticSymbolType::function, func->nameLocation, func, true } );
				}
			} else
			{
			...
			for ( uint32_t it = 0; it < symbol->size (); it++ )
			{
				...
				if ( s->initializer )
				{
					if ( s->initializer->getOp () == astOp::assign )
					{
						...
						if ( s->location.sourceIndex == sourceIndex && file->isInRegion ( s->location ) )
						{
							info.semanticTokens.insert ( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::parameter, *s->initializer->left, s->getType (), true } );
						}
					...
					} else
					{
						...
						if ( s->location.sourceIndex == sourceIndex && file->isInRegion ( s->location ) )
						{
							info.semanticTokens.insert ( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::parameter, *s->initializer, s->getType (), true } );
						}
					}
				} else
				{
					...
					if ( s->location.sourceIndex == sourceIndex && file->isInRegion ( s->location ) )
					{
						info.semanticTokens.insert ( astLSInfo::symbolRange{ acc, astLSInfo::semanticSymbolType::parameter, s->location, s->getType (), true } );
					}
				}
			}
		}
