...
static astNode *hasNVReturnCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, bool &hasNVReturn )
{
	switch ( node->getOp() )
	{
		case astOp::btReturn:
...
static void compMakeIterator ( compExecutable *comp, opFunction *func, opClass *oClass, astNode *node, uint64_t *nReturn, std::unordered_map<cacheString, localCnt> *symbols, symbolStack *sym, bool isAssign, bool processReturn, bool doesReturnIndex, std::vector<symbol *> &cleanupSymbols )
{
	...
	switch ( node->getOp() )
	{
					...
					if ( sym->isDefined ( node->symbolValue(), true ) )
					{
						switch ( sym->getScope ( node->symbolValue(), isAssign )  )
						{
							...
							case symbolSpaceScope::symClassStatic:
								node->symbolValue () = comp->file->sCache.get ( stringi ( "::" ) + (stringi)func->classDef->name + "::" + (stringi) node->symbolValue () );
								break;
		...
		case astOp::btInlineRet:
			compMakeIterator ( comp, func, oClass, node->node(), nReturn, symbols, sym, false, processReturn, doesReturnIndex, cleanupSymbols );
			break;
		case astOp::btInline:
...
bool compExecutable::compMakeIterator ( opFile *file, opFunction *func, bool generic, accessorType const &acc, unique_queue<accessorType> *scanQueue, bool isLS  )
{
	...
	char tmp[1024] = {};
	expr = tmp;
	sprintf ( tmp, "(){\r\nexcept ( new systemError ( %u ) ); \r\nreturn\r\n}", errorNum::scRESET_NOT_APPLICABLE );
	{
		source src ( &file->srcFiles, file->sCache, "(INTERNAL)", expr, func->location.lineNumberStart );
		newFunc = file->parseMethod ( src, itClass, buildString ( itClass->name.c_str ( ), elem->name, "method" ).c_str ( ), true, false );
	}
	...
	{
		source src ( &file->srcFiles, file->sCache, "(INTERNAL)", expr, func->location.lineNumberStart );
		newFunc = file->parseMethod ( src, itClass, buildString ( itClass->name.c_str ( ), elem->name, "access" ).c_str ( ), true, false );
	}
	file->functionList.insert( { newFunc->name, newFunc } );
	...
	{
		source src ( &file->srcFiles, file->sCache, "(INTERNAL)", expr, func->location.lineNumberStart );
		func->codeBlock = file->parseBlock ( src, func, false, true, isLS );
	}
