	...
	switch  ( node->left->getOp() )
	{
		case astOp::symbolPack:
			...
			} else
			{
				...
				if ( needValue )
				{
					codeSegment.putOp ( fglOpcodes::storeMulti, (uint32_t)node->left->pList().param.size () );
					sym->pushStack ( node->left->pList().param.size () );
				} else
				{
					sym->popStack ();
					codeSegment.putOp ( fglOpcodes::storeMultiPop, (uint32_t)node->left->pList().param.size () );
					sym->pushStack ( node->left->pList().param.size () );
				}
			}
...
int32_t compExecutable::compEmitFuncParameters ( opFunction *funcDef, cacheString const &name, bool isAccess, class astNode *node, class symbolStack *sym, symbolTypeClass const &needType, symbolTypeClass const &retType, int32_t nParamsNeeded, int32_t nParamsSent, int32_t numHidden, cacheString const &funcName, bool safeCalls )
{
	...
	if ( nParamsSent >= nParamsNeeded - numHidden )
	{
		...
		{
			if ( node->pList().param[nodeIndex - 1] )
			{
				// push the callers value
				if ( sym->isVariantParam ( funcName, isAccess ) && (nParamsSent + numHidden - 1 >= nodeIndex) )
				{
					pType = compEmitNode ( funcDef, node->pList().param[nodeIndex - 1], sym, symVariantType, retType, safeCalls,  true ); 
				} else
				{
					pType = compEmitNode ( funcDef, node->pList().param[nodeIndex - 1], sym, sym->getFuncParamType ( funcName, isAccess, nodeIndex - 1 + numHidden, accessorType(), nullptr ), retType, safeCalls,  true );
					compEmitCast ( pType, sym->getFuncParamType ( funcName, isAccess, nodeIndex - 1 + numHidden, accessorType(), nullptr ) );
				}
			} else
			{
	...
	} else 
	{
		...
		for ( nodeIndex = (int32_t)node->pList().param.size(); nodeIndex; nodeIndex-- )
		{
			...
			{
				// push the callers value
				pType = compEmitNode ( funcDef, node->pList().param[nodeIndex - 1], sym, sym->getFuncParamType ( funcName, isAccess, nodeIndex - 1 + numHidden, accessorType(), nullptr ).compType(), retType, safeCalls,  true );
				compEmitCast ( pType, sym->getFuncParamType ( funcName, isAccess, nodeIndex - 1 + numHidden, accessorType(), nullptr ).compType() );
			} else
			{
				newNode = sym->getFuncDefaultParam ( funcName, isAccess, nodeIndex - 1 + numHidden );
				...
				switch ( node->pList().param[0]->getOp() )
				{
					case astOp::nameValue:
						{
							...
							if ( cls )
							{
								if ( cls->newIndex )
								{
									...
									auto dstFunc = cls->elements[cls->newIndex - 1].methodAccess.func;
					...
					default:
						...
						for ( nodeIndex = (int32_t)node->pList().param.size(); nodeIndex - 1; nodeIndex-- )
						{
							...
							{
								// push the callers value
								compEmitNode( funcDef, node->pList().param[nodeIndex - 1], sym, symVariantType, retType, safeCalls,  true );
							} else
							{
			...
			} else
			{
				...
				if ( ((int) nParamsNeeded >= 0) )
				{
					...
					if ( !needValue && sym->isConst ( node->left->symbolValue(), true ) )
					{
						...
						{
							if ( node->pList().param[nodeIndex - 1] && (node->pList().param[nodeIndex - 1]->getOp() == astOp::refCreate) )
							{
								break;
							}
						}
						...
						if ( !nodeIndex )
						{
							for ( nodeIndex = (int) node->pList().param.size (); nodeIndex; nodeIndex-- )
							{
								if ( node->pList().param[nodeIndex - 1] )
								{
									...
									compEmitNode ( funcDef, node->pList().param[nodeIndex - 1], sym, symVariantType, retType, safeCalls,  false );
								}
							}
							ret = symVariantType;
							break;
				...
				if ( elem )
				{
					...
					switch ( elem->type )
					{
						case fgxClassElementType::fgxClassType_method:
							...
							break;
						case fgxClassElementType::fgxClassType_prop:
							if ( !elem->methodAccess.func ) throw errorNum::scINVALID_ASSIGN;
						case fgxClassElementType::fgxClassType_iVar:
						case fgxClassElementType::fgxClassType_const:
							// if we reached here then we have no choice but to do a slow dynamic dispatch call
			...
			switch ( node->left->getType ( sym ).compType() )
			{
				...
				case symbolType::symString:
					...
					for ( nodeIndex = (int32_t)node->pList().param.size(); nodeIndex; nodeIndex-- )
					{
						...
						{
							// push the callers value
							compEmitNode ( funcDef, node->pList().param[nodeIndex - 1], sym, symVariantType, symVariantType, safeCalls, true );
						} else
						{
							// try to find the def value code ( this is a soft link so we don't have it now)
							codeSegment.putOp( fglOpcodes::callDefValue, (uint32_t)nodeIndex );
	...
	switch ( block->getOp() )
	{
		...
		*/
					...
					case symbolSpaceType::symTypeParam:
						{
							...
							for ( size_t it = symbol->size ( ); it; it-- )
							{
								...
								{
									debug.addEntry ( init->location, codeSegment.getNumOps ( ), sym->getStackSize ( ) );
									compEmitNode ( funcDef, init->right, sym, (*symbol)[(uint32_t) it - 1]->getType ( ), symVariantType, safeCalls, true );
								} else
								{
									compEmitDefaultValue ( funcDef, sym, (*symbol)[(uint32_t) it - 1]->getType ( ) );
									sym->pushStack ();
								}
							}
		...
		case astOp::btInlineRet:
			...
			if ( block->node() )
			{
				resType = compEmitNode ( funcDef, block->node(), sym, retType, retType, safeCalls, true );
				// NOTE:  This is a bit f'ed.  we actually DO have a value on the stack.   but in ALL cases, an inline ret will be followed by a transition to the exit
				//		  of the inlined function.  we *MUST* not allow the value to exist on the symbol stack.  if it does it will cause an incorrect eval stack pop when
				//		  the blocks exit.   This doest occur in non-inlined functions as we use result as the return mechanism.  inline functions use the stack so we need
		...
		case astOp::btInline:
			...
			if ( file->functionList.find ( block->inlineFunc().funcName )->second->isProcedure )
			{
				if ( needValue )
				{
					compEmitNode ( funcDef, block->inlineFunc().node, sym, needType, needType, safeCalls, true );
					codeSegment.putOp ( fglOpcodes::pushNull );
					sym->pushStack ();
				} else
				{
					// no need to pop anything... procedures don't return values.
					compEmitNode ( funcDef, block->inlineFunc().node, sym, needType, needType, safeCalls, true );
				}
			} else
			{
		...
		case astOp::btSlide:
			...
			{
				codeSegment.putOp ( fglOpcodes::popadup, block->gotoData().nPop );
			}
			listing.emitOps ( sym );
					...
					} else
					{
						if ( block->loopData().condition )
						{
							...
							} else
							{
								block->loopData().condition->fixupReturn ( sym, funcDef );
								// emit the condition
								condType = compEmitNode ( funcDef, block->loopData().condition, sym, symIntType, retType, safeCalls,  true );
								if ( genDebug ) codeSegment.putOp ( fglOpcodes::debugCheckHalt );
								switch ( condType.compType () )
								{
									case symbolType::symInt:
