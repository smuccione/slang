...
static astNode *genErrorsWarningsCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, opFunction *func, bool isLS )
{
	switch ( node->getOp() )
	{
		case astOp::btBasic:
					...
					switch ( it->getOp () )
					{
						...
						default:
							if ( !doEmit )
							{
								...
								unreachableEmitted = true;
							} else if ( it && !it->hasNoSideEffects ( sym, false ) )
							{
		...
		case astOp::symbolValue:
			...
			break;
		case astOp::assign:
			if ( node->left->getOp () == astOp::symbolDef )
			{
				if ( node->symbolDef()->getSymbolName ()[0] != '$' )	// don't throw warnings for compiler generated temps
				{
					if ( !func->isInterface )
					{
						if ( node->symbolDef()->isAssigned ( node->symbolDef()->getSymbolName (), true ) )
						{
							if ( !node->symbolDef()->isAccessed ( node->symbolDef()->getSymbolName (), true ) )
							{
								errorLocality e ( &sym->file->errHandler, node );
								if ( isLS )
								{
									sym->file->warnings.push_back ( std::make_unique<astNode> ( sym->file->sCache, warnNum::scwVARIABLE_ASSIGNED_BUT_NEVER_USED_HERE, sym->file->errHandler.throwWarning ( isLS, warnNum::scwVARIABLE_ASSIGNED_BUT_NEVER_USED_HERE, node->left->symbolValue().c_str () ) ) );
									sym->file->warnings.back ().get ()->setLocation ( node );
								} else
								{
									sym->file->errHandler.throwWarning ( isLS, warnNum::scwVARIABLE_ASSIGNED_BUT_NEVER_USED_HERE, node->left->symbolValue ().c_str () );
								}
							}
						}
					}
				}
				symbolClass leftType;
						...
						if ( leftType.isAnObject () )
						{
							if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
							{
								if ( classDef && node->left->right->getOp () == astOp::nameValue )
								{
									...
									auto elem = classDef->getElement ( node->left->right->nameValue () );
									if ( elem )
									{
							...
							default:
								{
									if ( isLS )
									{
										sym->file->warnings.push_back ( std::make_unique<astNode> ( sym->file->sCache, warnNum::scwUNKNOWN_FUNCTION, sym->file->errHandler.throwWarning ( isLS, warnNum::scwUNKNOWN_FUNCTION ) ) );
			...
			if ( node->left )
			{
				...
				if ( leftType.hasClass () && (classDef = sym->findClass ( leftType.getClass () )) )
				{
					if ( classDef )
					{
						...
						{
							compClassElementSearch *elem = classDef->getElement ( node->right->nameValue () );
					...
					{
						errorLocality e ( &sym->file->errHandler, node );
						if ( !isLS ) sym->file->errors.push_back ( std::make_unique<astNode> ( errorNum::scINTERNAL, *node ) );
						sym->file->errHandler.throwFatalError ( isLS, errorNum::scINTERNAL );
					}
				}
			}
			break;
...
{
	if ( !func->inUse || func->isIterator ) return;
