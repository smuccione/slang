...
struct valCmpKey {
	...
	valCmpKey ( cacheString &name, compClassElementSearch *elem ) : name ( name ), elem ( elem )
	{}
	valCmpKey ( cacheString& name, compClassElementSearch* elem, char const *func ) : name ( name ), func ( func ), elem ( elem )
	{}
	virtual ~valCmpKey ( ) {};
};
...
#if 1
	if ( searchCache != elemMap.end () )
	{
		if ( searchCache->first == name )
		{
			return &elements[searchCache->second];
		}
	}
#endif
		...
		switch ( elem->type )
		{
							...
							for ( auto it = elements.begin (); it != elements.end (); it++ )
							{
								if ( (it->name == name) && it->ns == ns )
								{
									...
									} else
									{
										...
										if ( !virtClass )
										{
											vTable.push_back ( compClassVTable ( atomMethodAccess, delta ) );
										}
									}
			...
			case fgxClassElementType::fgxClassType_prop:
				...
				if ( elem->isVirtual || virtClass )
				{
					...
					if ( virtClass )
					{
						...
						{
							virtFixup.push_back ( std::make_tuple ( &oClass, ( uint32_t) vTable.size ( ) ) );
							vTable.push_back ( compClassVTable ( atomMethodAccess, deltaAccess ) );
						}
						if ( atomAssign != -1 )
						{
							virtFixup.push_back ( std::make_tuple ( &oClass, ( uint32_t) vTable.size ( ) ) );
							vTable.push_back ( compClassVTable ( atomAssign, deltaAssign ) );
						}
					}
					if ( elem->isStatic )
					{
			...
			case fgxClassElementType::fgxClassType_iVar:
				if ( virtClass )
				{
					...
					virtFixup.push_back ( std::make_tuple ( &oClass, (uint32_t) vTable.size () ) );
					vTable.push_back ( compClassVTable ( 0, varPos - objectPos ) );
				} else
				{
					addElement ( elem->type,
								 elemScope,
					...
					for ( auto it = elements.begin (); it != elements.end (); it++ )
					{
						if ( (it->name == elem->name) && (it->ns == ns) )
						{
							...
							} else
							{
								...
								if ( !virtClass )
								{
									vTable.push_back ( compClassVTable ( atom, delta ) );
								}
							}
...
void compClass::make ( compExecutable &comp, opClass &oClass, bool isLS )
{
	...
	{
		vTable[std::get<1> ( it )].delta = (int16_t)(int)vTable[std::get<1> ( it )].delta + (int16_t)(int)(virtStart[std::get<0> ( it )]);
	}
...
{
	return strccmp ( elem1->name.c_str(), elem2->name.c_str () );
}
