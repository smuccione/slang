	...
	*/
	...
	if ( func->params.symbols.size() )
	{
		...
		for (auto it = func->params.symbols.begin ( ); it != func->params.symbols.end ( ); it++)
		{
			...
			{
				(*it)->initializer->right->makeAtoms ( &paramSym, &file->errHandler, isLS );
			}
		}
	}
	...
	if ( func->codeBlock ) 
	{
		func->codeBlock->makeAtoms ( &sym, &file->errHandler, isLS );
	}
}
			...
			if ( it.second.loadTimeInitializable )
			{
				...
				if ( init )
				{
					try
					{
						...
						{
							init->right->makeAtoms ( &sym, &file->errHandler, isLS );
						} else
						{
							init->makeAtoms ( &sym, &file->errHandler, isLS );
						}
					} catch ( errorNum err )
					{
					...
					case fgxClassElementType::fgxClassType_static:
						try
						{
							...
							if ( (*it2)->data.iVar.initializer->getOp () == astOp::assign ) {
								(*it2)->data.iVar.initializer->makeAtoms ( &sym, &file->errHandler, isLS );
							}
						} catch ( errorNum err )
						{
							file->errHandler.throwFatalError ( isLS, err );
