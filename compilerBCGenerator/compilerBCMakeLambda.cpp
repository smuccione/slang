...
static astNode *compMakeLambdaCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, opFunction *func )
{
	switch ( node->getOp () )
	{
				...
				if ( sym->isDefined ( node->symbolValue(), true ) )
				{
					switch ( sym->getScope ( node->symbolValue(), isAccess ) )
					{
						...
						case symbolSpaceScope::symClassStatic:
							...
							node->symbolValue () = sym->file->sCache.get ( stringi ( "::" ) + (stringi)func->classDef->name + "::" + (stringi) node->symbolValue () );
							break;
...
cacheString compExecutable::compMakeLambda ( opFile *file, opFunction *func, std::vector<symbolLocal> &closures, bool isStatic, bool isExtension, bool isLS )
{
	...
	/**************************   new method   ***********************/
	// the only time we ever need to emit a new method is if we're doing a capture... including an autoatic this
	if ( needNew )
	{
		...
		expr = buff.data<char *> ();
		{
			...
			newFunc = file->parseMethod ( src, itClass, buildString ( itClass->name.c_str (), elem->name, "method" ).c_str (), true, false );
		}
	...
	elem->type			= fgxClassElementType::fgxClassType_method;
	elem->scope			= fgxClassElementScope::fgxClassScope_public;
	elem->location		= func->nameLocation;
	...
	func->params.add ( file->selfValue, symbolTypeClass ( symbolType::symObject, itClass->name ), func->location, (new astNode ( file->sCache, astOp::symbolValue, file->selfValue ))->setLocation ( func->nameLocation ), true );
