...
static astNode *makeForEachCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, opFunction *func, bool isLS )
{
	switch ( node->getOp() )
	{
		case astOp::btForEach:
				 ...
				 node->forEachData ().in->right && node->forEachData().in->right->getType ( sym ) == symbolType::symInt )
			{
				...
				f->loopData ().block = new astNode ( *node->forEachData ().statement );
...
*/
				...
				switch ( node->forEachData().var->getOp() )
				{
					case astOp::symbolValue:
						...
						whileBlock->loopData().block->pushNode( symBlock );
						basic->addSymbol ( node->forEachData().var->symbolValue(), symbolType::symUnknown, *node->forEachData ().var );
						break;
					case astOp::seq:
						/* current */
						...
						whileBlock->loopData().block->pushNode( symBlock );
						basic->addSymbol ( node->forEachData().var->left->symbolValue(), symbolType::symUnknown, *node->forEachData ().var->left );
						...
						whileBlock->loopData().block->pushNode( symBlock );
						basic->addSymbol ( node->forEachData().var->right->symbolValue(), symbolType::symUnknown, *node->forEachData ().var->right );
						break;
