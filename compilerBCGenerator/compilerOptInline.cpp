...
static astNode *hasNamingConflictCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, symbolStack *symParent, bool &hasNamingConflict )
{
	switch ( node->getOp () )
	{
...
static astNode *hasInlineBlockerCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, bool &blockerPresent )
{
	switch ( node->getOp () )
	{
...
static astNode *hasClosureCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, bool &hasClosure )
{
	switch ( node->getOp () )
	{
		case astOp::comp:
...
static astNode *hasSymbolCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess, bool &hasSymbol, cacheString &symbolName )
{
	switch ( node->getOp () )
	{
		case astOp::symbolDef:
...
static astNode *stripRenameCB ( astNode *node, astNode *parent, symbolStack *sym, bool isAccess )
{
	switch ( node->getOp () )
	{
		case astOp::symbolValue:
...
static void paramToSymLocal ( opFunction *func, astNode *callingNode, astNode *funcNode, symbolSpaceParams *params, symbolStack *sym, astNode *object, bool isExtension )
{
	...
	if ( func->params.size () )
	{
		if ( func->params[0]->name == sym->file->selfValue )
		{
			...
			if ( object )
			{
				auto typeObject = object;
				if ( object->getOp () == astOp::assign )
				{
					typeObject = object->right;
				}
				switch ( object->getOp () )
				{
					case astOp::symbolValue:
						if ( strccmp ( object->symbolValue(), sym->file->selfValue ) || isExtension )
						{
							if ( func->params[0]->isAccessed () || func->params[0]->isAssigned () || func->params[0]->isClosedOver () || !typeObject->hasNoSideEffects ( sym, false ) )
							{
								astNode *init = (new astNode ( sym->file->sCache, astOp::assign,
												 ...
												 object
												 ))->setLocation ( func->params[0]->location );
								params->add ( func->params[0]->name, symbolTypeClass ( typeObject->getType ( sym ).getClass () ), func->params[0]->location, init, false );
							} else
							{
								delete object;
							}
			...
			if ( object )
			{
				...
				switch ( object->getOp () )
				{
					case astOp::symbolValue:
						...
						break;
					default:
						if ( !func->params[0]->isAccessed () )
						{
							if ( callingNode->pList().param[0]->hasNoSideEffects ( sym, false ) && !typeObject->hasNoSideEffects ( sym, false ) )
							{
								delete object;
								break;
							}
						}
						{
							...
							astNode *init = (new astNode ( sym->file->sCache, astOp::assign,
											  (new astNode ( sym->file->sCache, astOp::symbolValue, newSymbol ))->setLocation ( func->params[0]->location ),
											  object
											  ))->setLocation ( func->params[0]->location );

							params->add ( newSymbol, symbolTypeClass ( typeObject->getType ( sym ).getClass () ), func->params[0]->location, init, false );
						}
						break;
				}
			}
	...
	for ( uint32_t index = (uint32_t) func->params.size (); index > numHidden; index-- )
	{
			...
			switch ( callingNode->pList().param[passed]->getOp () )
			{
				case astOp::refCreate:
					if ( (callingNode->pList().param[passed]->left->getOp () != astOp::symbolValue) || hasSymbol ( sym, funcNode, callingNode->pList().param[passed]->left->symbolValue() ) )  // can't rename if inlined function already has a symbol of the same name
					{
						...
						params->add ( newSymbol, symUnknownType, callingNode->location, init, false );
					} else
					{
				...
				case astOp::symbolValue:
						 ...
						 (sym->getScope ( callingNode->pList().param[passed]->symbolValue(), true ) != symbolSpaceScope::symLocal && sym->getScope ( callingNode->pList().param[passed]->symbolValue(), true ) != symbolSpaceScope::symLocalParam) )// can't rename if inlined function already has a symbol of the same name
					{
						...
						params->add ( newSymbol, symUnknownType, callingNode->pList().param[passed]->location, init, false );
					} else
					{
		...
		} else if ( param < (uint32_t) func->params.size () )
		{
			...
			if ( newNode )
			{
				...
				params->add ( newSymbol, symUnknownType, func->params[param]->location, new astNode ( *newNode ), false );
			} else
			{
				// no default
				funcNode->renameLocal ( func->params[param]->name, newSymbol );
				params->add ( newSymbol, symUnknownType, func->params[param]->location, nullptr, false );
			}
...
static bool inlineBlock ( compExecutable *comp, astNode *block, symbolStack *sym, opFunction *func, std::vector<opFunction *> &inlineNest, bool needValue )
{
	...
	switch ( block->getOp () )
	{
		...
		case astOp::btInlineRet:
			inlineBlock ( comp, block->node(), sym, func, inlineNest, true );
			break;
		case astOp::btInline:
...
//							if ( sym->getFuncNumParams ( block->left->data.s, true ) != block->pList().param.size () + (sym->isMethod ( block->left->data.s, true ) ? 1 : 0) ) break;	// TODO: handle default parameters;
							...
							if ( childFunc->codeBlock && childFunc->weight && (childFunc->weight < 150) )
							{
								...
								if ( (memcmp ( block->left->symbolValue().c_str (), "::", 2 ) != 0) && block->left->symbolValue().find ( "::" ) != cacheString::npos && !sym->isStatic ( block->left->symbolValue(), true ) )
								{
								...
								newChild->pushNodeFront ( (new astNode ( new symbolSpaceInline ( childFunc->name ) ))->setLocation ( childFunc->codeBlock ) );
...
/* verified */
		...
		case astOp::getEnumerator:
			switch ( block->left->getCompType ( sym ) )
			{
				case symbolType::symObject:
					if ( block->left->getType ( sym ).hasClass () )
					{
						throw errorNum::scINTERNAL;			// should have been converted to function calls during type inferencing
					}
					[[fallthrough]];
				default:
					inlineBlock ( comp, block->left, sym, func, inlineNest, true );
