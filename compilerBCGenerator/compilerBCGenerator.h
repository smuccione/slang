...
public:
	...
	std::unordered_map<cacheString, size_t>::iterator	searchCache = elemMap.end();
...
public:
	...
	void annotate ( symbolStack *sym, char const *format, ... )
	{
		...
		if ( hasAnnotation )
		{
			sprintf_s ( annotation + strlen ( annotation ), sizeof ( annotation ) - strlen ( annotation ), ", " );
			vsprintf_s ( annotation + strlen ( annotation ), sizeof ( annotation ) - strlen ( annotation ), format, argptr );
		} else
		{
			vsprintf_s ( annotation, sizeof ( annotation ), format, argptr );
		}
		hasAnnotation = true;
	}
...
public:
	...
	compFixup						*fixup;					// transitory... only valid during instantiation of a particular function.
