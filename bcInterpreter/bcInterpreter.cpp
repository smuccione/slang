			...
			if ( sPtr->type == slangType::ePCALL )
			{
				...
				for ( tcEntry = parentFunc->tcList; tcEntry->tryOffset != 0xFFFFFFFF; tcEntry++ )
				{
					if ( (opOffset >= tcEntry->tryOffset) && (opOffset < tcEntry->catchOffset) )
					{
						if ( opOffset <= (tcEntry + 1)->tryOffset )
						{
							// we will catch this later on, so rethrow and allow the catcher to get it
							throw;
						}
					}
				}
...
void vmInstance::printCallStack ( VAR *stack, fglOp *op )
{
	...
	while ( evalPtr >= eval )
	{
		...
		{
			callStack->entry.push_back ( vmCallstackEntry ( this, evalPtr->dat.pCall.func, debug->debugGetCurrentLine ( evalPtr->dat.pCall.func, op ), evalPtr->dat.pCall.nParams, evalPtr ) );
	...
	{
		printf ( "%s(%i) %s ( ", it.fileName, (int)it.line, it.funcName );
...
void vmInstance::run ( char const *name, bool halt )
{
	...
	} catch ( errorNum err )
	{
		if ( err == errorNum::scUSER_TERMINATE )
		{
			if ( error.valid )
			{
				...
				printCallStack ( error.stack, (fglOp *)error.opCode );
			}
		} else
		{
			printf ( "error: %x - %s\r\n", err, scCompErrorAsText ( int ( err ) ).c_str () );
	...
	{
		UNUSED ( var );
		printf ( "error: user defined error\r\n" );
		printCallStack ( error.stack, (fglOp *)error.opCode );
		debugSignal ( this, vmStateType::vmThreadEnd, nullptr, funcDef, nullptr );
		throw;
	} catch ( ... )
	{
		printf ( "error: internal vm error\r\n" );
		printCallStack ( error.stack, (fglOp *)error.opCode );
		debugSignal ( this, vmStateType::vmThreadEnd, nullptr, funcDef, nullptr );
		throw;
	}
