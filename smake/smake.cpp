...
maker::maker ()
{
	...
	functionList = {
		...
		{ stringi ( "file" ),		{ 1,	&maker::macroFuncFile, true } },
...
bool maker::makeTarget ( stringi const &target, std::promise<bool> &&run, bool sharedState, bool topInclude )
{
			...
			for ( auto const &targetIt : rule.targets )
			{
				if ( targetMatch ( target, targetIt, stem ) )
				{
					...
					for ( auto const &dep : rule.dependants )
					{
						...
						prereqName = applySubstitutions ( prereqName, topInclude );
						...
						for ( auto const &d : dep2 )
						{
							if ( jobServer && rule.dependants.size () > 1 )
							{
								...
								builderThreads.emplace_back ( std::thread ( &maker::makeTarget, this, d, std::move ( p ), true, topInclude ) );			// TODO: speed this up by using worker queues
							} else
							{
								std::promise<bool>	p;
								if ( std::get<1> ( dep ) )
								{
									makeTarget ( d, std::move ( p ), false, topInclude );
								} else
								{
									runRecipe |= makeTarget ( d, std::move ( p ), false, topInclude );
								}
								if ( fatalBuild )
								{
									SetEvent ( std::get<0> ( buildStatus->second ) );
					...
					if ( runRecipe && !fatalBuild )
					{
						...
						if ( rule.rules.size () )
						{
							...
							for ( auto const &rule : rule.rules )
							{
								stringi r = rule.findAndReplace ( "$@", target );
								...
								auto c = applySubstitutions ( r, topInclude ).trim ();
								if ( c.size () )
								{
									cmds.push_back ( c );
								}
...
stringi maker::applySubstitutions ( stringi const &line, bool topInclude )
{
	stringi res;
			...
			if ( !memcmp ( ptr, "$(", 2 ) )
			{
				...
				if ( functionList.find ( token ) != functionList.end () )
				{
					...
					if ( std::get<2> ( func->second ) )
					{
						for ( auto &it : params )
						{
							it = applySubstitutions ( it, topInclude );
						}
					}
					res += (this->*(std::get<1> ( func->second ))) (params, topInclude);
					while ( isspace ( ptr ) ) ptr++;
					if ( *ptr != ')' )
					{
						throw "missing )";
				...
				} else if ( *ptr == ':' )
				{
					...
					if ( subs != variables.end () )
					{
						...
						auto var = subs->second.subst;
						for ( auto &pat : pats )
						{
							auto params = getParamVec ( pat, "=" );
							for ( auto &it : params )
							{
								it = applySubstitutions ( it, topInclude ).trim ();
							}
							if ( params.size () != 2 )
							{
				...
				} else if ( *ptr == ')' )
				{
					...
					if ( subs != variables.end () )
					{
						res += applySubstitutions ( subs->second.subst, topInclude );
					} else
					{
						//						throw "unknown variable";
					}
...
bool maker::testConditional ( char const *&linePtr, bool topInclude )
{
	if ( *linePtr != '(' )
	{
		throw "missing '('";
...
void maker::processStream ( std::istream &inFile, bool topInclude )
{
	...
	// comments start with #
		if ( inLine[0] != '#' )
		{
			if ( !inLine.size () || (inLine.size () && inLine[inLine.size () - 1] != '\\') )
			{
				...
				{
					line += " ";
					line += (std::string)stringi ( inLine ).trim ();
				} else
				{
					line = inLine;
				}
				...
				if ( condStack.top ().first )
				{
					...
					{
						execute = testConditional ( linePtr, topInclude );
					...
					} else if ( !memcmp ( linePtr, "::=", 3 ) )
					{
						...
						linePtr += 3;
						firstToken = applySubstitutions ( firstToken, topInclude );
						variables[firstToken] = variable ( applySubstitutions ( linePtr, topInclude ), maker::origin::o_file  );
					...
					} else if ( !memcmp ( linePtr, ":", 1 ) )
					{
						...
						if ( depLine.size () )
						{
							auto dep = applySubstitutions ( depLine.c_str (), topInclude );
					...
					{
						firstToken = applySubstitutions ( linePtr, topInclude );
						...
						for ( auto &it : incVec )
						{
							...
							processStream ( includeFile, false );
						}
					} else if ( firstToken == "-include" )
					{
						firstToken = applySubstitutions ( linePtr, topInclude );
					...
					{
						firstToken = applySubstitutions ( getToken ( linePtr ), topInclude );
						...
						{
							// POSIX simply expanded variable
							variables[firstToken] = variable ( applySubstitutions ( defineLine, topInclude ), maker::origin::o_file );
						} else if ( op == "?:=" )
						{
							// conditional expansion of variable
							if ( variables.find ( firstToken ) != variables.end () )
					...
					{
						std::stringstream stream ( (std::string) applySubstitutions ( line, topInclude ).trim () );
			...
			} else
			{
				...
				line += " ";
				line += (std::string)stringi ( inLine.substr ( 0, inLine.size () - 1 ) ).trim ();
			}
		}
	}
}
...
//	auto outPipe = CreateNamedPipe ( "\\\\.\\pipe\\smake.out.pipe", PIPE_ACCESS_DUPLEX, FILE_FLAG_FIRST_PIPE_INSTANCE, 
	...
	try
	{
		...
		try
		{
			...
			m.processStream ( inFile, true );

			// process the targets and generate the list of things to build
			for ( auto &it : targets )
			{
				std::promise<bool> p;
				m.makeTarget ( it, std::move ( p ), false, true );
			}
